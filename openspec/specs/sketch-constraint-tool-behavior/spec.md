...
The system SHALL expose Concentric, Midpoint, Normal, Pierce, Symmetric, and Fix Geometry as sketch-mode constraint tools with stable tool IDs, tooltips, toolbar grouping, and dedicated SVG icons.

#### Scenario: User enters sketch mode
- **WHEN** the sketch toolbar is built for an active sketch session
- **THEN** it includes `constraintConcentric`, `constraintMidpoint`, `constraintNormal`, `constraintPierce`, `constraintSymmetric`, and `constraintFix` in the constraint tool group
