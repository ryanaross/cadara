...
While a sketch is being edited, the history area SHALL show an ordered sketch-local history for the active sketch instead of the normal document history items.

#### Scenario: Render sketch-local history
- **WHEN** an active sketch edit session is present
- **THEN** the history area shows sketch entities, constraints, and dimensions for that sketch in sketch history order
...
- **WHEN** the user moves the sketch history cursor to an earlier sketch-local item
- **THEN** sketch-local items after that cursor are treated as after the current cursor
- **AND** the displayed sketch geometry and annotations reflect the active sketch cursor position
...
- **THEN** the new sketch-local item is inserted after the current sketch cursor
- **AND** the sketch cursor advances to the new item
