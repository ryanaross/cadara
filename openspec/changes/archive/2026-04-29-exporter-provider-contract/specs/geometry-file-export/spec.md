...
**Reason**: Metadata (filename, extension, MIME type) is now declared as static properties on each `ExportProvider` rather than computed per-result. The orchestrator constructs result metadata from provider properties.
**Migration**: Provider implementations declare `fileExtension` and `mimeType` as interface properties. The orchestrator uses these when building the download result.
