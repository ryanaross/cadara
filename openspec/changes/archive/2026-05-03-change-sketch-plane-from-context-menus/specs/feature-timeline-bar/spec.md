...
The bottom document history bar SHALL expose context-menu Rename, Edit, Roll History Here, Roll To End, and Delete actions for every supported committed document history item without requiring item-kind-specific presentation code. Committed sketch items that participate in support-plane reassignment SHALL also expose `Change Sketch Plane`. Feature-only actions such as Suppress SHALL remain available only for committed feature items.
...
#### Scenario: Sketch history item menu includes sketch plane action
- **WHEN** the user opens the context menu for an eligible committed sketch history item
- **THEN** the menu includes Rename, Edit, Change Sketch Plane, Roll History Here, Roll To End, and Delete for that item

#### Scenario: Change sketch plane from sketch history item
- **WHEN** the user selects `Change Sketch Plane` from the context menu for an eligible committed sketch history item in the bottom history bar
