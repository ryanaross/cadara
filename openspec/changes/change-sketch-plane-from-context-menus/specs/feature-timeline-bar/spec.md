...
The bottom document history bar SHALL expose context-menu Rename, Edit, Roll History Here, Roll To End, and Delete actions for every supported committed document history item without requiring item-kind-specific presentation code. Committed sketch items that participate in origin-plane reassignment SHALL also expose `Change Sketch Plane`. Feature-only actions such as Suppress SHALL remain available only for committed feature items.
