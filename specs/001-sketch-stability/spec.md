...
A user can start a sketch by selecting one of the origin planes directly in the 3D viewport instead of being forced to use the feature tree.

**Why this priority**: Sketch creation is blocked if datum planes in the viewport cannot be selected.

**Independent Test**: Can be fully tested by activating `Sketch`, clicking each visible origin plane in the viewport, and confirming that sketch mode opens on the selected plane.
...
- What happens when the user clicks near the border of overlapping datum planes and body geometry while `Sketch` is armed?
...
- **FR-001**: The system MUST expose selectable viewport render geometry for each visible construction origin plane used to start a sketch.
...
- **Construction Plane Renderable**: The viewport-visible geometry and picking binding that makes a construction plane selectable in the scene.
...
- **SC-001**: A user can open a sketch from each origin plane in the viewport in one click after activating `Sketch`.
...
- The current scope is limited to primary datum planes and existing sketch reopen flows already represented in the editor state machine.
