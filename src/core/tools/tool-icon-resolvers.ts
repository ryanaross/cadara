...
export function getFeatureSnapshotToolIcon(
  feature: FeatureSnapshotRecord | null,
): ToolIconId | null {
  if (!feature) {
    return null;
  }
  ...
  return (
    getRegisteredFeatureAuthoringDefinitions().find(
      (definition) => definition.metadata.kind === feature.definition.kind,
    )?.metadata.icon ?? null
  );
}
...
export function getDocumentHistoryItemToolIcon(
  ...
  const feature =
    features.find((entry) => entry.featureId === item.featureId) ?? null;
  return getFeatureSnapshotToolIcon(feature);
}

export function getObjectTreeNodeToolIcon(
  item: ObjectTreeNodeRecord,
): ToolIconId | null {
  switch (item.kind) {
    case "sketch":
      return "sketch";
...
export function getSketchHistoryItemToolIcon(
  ...
  switch (item.kind) {
    ...
    case "entity": {
      const entity =
        definition.entities.find((entry) => entry.entityId === item.id) ?? null;
      return entity ? getSketchEntityToolIcon(entity) : null;
    }
    case "constraint": {
      const constraint =
        definition.constraints.find(
          (entry) => entry.constraintId === item.id,
        ) ?? null;
      return constraint ? getSketchConstraintToolIcon(constraint) : null;
    }
    case "dimension": {
      const dimension =
        definition.dimensions.find((entry) => entry.dimensionId === item.id) ??
        null;
...
function getSketchConstraintToolIcon(
