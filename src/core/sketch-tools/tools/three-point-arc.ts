...
function buildThreePointArcPreview(
  ...
  if (points.length < 3) {
    return [
      {
        id: "preview-three-point-arc-chord",
        kind: "line",
        start: points[0]!,
        end: points[1]!,
        entityId: null,
        status: "preview",
        label: "3-point arc chord",
  ...
  const sweepDirection = getArcSweepDirectionThroughPoint(
    circle.center,
    points[0]!,
    points[1]!,
    points[2]!,
...
function validateThreePointArc(
  points: readonly SketchPoint[],
): SketchToolValidationResult {
  if (points.length < REQUIRED_POINTS) {
    return {
      valid: false,
      message: `3-point arc requires ${REQUIRED_POINTS} points.`,
    };
  }
...
function buildThreePointArcPresentation(
  state: SketchToolRuntimeState,
  ...
  return {
    prompts: [
      {
        id: "three-point-arc-prompt",
        text: ready ? "Place arc" : "Pick arc points",
        tone: validation.length > 0 ? "warning" : "neutral",
      },
    ],
    steps: [
      {
        id: "three-point-arc-step",
        label: `${Math.min(points.length, REQUIRED_POINTS)}/${REQUIRED_POINTS} points`,
      },
    ],
    completionHints: [
      {
        id: "three-point-arc-completion",
        text: ready
    ...
    overlays: [
      ...getPlacedPoints(state).map((point, index) => ({
        id: `three-point-arc-point-${index}`,
...
export const threePointArcSketchToolDefinition: SketchToolDefinition<"threePointArc"> =
  {
    ...
    activate() {
      ...
      return {
        state,
        stagedEntities: [],
        presentation: buildThreePointArcPresentation(state),
      };
    },
    pointerMove({ state, point }) {
      ...
      return {
        state: nextState,
        stagedEntities: buildThreePointArcPreview(previewPoints),
        presentation: buildThreePointArcPresentation(nextState),
      };
    },
    pointerRelease({ state, point }) {
      if (!point) {
        return {
          state,
          stagedEntities: buildThreePointArcPreview(getPlacedPoints(state)),
          presentation: buildThreePointArcPresentation(state),
        };
      }
      ...
      return {
        state: nextState,
        stagedEntities: complete ? [] : buildThreePointArcPreview(nextPoints),
        presentation: buildThreePointArcPresentation(nextState),
      };
    },
      ...
      return {
        points: [
          factories.createPoint(
            `3-point arc ${sequence} center`,
            centerPointId,
            circle.center,
          ),
          factories.createPoint(
            `3-point arc ${sequence} start`,
            startPointId,
            arcPoints[0]!,
          ),
          factories.createPoint(
            `3-point arc ${sequence} through`,
            throughPointId,
            arcPoints[1]!,
          ),
          factories.createPoint(
            `3-point arc ${sequence} end`,
            endPointId,
            arcPoints[2]!,
          ),
        ],
        ...
        constraints: [
          createPointOnCurveConstraint({
            constraintId: factories.createConstraintId(
              "three-point-arc-through",
            ),
            label: `3-point arc ${sequence} through point`,
            pointId: throughPointId,
            curveEntityId: entityId,
          }),
        ],
        dimensions: createArcEndpointDimensions({
          createDimensionId: factories.createDimensionId,
          labelPrefix: `3-point arc ${sequence}`,
          entityId,
          startPointId,
          endPointId,
        }),
