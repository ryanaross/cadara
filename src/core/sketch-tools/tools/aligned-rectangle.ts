...
function buildAlignedRectanglePreview(
  ...
  return [
    {
      id: "preview-aligned-rect-bottom",
      kind: "line",
      start: corners.first,
      end: corners.second,
      entityId: null,
      status: "preview",
      label: "Aligned rectangle preview bottom",
      isConstruction: false,
    },
    {
      id: "preview-aligned-rect-right",
      kind: "line",
      start: corners.second,
      end: corners.third,
      entityId: null,
      status: "preview",
      label: "Aligned rectangle preview right",
      isConstruction: false,
    },
    {
      id: "preview-aligned-rect-top",
      kind: "line",
      start: corners.third,
      end: corners.fourth,
      entityId: null,
      status: "preview",
      label: "Aligned rectangle preview top",
      isConstruction: false,
    },
    {
      id: "preview-aligned-rect-left",
      kind: "line",
      start: corners.fourth,
      end: corners.first,
      entityId: null,
      status: "preview",
      label: "Aligned rectangle preview left",
      isConstruction: false,
    },
  ];
}

function validateAlignedRectanglePoints(
  points: readonly SketchPoint[],
): SketchToolValidationResult {
  if (points.length < REQUIRED_POINTS) {
    return {
      valid: false,
      message: `Aligned rectangle requires ${REQUIRED_POINTS} points.`,
    };
  }
  ...
  if (!pointsDistinct(points[0]!, points[1]!)) {
    return {
      valid: false,
      message: "Aligned rectangle first edge must have length.",
    };
  }
  ...
  if (!corners || corners.height <= SKETCH_TOOL_EPSILON) {
    return {
      valid: false,
      message: "Aligned rectangle height must be greater than zero.",
    };
  }
...
function buildAlignedRectanglePresentation(
  state: SketchToolRuntimeState,
  ...
  const validation = state.validationMessage
    ? [
        {
          id: "aligned-rectangle-validation",
  ...
  const corners =
    points.length >= 3
      ? getAlignedRectangleCorners(points[0]!, points[1]!, points[2]!)
      : null;
  ...
  return {
    ...
    measurements: corners
      ? [
          {
            id: "aligned-rectangle-width",
            label: "Width",
            value: corners.width,
            unit: "mm",
          },
          {
            id: "aligned-rectangle-height",
            label: "Height",
            value: corners.height,
            unit: "mm",
          },
        ]
      : [],
    completionHints: [
      {
        ...
        text: ready
          ? "Click to accept the aligned rectangle"
...
export const alignedRectangleSketchToolDefinition: SketchToolDefinition<"alignedRectangle"> =
  {
    ...
    activate() {
      ...
      return {
        state,
        stagedEntities: [],
        presentation: buildAlignedRectanglePresentation(state),
      };
    },
    ...
    pointerRelease({ state, point }) {
      if (!point) {
        return {
          state,
          stagedEntities: buildAlignedRectanglePreview(getPlacedPoints(state)),
          presentation: buildAlignedRectanglePresentation(state),
        };
      }
      ...
      return {
        ...
        stagedEntities: complete
          ? []
          : buildAlignedRectanglePreview(nextPoints),
        presentation: buildAlignedRectanglePresentation(nextState),
      };
    },
      ...
      return {
        points: [
          factories.createPoint(
            `Aligned rectangle ${sequence} first`,
            cornerIds[0],
            corners.first,
          ),
          factories.createPoint(
            `Aligned rectangle ${sequence} second`,
            cornerIds[1],
            corners.second,
          ),
          factories.createPoint(
            `Aligned rectangle ${sequence} third`,
            cornerIds[2],
            corners.third,
          ),
          factories.createPoint(
            `Aligned rectangle ${sequence} fourth`,
            cornerIds[3],
            corners.fourth,
          ),
        ],
        entities: [
          factories.createLineEntity(
            `Aligned rectangle ${sequence} bottom`,
            entityIds[0],
            cornerIds[0],
            cornerIds[1],
          ),
          factories.createLineEntity(
            `Aligned rectangle ${sequence} right`,
            entityIds[1],
            cornerIds[1],
            cornerIds[2],
          ),
          factories.createLineEntity(
            `Aligned rectangle ${sequence} top`,
            entityIds[2],
            cornerIds[2],
            cornerIds[3],
          ),
          factories.createLineEntity(
            `Aligned rectangle ${sequence} left`,
            entityIds[3],
            cornerIds[3],
            cornerIds[0],
          ),
        ...
        constraints: [
          createParallelConstraint({
            constraintId: factories.createConstraintId(
              "aligned-rect-horizontal-pair",
            ),
            label: `Aligned rectangle ${sequence} opposite edges`,
            entityIds: [entityIds[0], entityIds[2]],
          }),
          createParallelConstraint({
            constraintId: factories.createConstraintId(
              "aligned-rect-vertical-pair",
            ),
            label: `Aligned rectangle ${sequence} side edges`,
            entityIds: [entityIds[1], entityIds[3]],
          }),
          createPerpendicularConstraint({
            constraintId: factories.createConstraintId("aligned-rect-corner"),
            label: `Aligned rectangle ${sequence} corner`,
            entityIds: [entityIds[0], entityIds[1]],
          }),
          createEqualLengthConstraint({
            constraintId: factories.createConstraintId("aligned-rect-widths"),
            label: `Aligned rectangle ${sequence} equal widths`,
            entityIds: [entityIds[0], entityIds[2]],
          }),
          createEqualLengthConstraint({
            constraintId: factories.createConstraintId("aligned-rect-heights"),
            label: `Aligned rectangle ${sequence} equal heights`,
            entityIds: [entityIds[1], entityIds[3]],
          }),
        ],
        dimensions: [
          createDistanceDimension({
            dimensionId: factories.createDimensionId("aligned-rect-width"),
            label: `Aligned rectangle ${sequence} width`,
            axis: "aligned",
            pointIds: [cornerIds[0], cornerIds[1]],
            value: corners.width,
          }),
          createDistanceDimension({
            dimensionId: factories.createDimensionId("aligned-rect-height"),
            label: `Aligned rectangle ${sequence} height`,
            axis: "aligned",
            pointIds: [cornerIds[1], cornerIds[2]],
            value: corners.height,
          }),
        ],
      };
