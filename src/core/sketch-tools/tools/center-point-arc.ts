...
import { pointsDistinct, sampleArcPoints } from "@/core/sketch-tools/geometry";
...
function buildCenterPointArcPreview(
...
function validateCenterPointArc(
  points: readonly SketchPoint[],
): SketchToolValidationResult {
  if (points.length < REQUIRED_POINTS) {
    return {
      valid: false,
      message: `Center-point arc requires ${REQUIRED_POINTS} points.`,
    };
  }
...
function buildCenterPointArcPresentation(
  state: SketchToolRuntimeState,
  ...
  const validation = state.validationMessage
    ? [
        {
          id: "center-point-arc-validation",
  ...
  return {
    prompts: [
      {
        id: "center-point-arc-prompt",
        text: points.length < 2 ? "Pick center and start" : "Place arc end",
        tone: validation.length > 0 ? "warning" : "neutral",
      },
    ],
    steps: [
      {
        id: "center-point-arc-step",
    ...
    completionHints: [
      {
        id: "center-point-arc-completion",
        text: ready
          ? "Click to accept the arc"
    ...
    overlays: [
      ...getPlacedPoints(state).map((point, index) => ({
        id: `center-point-arc-point-${index}`,
...
export const centerPointArcSketchToolDefinition: SketchToolDefinition<"centerPointArc"> =
  {
    ...
    activate() {
      ...
      return {
        state,
        stagedEntities: [],
        presentation: buildCenterPointArcPresentation(state),
      };
    },
    pointerMove({ state, point }) {
      ...
      return {
        state: nextState,
        stagedEntities: buildCenterPointArcPreview(previewPoints),
        presentation: buildCenterPointArcPresentation(nextState),
      };
    },
    pointerRelease({ state, point }) {
      if (!point) {
        return {
          state,
          stagedEntities: buildCenterPointArcPreview(getPlacedPoints(state)),
          presentation: buildCenterPointArcPresentation(state),
        };
      }
      ...
      return {
        state: nextState,
        stagedEntities: complete ? [] : buildCenterPointArcPreview(nextPoints),
        presentation: buildCenterPointArcPresentation(nextState),
      };
    },
      ...
      return {
        points: [
          factories.createPoint(
            `Center arc ${sequence} center`,
            centerPointId,
            arcPoints[0]!,
          ),
          factories.createPoint(
            `Center arc ${sequence} start`,
            startPointId,
            arcPoints[1]!,
          ),
          factories.createPoint(
            `Center arc ${sequence} end`,
            endPointId,
            arcPoints[2]!,
          ),
        ],
        ...
        dimensions: [
          createDistanceDimension({
            dimensionId: factories.createDimensionId("center-arc-radius"),
            label: `Center arc ${sequence} radius`,
            axis: "aligned",
            pointIds: [centerPointId, startPointId],
            value: distanceBetween(arcPoints[0]!, arcPoints[1]!),
          }),
          ...createArcEndpointDimensions({
            createDimensionId: factories.createDimensionId,
            labelPrefix: `Center arc ${sequence}`,
            entityId,
            startPointId,
            endPointId,
          }),
