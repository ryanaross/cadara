...
import { createMidpointConstraint } from "@/core/sketch-tools/constraints";
import { mirrorPointAcrossCenter } from "@/core/sketch-tools/geometry";
import {
...
function buildMidpointLinePreview(
...
function validateMidpointLine(midpoint: SketchPoint, endpoint: SketchPoint) {
  return validateDistance(
    midpoint,
    endpoint,
    "Midpoint line requires a midpoint and distinct endpoint.",
  );
}

function buildMidpointLinePresentation(
  ...
  const length = isDrawing
    ? distanceBetween(mirrorPointAcrossCenter(start, end), end)
    : 0;
  const validation = state.validationMessage
    ? [
        {
          id: "midpoint-line-validation",
  ...
  return {
    ...
    steps: [
      { id: "midpoint-line-step", label: isDrawing ? "Endpoint" : "Midpoint" },
    ],
    measurements: isDrawing
      ? [
          {
            id: "midpoint-line-length",
            label: "Length",
            value: length,
            unit: "mm",
          },
    ...
    completionHints: [
      {
        ...
        text: isDrawing
          ? "Click to accept the midpoint line"
          : "Click to set the midpoint",
        ...
        return {
          points: [
            factories.createPoint(
              `Midpoint line ${sequence} midpoint`,
              midpointId,
              start,
            ),
            factories.createPoint(
              `Midpoint line ${sequence} start`,
              startPointId,
              reflected,
            ),
            factories.createPoint(
              `Midpoint line ${sequence} end`,
              endPointId,
              end,
            ),
          ],
