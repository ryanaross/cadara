...
import type { SketchToolControlValue, SketchToolPresentationSchema } from '@/core/sketch-tools/editor-schema'
import {
  addPoints,
  dotPoints,
  normalizePoint,
...
type FixedPointToolConfig = {
  ...
  readyPrompt: string
  cursorLabel: string
  validate(points: readonly SketchPoint[], state: SketchToolRuntimeState): SketchToolValidationResult
  buildPreview(points: readonly SketchPoint[], state: SketchToolRuntimeState): readonly SketchDraftEntity[]
  createCommitContribution(input: {
    sequence: number
    points: readonly SketchPoint[]
    state: SketchToolRuntimeState
    factories: Parameters<SketchToolDefinition['createCommitContribution']>[0]['factories']
  }): SketchToolCommitContribution
  controls?(state: SketchToolRuntimeState): SketchToolPresentationSchema['controls']
}
...
function advanceFixedPointTool(
  ...
  const validation = candidatePoints.length >= config.requiredPointCount
    ? config.validate(candidatePoints, state)
    : { valid: false, message: null }

  if (candidatePoints.length >= config.requiredPointCount && !validation.valid) {
    const nextState = {
      ...state,
      status: 'drawing',
      livePoint: point,
    ...
    return {
      state: nextState,
      stagedEntities: config.buildPreview(getPreviewPoints(nextState), nextState),
  ...
  return {
    state: nextState,
    stagedEntities: complete ? [] : config.buildPreview(candidatePoints, nextState),
    presentation: buildFixedPointPresentation(nextState, config),
  }
}
...
function buildFixedPointPresentation(
  ...
  const validation = state.validationMessage
    ? [{ id: `${config.id}-validation`, message: state.validationMessage, severity: 'error' as const }]
    : []
  const ready = points.length >= config.requiredPointCount && config.validate(points, state).valid
  ...
  return {
    ...
    steps: [{ id: `${config.id}-step`, label: `${Math.min(points.length, config.requiredPointCount)}/${config.requiredPointCount} points` }],
    cursor: { id: `${config.id}-cursor`, label: config.cursorLabel, icon: 'crosshair' },
    controls: config.controls?.(state),
    completionHints: [{
      id: `${config.id}-completion`,
      text: ready ? `Click to accept ${config.name.toLowerCase()}` : `Place ${remaining} more point${remaining === 1 ? '' : 's'}`,
      ready,
    }],
...
function createFixedPointTool<TToolId extends SketchToolDefinition['metadata']['id']>(
  ...
  return {
    ...
    activate() {
      ...
      return { state, stagedEntities: [], presentation: buildFixedPointPresentation(state, config) }
    },
    ...
    validate(start, end) {
      return config.validate([start, end], { status: 'drawing', pointerDownPoint: start, livePoint: end, placedPoints: [start, end], settings: {}, validationMessage: null })
    },
    ...
    createCommitContribution({ sequence, points, settings, factories }) {
      return config.createCommitContribution({
        sequence,
        points: points ?? [],
        state: { status: 'idle', pointerDownPoint: null, livePoint: null, placedPoints: points ?? [], settings: settings ?? {}, validationMessage: null },
        factories,
      })
    },
  }
...
function minorRadiusFromPoint(center: SketchPoint, majorAxisEndpoint: SketchPoint, minorPoint: SketchPoint) {
...
function sampleEllipsePoints(center: SketchPoint, majorAxisEndpoint: SketchPoint, minorRadius: number) {
...
function sweepBetween(startAngle: number, endAngle: number, direction: 'clockwise' | 'counterClockwise') {
...
function sampleEllipticalArcPoints(
  ...
  const ellipseAngle = (point: SketchPoint) => {
    ...
    return Math.atan2(dotPoints(delta, minorUnit) / minorRadius, dotPoints(delta, majorUnit) / majorRadius)
  }
  ...
  return Array.from({ length: ADVANCED_SAMPLE_COUNT }, (_, index) => {
    ...
    const angle = direction === 'counterClockwise' ? startAngle + sweep * t : startAngle - sweep * t
    return addPoints(
      center,
      addPoints(
        scalePoint(majorUnit, Math.cos(angle) * majorRadius),
...
function sampleConicPoints(start: SketchPoint, control: SketchPoint, end: SketchPoint, rho: number) {
  return Array.from({ length: ADVANCED_SAMPLE_COUNT }, (_, index) => {
    const t = index / (ADVANCED_SAMPLE_COUNT - 1)
    const oneMinusT = 1 - t
...
function getSetting(state: SketchToolRuntimeState, key: string, fallback: SketchToolControlValue) {
...
function getHorizontalAlign(state: SketchToolRuntimeState): 'left' | 'center' | 'right' {
  const value = getSetting(state, 'horizontalAlign', 'left')
  return value === 'center' || value === 'right' ? value : 'left'
}

function getVerticalAlign(state: SketchToolRuntimeState): 'baseline' | 'middle' | 'top' | 'bottom' {
  const value = getSetting(state, 'verticalAlign', 'baseline')
  return value === 'middle' || value === 'top' || value === 'bottom' ? value : 'baseline'
}

function sampleProfileTextOutline(state: SketchToolRuntimeState, anchor: SketchPoint, heightPoint: SketchPoint) {
  ...
  const y = verticalAlign === 'middle'
    ...
    : verticalAlign === 'top'
      ...
      : verticalAlign === 'baseline'
        ? -height * 0.2
        : 0
  const angle = Math.atan2(heightPoint[1] - anchor[1], heightPoint[0] - anchor[0])
...
function pointPolylinePreview(id: string, label: string, points: readonly SketchPoint[], isClosed = false): readonly SketchDraftEntity[] {
  return points.length >= 2
    ? [{ id, kind: 'polyline', points, isClosed, entityId: null, status: 'preview', label, isConstruction: false }]
    : []
}
...
export const ellipseSketchToolDefinition = createFixedPointTool({
  ...
  validate(points) {
    if (points.length < 3) {
      return { valid: false, message: 'Ellipse requires center, major axis, and minor radius points.' }
    }
    const minorRadius = minorRadiusFromPoint(points[0]!, points[1]!, points[2]!)
    return pointsDistinct(points[0]!, points[1]!) && minorRadius > EPSILON
      ? { valid: true, message: null }
      : { valid: false, message: 'Ellipse requires non-zero major and minor radii.' }
  },
  buildPreview(points) {
    if (points.length < 2) {
      return []
    ...
    if (points.length === 2) {
      return [{ id: 'preview-ellipse-major-axis', kind: 'line', start: points[0]!, end: points[1]!, entityId: null, status: 'preview', label: 'Ellipse major axis', isConstruction: true }]
    }
    return pointPolylinePreview('preview-ellipse', 'Ellipse preview', sampleEllipsePoints(points[0]!, points[1]!, minorRadiusFromPoint(points[0]!, points[1]!, points[2]!)), true)
  },
  createCommitContribution({ sequence, points, factories }) {
    ...
    return {
      points: [
        factories.createPoint(`Ellipse ${sequence} center`, centerPointId, points[0]!),
        factories.createPoint(`Ellipse ${sequence} major`, majorPointId, points[1]!),
      ],
      entities: [
        factories.createEllipseEntity(`Ellipse ${sequence}`, entityId, centerPointId, majorPointId, minorRadiusFromPoint(points[0]!, points[1]!, points[2]!)),
      ],
    }
  },
})
...
export const ellipticalArcSketchToolDefinition = createFixedPointTool({
  ...
  validate(points) {
    if (points.length < 5) {
      return { valid: false, message: 'Elliptical arc requires center, major, minor, start, and end points.' }
    }
    const minorRadius = minorRadiusFromPoint(points[0]!, points[1]!, points[2]!)
    return pointsDistinct(points[0]!, points[1]!)
      ...
      && minorRadius > EPSILON
      ? { valid: true, message: null }
      : { valid: false, message: 'Elliptical arc requires non-zero axes and distinct start/end points.' }
  },
  buildPreview(points) {
    if (points.length < 3) {
      return points.length === 2
        ? [{ id: 'preview-elliptical-arc-major-axis', kind: 'line', start: points[0]!, end: points[1]!, entityId: null, status: 'preview', label: 'Elliptical arc major axis', isConstruction: true }]
        : []
    }
    if (points.length < 5) {
      return pointPolylinePreview('preview-elliptical-arc-ellipse', 'Elliptical arc ellipse preview', sampleEllipsePoints(points[0]!, points[1]!, minorRadiusFromPoint(points[0]!, points[1]!, points[2]!)), true)
    }
    return pointPolylinePreview('preview-elliptical-arc', 'Elliptical arc preview', sampleEllipticalArcPoints(points[0]!, points[1]!, minorRadiusFromPoint(points[0]!, points[1]!, points[2]!), points[3]!, points[4]!, 'counterClockwise'))
  },
  createCommitContribution({ sequence, points, factories }) {
    ...
    return {
      points: [
        factories.createPoint(`Elliptical arc ${sequence} center`, centerPointId, points[0]!),
        factories.createPoint(`Elliptical arc ${sequence} major`, majorPointId, points[1]!),
        factories.createPoint(`Elliptical arc ${sequence} start`, startPointId, points[3]!),
        factories.createPoint(`Elliptical arc ${sequence} end`, endPointId, points[4]!),
      ],
      entities: [
        factories.createEllipticalArcEntity(`Elliptical arc ${sequence}`, entityId, centerPointId, majorPointId, startPointId, endPointId, minorRadiusFromPoint(points[0]!, points[1]!, points[2]!), 'counterClockwise'),
      ],
    }
  },
})
...
export const conicSketchToolDefinition = createFixedPointTool({
  ...
  validate(points) {
    return points.length >= 3 && distinctPoints(points.slice(0, 3))
      ? { valid: true, message: null }
      : { valid: false, message: 'Conic requires three distinct defining points.' }
  },
  buildPreview(points) {
    return points.length < 3
      ? controlLines(points, 'preview-conic')
      : [...controlLines(points, 'preview-conic'), ...pointPolylinePreview('preview-conic', 'Conic preview', sampleConicPoints(points[0]!, points[1]!, points[2]!, 0.5))]
  },
  createCommitContribution({ sequence, points, factories }) {
    ...
    return {
      points: [
        factories.createPoint(`Conic ${sequence} start`, startPointId, points[0]!),
        factories.createPoint(`Conic ${sequence} control`, controlPointId, points[1]!),
        factories.createPoint(`Conic ${sequence} end`, endPointId, points[2]!),
      ],
      entities: [
        factories.createConicEntity(`Conic ${sequence}`, entityId, startPointId, controlPointId, endPointId, 0.5),
      ],
    }
  },
})
...
export const bezierCurveSketchToolDefinition = createFixedPointTool({
  ...
  validate(points) {
    return points.length >= 4 && distinctPoints(points.slice(0, 4))
      ? { valid: true, message: null }
      : { valid: false, message: 'Bezier curve requires four distinct control points.' }
  },
  buildPreview(points) {
    return points.length < 4
      ? controlLines(points, 'preview-bezier')
      : [...controlLines(points, 'preview-bezier'), ...pointPolylinePreview('preview-bezier', 'Bezier preview', sampleBezierPoints(points.slice(0, 4)))]
  },
  createCommitContribution({ sequence, points, factories }) {
    const pointIds = points.slice(0, 4).map((_, index) => factories.createPointId(`bezier-control-${index + 1}`))
    const entityId = factories.createEntityId('bezier')
    return {
      points: points.slice(0, 4).map((point, index) =>
        factories.createPoint(`Bezier ${sequence} control ${index + 1}`, pointIds[index]!, point),
      ),
      entities: [
        factories.createBezierCurveEntity(`Bezier ${sequence}`, entityId, pointIds, 3),
      ],
    }
  },
})
...
export const controlPointSplineSketchToolDefinition = createFixedPointTool({
  ...
  group: 'drawing',
  name: 'Control Spline',
  tooltip: 'Create a spline from control points without changing fit-point spline behavior.',
  ...
  validate(points) {
    return points.length >= 4 && distinctPoints(points.slice(0, 4))
      ? { valid: true, message: null }
      : { valid: false, message: 'Control-point spline requires four distinct points.' }
  },
  buildPreview(points) {
    return points.length < 2
      ? []
      : [
          ...
          { id: 'preview-control-spline', kind: 'spline', points, entityId: null, status: 'preview', label: 'Control-point spline preview', isConstruction: false },
        ]
  },
  createCommitContribution({ sequence, points, factories }) {
    const pointIds = points.slice(0, 4).map((_, index) => factories.createPointId(`control-spline-${index + 1}`))
    const entityId = factories.createEntityId('control-spline')
    return {
      points: points.slice(0, 4).map((point, index) =>
        factories.createPoint(`Control spline ${sequence} point ${index + 1}`, pointIds[index]!, point),
      ),
      entities: [
        factories.createSplineEntity(`Control spline ${sequence}`, entityId, pointIds, 3),
      ],
    }
  },
})
...
export const profileTextSketchToolDefinition = createFixedPointTool({
    ...
    return [
      {
        ...
        placeholder: 'Text',
        action: { type: 'patch', patch: { intent: 'setToolSetting', key: 'text' } },
      },
      {
        ...
        action: { type: 'patch', patch: { intent: 'setToolSetting', key: 'horizontalAlign' } },
      },
      {
        ...
        action: { type: 'patch', patch: { intent: 'setToolSetting', key: 'verticalAlign' } },
      },
    ]
  },
  validate(points, state) {
    ...
    return points.length >= 2 && distanceBetween(points[0]!, points[1]!) > EPSILON
      ? { valid: true, message: null }
      : { valid: false, message: 'Text height must be greater than zero.' }
  },
  buildPreview(points, state) {
    return points.length >= 2
      ? pointPolylinePreview('preview-profile-text', 'Text preview', sampleProfileTextOutline(state, points[0]!, points[1]!), true)
      : []
  },
  createCommitContribution({ sequence, points, state, factories }) {
    ...
    return {
      points: [
        factories.createPoint(`Text ${sequence} anchor`, anchorPointId, anchor),
      ],
      entities: [
