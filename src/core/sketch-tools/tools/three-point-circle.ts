...
function buildThreePointCirclePreview(
...
function validateThreePointCircle(
  points: readonly SketchPoint[],
): SketchToolValidationResult {
  if (points.length < REQUIRED_POINTS) {
    return {
      valid: false,
      message: `3-point circle requires ${REQUIRED_POINTS} points.`,
    };
  }

  if (
    !pointsDistinct(points[0]!, points[1]!) ||
    !pointsDistinct(points[1]!, points[2]!) ||
...
function buildThreePointCirclePresentation(
  state: SketchToolRuntimeState,
  ...
  const validation = state.validationMessage
    ? [
        {
          id: "three-point-circle-validation",
  ...
  return {
    ...
    overlays: [
      ...
      ...(points.at(-1)
        ? [
            {
              id: "three-point-circle-cue",
              kind: "completionCue" as const,
              label: ready ? "Accept circle" : "Add point",
              point: points.at(-1)!,
              ready,
            },
          ]
...
export const threePointCircleSketchToolDefinition: SketchToolDefinition<"threePointCircle"> =
  {
    ...
    activate() {
      ...
      return {
        state,
        stagedEntities: [],
        presentation: buildThreePointCirclePresentation(state),
      };
    },
    pointerMove({ state, point }) {
      ...
      return {
        state: nextState,
        stagedEntities: buildThreePointCirclePreview(previewPoints),
        presentation: buildThreePointCirclePresentation(nextState),
      };
    },
    pointerRelease({ state, point }) {
      if (!point) {
        return {
          state,
          stagedEntities: buildThreePointCirclePreview(getPlacedPoints(state)),
          presentation: buildThreePointCirclePresentation(state),
        };
      }
      ...
      return {
        ...
        stagedEntities: complete
          ? []
          : buildThreePointCirclePreview(nextPoints),
        presentation: buildThreePointCirclePresentation(nextState),
      };
    },
    ...
    createCommitContribution({
      ...
      const circle =
        circlePoints.length === REQUIRED_POINTS
          ? getCircumcircleFromPoints(
              circlePoints[0]!,
              circlePoints[1]!,
              circlePoints[2]!,
      ...
      const centerPointId = factories.createPointId(
        "three-point-circle-center",
      );
      const perimeterPointIds = circlePoints.map((_, index) =>
        factories.createPointId(`three-point-circle-${index + 1}`),
      );
      ...
      return {
        points: [
          factories.createPoint(
            `3-point circle ${sequence} center`,
            centerPointId,
            circle.center,
          ),
          ...circlePoints.map((point, index) =>
            factories.createPoint(
              `3-point circle ${sequence} point ${index + 1}`,
              perimeterPointIds[index]!,
              point,
            ),
          ),
        ],
        ...
        constraints: perimeterPointIds.map((pointId, index) =>
          createPointOnCurveConstraint({
            constraintId: factories.createConstraintId(
              `three-point-circle-point-${index + 1}`,
            ),
        ...
        dimensions: [
          createCircleRadiusDimension({
            dimensionId: factories.createDimensionId(
              "three-point-circle-radius",
            ),
