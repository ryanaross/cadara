...
export function createRegularPolygonSketchToolDefinition(input: {
  ...
  const buildPreview = (center: SketchPoint, edgePoint: SketchPoint): readonly SketchDraftEntity[] => {
    const radius = getVertexRadius(input.mode, distanceBetween(center, edgePoint), DEFAULT_SIDE_COUNT)
    const vertices = createRegularPolygonVertices({ center, radius, sideCount: DEFAULT_SIDE_COUNT })
  ...
  const validate = (center: SketchPoint, edgePoint: SketchPoint) =>
    validateDistance(center, edgePoint, `${input.name} requires a center and radius.`)

  const buildPresentation = (state: SketchToolRuntimeState): SketchToolPresentationSchema => {
    ...
    const validation = state.validationMessage
      ? [{ id: `${input.id}-validation`, message: state.validationMessage, severity: 'error' as const }]
      : []

    return {
      prompts: [{ id: `${input.id}-prompt`, text: isDrawing ? 'Set polygon radius' : 'Pick polygon center', tone: validation.length > 0 ? 'warning' : 'neutral' }],
      steps: [{ id: `${input.id}-step`, label: isDrawing ? 'Radius' : 'Center' }],
      measurements: isDrawing ? [{ id: `${input.id}-radius`, label: 'Radius', value: radius, unit: 'mm' }] : [],
      completionHints: [{ id: `${input.id}-completion`, text: isDrawing ? 'Click to accept the polygon' : 'Click to set the center', ready: isDrawing ? validate(center, edgePoint).valid : false }],
      overlays: isDrawing
        ? [
            { id: `${input.id}-center`, kind: 'helperMarker', label: 'Center', point: center },
            { id: `${input.id}-cue`, kind: 'completionCue', label: 'Place radius', point: edgePoint, ready: validate(center, edgePoint).valid },
          ]
        : [],
      validation,
    }
    ...
    },
  {
    ...
    createCommitContribution({ sequence, start, end, factories }): SketchToolCommitContribution {
      ...
      const vertexRadius = getVertexRadius(input.mode, circleRadius, DEFAULT_SIDE_COUNT)
      const vertices = createRegularPolygonVertices({ center: start, radius: vertexRadius, sideCount: DEFAULT_SIDE_COUNT })
      const centerPointId = factories.createPointId(`${input.id}-center`)
      const vertexPointIds = vertices.map((_, index) => factories.createPointId(`${input.id}-vertex-${index + 1}`))
      const entityIds = vertices.map((_, index) => factories.createEntityId(`${input.id}-side-${index + 1}`))
      const circleEntityId = factories.createEntityId(`${input.id}-construction-circle`)

      return {
        points: [
          factories.createPoint(`${input.name} ${sequence} center`, centerPointId, start),
          ...vertices.map((point, index) => factories.createPoint(`${input.name} ${sequence} vertex ${index + 1}`, vertexPointIds[index]!, point)),
        ],
        entities: [
          { ...factories.createCircleEntity(`${input.name} ${sequence} construction circle`, circleEntityId, centerPointId, circleRadius), isConstruction: true },
        ...
        constraints: [
          ...entityIds.slice(1).map((entityId, index) =>
            createEqualLengthConstraint({
              constraintId: factories.createConstraintId(`${input.id}-equal-${index + 1}`),
          ...
          ...(input.mode === 'inscribed'
            ? vertexPointIds.map((pointId, index) =>
                createPointOnCurveConstraint({
                  constraintId: factories.createConstraintId(`${input.id}-vertex-on-circle-${index + 1}`),
            ...
            : entityIds.map((entityId, index) =>
                createTangentConstraint({
                  constraintId: factories.createConstraintId(`${input.id}-side-tangent-${index + 1}`),
