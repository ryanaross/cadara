...
export function subtractPoints(left: SketchPoint, right: SketchPoint): SketchPoint {
  return [left[0] - right[0], left[1] - right[1]]
}
...
export function mirrorPointAcrossCenter(center: SketchPoint, point: SketchPoint): SketchPoint {
  return [center[0] * 2 - point[0], center[1] * 2 - point[1]]
}

export function getAxisAlignedRectangleCorners(center: SketchPoint, corner: SketchPoint) {
  const opposite = mirrorPointAcrossCenter(center, corner)
  const bottomLeft: SketchPoint = [Math.min(corner[0], opposite[0]), Math.min(corner[1], opposite[1])]
  const topRight: SketchPoint = [Math.max(corner[0], opposite[0]), Math.max(corner[1], opposite[1])]
...
export function getAlignedRectangleCorners(first: SketchPoint, second: SketchPoint, third: SketchPoint) {
...
export function getTangentArcCircle(
...
): (CircleFromPoints & { sweepDirection: 'clockwise' | 'counterClockwise' }) | null {
  ...
  const center = addPoints(start, scalePoint(normal, dotPoints(chord, chord) / denominator))
  ...
  const sweepDirection = dotPoints(ccwTangent, tangent) >= dotPoints(clockwiseTangent, tangent)
    ? 'counterClockwise'
    : 'clockwise'
