...
export function validateRectangle(
  ...
  const valid =
    Math.abs(end[0] - start[0]) > epsilon &&
    Math.abs(end[1] - start[1]) > epsilon;
...
export function createSketchToolDefinition<
  TToolId extends SketchToolMetadata["id"],
>(
