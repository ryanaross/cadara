...
export function getEditorEffectContext(
  effect: EditorEffect,
  ...
  if (
    effect.type === "feature.evaluatePreview" ||
    effect.type === "feature.commit"
  ) {
    context.push({ key: "previewId", value: effect.featureSession.previewId });
    if (effect.featureSession.featureId) {
      context.push({
        key: "featureId",
        value: effect.featureSession.featureId,
      });
    }
  }
...
export function getAppErrorRevisionId(
  appError: AppError,
  key: string,
): RevisionId | undefined {
  const value = getAppErrorContextValue(appError, key);

  return typeof value === "string" && value.startsWith("rev_")
    ? (value as RevisionId)
    : undefined;
}
...
export function isModelingMutationError(appError: AppError) {
  return (
    appError.code === "modeling/diagnostic" ||
    appError.code === "modeling/revision-rejected"
  );
}

export function modelingMutationErrorToDiagnostic(
  appError: AppError,
...
export function getDurableDiagnosticTarget(
  target: PrimitiveRef | null,
): DurableRef | null {
  if (
    !target ||
