...
function getDeleteSolidValidationDiagnostics(
  draft: DeleteSolidFeatureParameterDraft,
) {
  return validateAdvancedSolidFeatureDefinition(
    buildDeleteSolidDefinition(draft),
    {
...
export const deleteSolidAuthoringDefinition = {
  ...
  buildDefinition(draft) {
    return getDeleteSolidValidationDiagnostics(draft).length === 0
      ? buildDeleteSolidDefinition(draft)
      : null;
  },
          ...
          fields: [
            {
              ...
              helper:
                "Select each durable body that should be removed when this feature commits.",
              error:
