...
import type { FeatureAuthoringDefinition, SweepFeatureParameterDraft } from '@/core/feature-authoring/definition'
import { createSelectionFilterForRequirement, sweepSelectionFilter, type PrimitiveRef } from '@/core/editor/schema'
import { acceptAuthoredPatch, appendUniqueTarget, asBodyRef, asExtrudeProfileRef, asFaceRef, asSweepPathRef, authoredDefinitionValue, authoredNumberFormValue, authoredNumberLiteral, authoredStringLiteral, buildAdvancedSolidParticipants, createAdvancedOperationIntentFields, createMissingInputDiagnostic, createReferenceCollectionField, expressionCapableAuthoredValue, filterTargets, isAdvancedOperationIntent, isPositiveAuthoredNumber, toFeaturePhaseDiagnostics, validateAdvancedSolidDraft } from '@/core/feature-authoring/features/shared'
...
function getTargetsForRole(draft: SweepFeatureParameterDraft, role: AdvancedParticipantRole) {
  switch (role) {
    ...
    case 'lockProfileDirection':
      return draft.lockProfileDirectionTarget ? [draft.lockProfileDirectionTarget] : []
    case 'targetBody':
...
function asLockProfileDirectionRef(value: PrimitiveRef | null): Extract<PrimitiveRef, { kind: 'edge' | 'construction' }> | null {
  return value?.kind === 'edge' || value?.kind === 'construction' ? value : null
}
...
function isSweepTwistType(value: unknown): value is SweepFeatureParameterDraft['options']['twist']['type'] {
  return value === 'none' || value === 'turns' || value === 'angle' || value === 'pitch'
}
...
function coerceSweepProfileControlOption(
  ...
  return isSweepProfileControl(literal) ? value as SweepFeatureParameterDraft['options']['profileControl'] : fallback
}
...
function sanitizeSweepTwistOption(
  ...
  switch (twist.type) {
    case 'turns':
      return { type: 'turns', turns: authoredDefinitionValue(twist.turns ?? 1, 1) }
    case 'angle':
      return { type: 'angle', angle: authoredDefinitionValue(twist.angle ?? Math.PI / 2, Math.PI / 2) }
    case 'pitch':
      return { type: 'pitch', pitch: authoredDefinitionValue(twist.pitch ?? 1, 1) }
    default:
      return { type: 'none' }
  }
}

function buildSweepOptions(draft: SweepFeatureParameterDraft): SweepAdvancedOptions {
  return {
    profileControl: authoredDefinitionValue(draft.options.profileControl, 'none'),
...
function getOptionPatchObject(patch: Record<string, unknown>) {
  return patch.options && typeof patch.options === 'object' && !Array.isArray(patch.options)
...
function mergeSweepOptions(
  ...
  const twistPatch = optionPatch.twist && typeof optionPatch.twist === 'object' && !Array.isArray(optionPatch.twist)
    ? optionPatch.twist as Record<string, unknown>
    : null
  const nextTwistType = isSweepTwistType(twistPatch?.type) ? twistPatch.type : current.twist.type
  ...
  return {
    ...
    twist: {
      ...current.twist,
      type: nextTwistType,
      turns: acceptAuthoredPatch(twistPatch?.turns, current.twist.turns ?? 1, (value): value is number => typeof value === 'number'),
      angle: acceptAuthoredPatch(twistPatch?.angle, current.twist.angle ?? Math.PI / 2, (value): value is number => typeof value === 'number'),
      pitch: acceptAuthoredPatch(twistPatch?.pitch, current.twist.pitch ?? 1, (value): value is number => typeof value === 'number'),
    },
...
function getSweepValidationDiagnostics(draft: SweepFeatureParameterDraft) {
  ...
  const profileControl = authoredStringLiteral(draft.options.profileControl, 'none')
  if (profileControl === 'lockProfileFaces' && draft.lockProfileFaceTargets.length === 0) {
    diagnostics.push(createSweepValidationDiagnostic({
      role: 'lockProfileFace',
      message: 'Lock profile faces requires at least one face reference.',
    }))
  }

  if (profileControl === 'lockProfileDirection' && !draft.lockProfileDirectionTarget) {
    diagnostics.push(createSweepValidationDiagnostic({
      role: 'lockProfileDirection',
      message: 'Lock profile direction requires exactly one edge or construction reference.',
    }))
  }
...
export const sweepAuthoringDefinition = {
  ...
  hydrateDraft(feature) {
    ...
    const lockProfileDirectionTargets = getParticipantTargets('lockProfileDirection')
    const featureOptions = feature.parameters.options ?? {}
    const twist = featureOptions.twist && typeof featureOptions.twist === 'object' && !Array.isArray(featureOptions.twist)
    ...
    return {
      ...
      lockProfileFaceTargets: filterTargets(getParticipantTargets('lockProfileFace'), asFaceRef),
      lockProfileDirectionTarget: asLockProfileDirectionRef(lockProfileDirectionTargets[0] ?? null),
      ...
      options: {
        profileControl: coerceSweepProfileControlOption(featureOptions.profileControl, defaultOptions.profileControl),
        twist: {
          type: twistType,
          turns: (twist.turns ?? defaultOptions.twist.turns) as MaybeAuthoredValue<number>,
          angle: (twist.angle ?? defaultOptions.twist.angle) as MaybeAuthoredValue<number>,
          pitch: (twist.pitch ?? defaultOptions.twist.pitch) as MaybeAuthoredValue<number>,
        },
        endScale: (featureOptions.endScale ?? defaultOptions.endScale) as MaybeAuthoredValue<number>,
      },
    }
  },
    ...
    return {
      ...
      lockProfileDirectionTarget:
        patch.lockProfileDirectionTarget === undefined
          ...
          : asLockProfileDirectionRef(patch.lockProfileDirectionTarget as PrimitiveRef | null),
  ...
  getFormSchema(session) {
    ...
    const profileControl = authoredStringLiteral(session.draft.options.profileControl, 'none')
    ...
    const endScaleIsValid = isExpressionAuthoredValue(session.draft.options.endScale)
          ...
          fields: [
            ...
            }),
            {
              ...createReferenceCollectionField({
                ...
                error:
                  profileControl !== 'lockProfileFaces' || session.draft.lockProfileFaceTargets.length > 0
                    ? null
            ...
            },
            {
              ...
              emptyLabel: 'None selected',
              helper: 'Accepted targets: one durable edge or construction reference.',
              hidden: profileControl !== 'lockProfileDirection',
              error:
                profileControl !== 'lockProfileDirection' || session.draft.lockProfileDirectionTarget
                  ? null
                  : { message: 'Select one lock direction reference.' },
              advancedParticipant: {
                role: 'lockProfileDirection',
                required: profileControl === 'lockProfileDirection',
                cardinality: { min: profileControl === 'lockProfileDirection' ? 1 : 0, max: 1 },
              ...
              fields: [
                {
                  ...
                  options: [
                    ...
                    { value: 'keepProfileOrientation', label: 'keepProfileOrientation' },
                    { value: 'lockProfileFaces', label: 'lockProfileFaces' },
                    { value: 'lockProfileDirection', label: 'lockProfileDirection' },
                  ],
                ...
                },
                {
                  ...
                  discriminant: {
                    ...
                    patch: { patchKey: 'options', valuePath: ['twist', 'type'] },
                  },
                  variants: [
                      ...
                      fields: [
                        {
                          ...
                          input: 'number',
                          step: 0.25,
                          error: isPositiveAuthoredNumber(twistTurns) ? null : { message: 'Turns must be greater than zero.' },
                          authoredValue: expressionCapableAuthoredValue(twistTurns, { kind: 'positiveNumber' }),
                          patch: { patchKey: 'options', valuePath: ['twist', 'turns'] },
                        },
                      ],
                    },
                    {
                      ...
                      fields: [
                        {
                          ...
                          label: 'Angle',
                          value: authoredNumberFormValue(twistAngle, (value) => value * (180 / Math.PI)),
                          input: 'angleDegrees',
                          step: 5,
                          authoredValue: expressionCapableAuthoredValue(twistAngle, { kind: 'angle' }),
                          patch: { patchKey: 'options', valuePath: ['twist', 'angle'] },
                        },
                      ],
                    },
                    {
                      ...
                      fields: [
                        {
                          ...
                          input: 'number',
                          step: 0.1,
                          error: isPositiveAuthoredNumber(twistPitch) ? null : { message: 'Pitch must be greater than zero.' },
                          authoredValue: expressionCapableAuthoredValue(twistPitch, { kind: 'positiveNumber' }),
                          patch: { patchKey: 'options', valuePath: ['twist', 'pitch'] },
                        },
                      ],
                    },
                  ],
                },
                {
                  ...
                  id: 'sweep-end-scale',
                  label: 'End scale',
                  value: authoredNumberFormValue(session.draft.options.endScale),
                  input: 'number',
                  step: 0.1,
                  error: endScaleIsValid ? null : { message: 'End scale must be greater than zero.' },
                  authoredValue: expressionCapableAuthoredValue(session.draft.options.endScale, { kind: 'positiveNumber' }),
