    ...
    parameters: {
      participants: [
        ...(draft.bodyTargets.length > 0
          ...
          : []),
        ...(draft.planeTarget
          ? [{ role: "plane" as const, targets: [draft.planeTarget] }]
          : []),
      ],
      options: {
...
export const mirrorAuthoringDefinition = {
  ...
  hydrateDraft(feature) {
    const planeTarget = asPlaneReferenceTarget(
    ...
    return {
      ...
      planeTarget,
      copy: (feature.parameters.options?.copy ??
        true) as MirrorFeatureParameterDraft["copy"],
    };
  },
  ...
  getMissingInputsDiagnostics(input) {
    ...
    return [
      createMissingInputDiagnostic({
        ...
        suffix: "references",
        message:
          "Mirror preview requires body targets, one explicit mirror plane, and copy mode.",
      }),
    ];
  },
  buildDefinition(draft) {
    return getMirrorValidationDiagnostics(draft).length === 0
      ? buildMirrorDefinition(draft)
      : null;
  },
          ...
          fields: [
            {
              ...
              helper:
                "Select each explicit durable body that should be mirrored.",
              error:
            ...
            },
            {
              ...
              error: session.draft.planeTarget
                ? null
                : { message: "Select one mirror plane." },
              advancedParticipant: {
                role: "plane",
                required: true,
                cardinality: { min: 1, max: 1 },
          ...
          fields: [
            {
              ...
              id: "mirror-copy-mode",
              label: "Result policy",
              value: authoredBooleanLiteral(session.draft.copy, true)
                ? "copy"
                : "replace",
              options: [
              ...
              helper:
                "Replace is preserved in the draft contract shape but remains unsupported by the first adapter slice.",
