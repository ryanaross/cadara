...
>;
type FeatureEditorReferenceField = Extract<
  FeatureEditorFormField,
...
export function createFeatureEditorFieldPatch(
  ...
  const nextValue =
    field.kind === "numeric" &&
    field.input === "angleDegrees" &&
    isLiteralAuthoredValue(value)
      ? createLiteralAuthoredValue(
  ...
  if (field.patch.valuePath) {
    return createNestedPatch(
      field.patch.patchKey,
      field.patch.valuePath,
      nextValue,
    );
  }
...
export function createFeatureEditorClearReferencePatch(
  field: FeatureEditorReferenceField,
): FeatureEditorPatch {
  return createFeatureEditorFieldPatch(
    field,
    field.picker.allowsMultiple ? [] : null,
  );
}
...
export function createFeatureEditorRemoveReferenceItemPatch(
  ...
  const next = getReferenceArray(field).filter(
