...
export function pruneUnknownShortcutOverrides(
  ...
  for (const [commandId, override] of Object.entries(overrides) as Array<
    [ShortcutCommandId, { shortcuts: readonly string[] }]
  >) {
  ...
  try {
    ...
    for (const [commandId, override] of Object.entries(value)) {
      ...
      if (
        ...
        !shortcuts.every((shortcut) => typeof shortcut === "string")
      ) {
        continue;
      }
