...
test('src/infrastructure/workers/document-sync-worker-dispatcher.spec.ts', async () => {
  async function testRequestsWaitForBootstrapConfiguration() {
    ...
    dispatcher({
      kind: 'bootstrap',
      search: '?cadLocalPeerSync=1&cadLocalPeerSyncChannel=peer-a&cadRepositoryDbName=repo-a',
    })

    expectTrue(
      observedSearches.join(',') === '?cadLocalPeerSync=1&cadLocalPeerSyncChannel=peer-a&cadRepositoryDbName=repo-a',
      'The worker dispatcher should initialize the worker message handler from the bootstrap search string.',
    )
    expectTrue(
      handledRequests.join(',') === '?cadLocalPeerSync=1&cadLocalPeerSyncChannel=peer-a&cadRepositoryDbName=repo-a:subscribe:doc_workspace',
  ...
  function testConfiguredWorkerHandlesLaterRequestsImmediately() {
    ...
    dispatcher({
      ...
      documentId: 'doc_workspace',
      seedDocument: {} as Extract<DocumentSyncWorkerRequest, { kind: 'load' }>['seedDocument'],
    })

    expectTrue(
      handledRequests.join(',') === '?cadRepositoryDbName=repo-b:load:doc_workspace',
