...
const LOCAL_HISTORY_SCOPE_STORAGE_KEY = 'cad.documentRepository.localHistoryScope.v1'
...
function getOrCreateLocalHistoryScope(sessionStorage: Pick<Storage, 'getItem' | 'setItem'>) {
  const existing = sessionStorage.getItem(LOCAL_HISTORY_SCOPE_STORAGE_KEY)
  if (existing) {
    return existing
  }

  const scope = typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
    ? crypto.randomUUID()
...
function getDefaultSessionStorage() {
  return typeof globalThis.sessionStorage?.getItem === 'function' && typeof globalThis.sessionStorage?.setItem === 'function'
