...
export function createDocumentSyncWorkerMessageHandler(
  ...
  const pendingWrites = new Map<
    string,
    {
      ...
      metadata: LocalFileBindingRecord["metadata"];
    }
  >();
  ...
  async function restoreBoundFileRecord(
    documentId: DocumentId,
    restoreOptions?: { throwOnFailure?: boolean },
  ) {
    const restored = await options.bindingStore?.load(documentId);
    if (restored && !restored.ok) {
      if (restoreOptions?.throwOnFailure) {
        throw new Error(
          restored.reason === "unsupported-storage"
            ? "Persistent local file binding storage is unavailable."
            : "Local file binding could not be restored.",
        );
      }
      return null;
    }
  ...
  async function readBoundFileDocument(
    documentId: DocumentId,
    record: LocalFileBindingRecord,
  ) {
    let payload: unknown;
    try {
      ...
      return createFailedLoadResult(documentId, {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Linked local file could not be read.",
      });
    }
  ...
  return async function handleDocumentSyncWorkerMessage(
    ...
    try {
      switch (request.kind) {
        case "load": {
          if (request.storageKey) {
            options.repositoryUrlStore?.set(
              request.documentId,
              request.storageKey as Parameters<
          ...
          const boundFileRecord = await restoreBoundFileRecord(
            request.documentId,
          );
          if (!boundFileRecord) {
            postMessage({
          ...
          const boundFileDocument = await readBoundFileDocument(
            request.documentId,
            boundFileRecord,
          );
        ...
        case "restoreBinding": {
          const record = await restoreBoundFileRecord(request.documentId, {
            throwOnFailure: true,
          });
          postMessage({
        ...
        case "bindFileHandle": {
          ...
          const persistentBindingUnavailable =
            saved && !saved.ok && saved.reason === "unsupported-storage";
          ...
          if (persistentBindingUnavailable) {
            publishWriteStatus({
              ...
              metadata: request.metadata,
              message:
                "Local file sync is active, but this browser cannot remember the file after refresh.",
            });
          }
          publishWriteStatus({
            kind: "synced",
        ...
        case "getDurableHistoryAvailability": {
          postMessage({
            ...
            requestId: request.requestId,
            availability:
              await options.repository.getDurableHistoryAvailability(
                request.documentId,
              ),
          });
          return;
        ...
        case "getSketchDraftHistory": {
          const result = await options.repository.getSketchDraftHistory(
            request.documentId,
            request.draftKey,
          );
          postMessage({
        ...
        case "undoSketchDraftHistory": {
          const result = await options.repository.undoSketchDraftHistory(
            request.documentId,
            request.draftKey,
          );
          postMessage({
        ...
        case "redoSketchDraftHistory": {
          const result = await options.repository.redoSketchDraftHistory(
            request.documentId,
            request.draftKey,
          );
          postMessage({
        ...
        case "clearSketchDraftHistory": {
          await options.repository.clearSketchDraftHistory(
            request.documentId,
            request.draftKey,
          );
          postMessage({
