...
export const VIEW_CUBE_CORNER_TARGETS: ViewCubeCornerTarget[] = VIEW_CUBE_CORNER_PRESET_IDS.map((presetId) => {
  const [x, y, z] = VIEW_NAVIGATION_PRESETS[presetId].direction

  return {
    presetId,
    position: [x * CORNER_TARGET_DISTANCE, y * CORNER_TARGET_DISTANCE, z * CORNER_TARGET_DISTANCE],
  }
})
