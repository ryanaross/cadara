...
export const DEFAULT_VIEWPORT_PROJECTION_MODE: ViewportProjectionMode =
  "orthographic";
...
export function updateViewportCameraAspect(
...
export function getViewportCameraProjectionMode(
  camera: ViewportCamera,
...
export function getDefaultViewportCameraFrame(): ViewportCameraFrame {
  const cameraDistance = DEFAULT_CAMERA_POSITION.distanceTo(
...
export function updateViewportCameraClipping(
  ...
  camera.far = Math.max(
    MIN_CAMERA_FAR,
    targetDistance * PERSPECTIVE_CLIP_PADDING_FACTOR,
  );
  camera.updateProjectionMatrix();
}

export function cloneViewportCameraFrame(
  frame: ViewportCameraFrame,
): ViewportCameraFrame {
  return {
...
export function interpolateViewportCameraFrame(
  ...
  return {
    ...
    cameraDistance: THREE.MathUtils.lerp(
      fromFrame.cameraDistance,
      toFrame.cameraDistance,
      clampedAlpha,
    ),
    perspectiveDistance: THREE.MathUtils.lerp(
      fromFrame.perspectiveDistance,
      toFrame.perspectiveDistance,
      clampedAlpha,
    ),
    orthographicZoom: THREE.MathUtils.lerp(
      fromFrame.orthographicZoom,
      toFrame.orthographicZoom,
      clampedAlpha,
    ),
  };
}
...
export function computeViewportRenderableFitFrame(input: {
  ...
  const rightAxis = new THREE.Vector3(1, 0, 0)
    .applyQuaternion(input.camera.quaternion)
    .normalize();
  ...
  const halfWidth =
    Math.max((maxX - minX) / 2, MIN_FIT_HALF_EXTENT) * FIT_PADDING_FACTOR;
  const halfHeight =
    Math.max((maxY - minY) / 2, MIN_FIT_HALF_EXTENT) * FIT_PADDING_FACTOR;

  if (input.camera instanceof THREE.OrthographicCamera) {
    const halfFrustumWidth = Math.max(
      (input.camera.right - input.camera.left) / 2,
      MIN_FIT_HALF_EXTENT,
    );
    const halfFrustumHeight = Math.max(
      (input.camera.top - input.camera.bottom) / 2,
      MIN_FIT_HALF_EXTENT,
    );
    ...
    const distance = Math.max(
      input.camera.position.distanceTo(input.controls.target),
      radius * 2,
      MIN_FIT_CAMERA_DISTANCE,
    );
...
function computeOrthographicZoomForPerspectiveDistance(input: {
  ...
  const visibleHeight =
    2 *
    Math.max(input.perspectiveDistance, MIN_CAMERA_DISTANCE) *
    Math.tan(THREE.MathUtils.degToRad(input.fovDegrees) / 2);

  return Math.max(
    ORTHOGRAPHIC_FRUSTUM_HEIGHT / Math.max(visibleHeight, MIN_CAMERA_DISTANCE),
    MIN_ORTHOGRAPHIC_ZOOM,
  );
}
...
function computePerspectiveDistanceForOrthographicZoom(input: {
  ...
  return Math.max(
    visibleHeight /
      (2 * Math.tan(THREE.MathUtils.degToRad(input.fovDegrees) / 2)),
    MIN_CAMERA_DISTANCE,
  );
}

function collectBodyRenderablePoints(
  renderables: readonly RenderableEntityRecord[],
) {
