...
export function computeSketchCameraFrame({
  ...
  const halfWidth =
    Math.max((maxX - minX) / 2, MIN_HALF_EXTENT) * FRAME_PADDING_FACTOR;
  const halfHeight =
    Math.max((maxY - minY) / 2, MIN_HALF_EXTENT) * FRAME_PADDING_FACTOR;
...
function collectRenderableWorldPoints(
  renderables: SketchSessionDisplayRenderable[],
): THREE.Vector3[] {
  ...
  for (const renderable of renderables) {
    if (renderable.geometry.kind === "mesh") {
      points.push(
        ...renderable.geometry.vertexPositions.map((point) => toVector3(point)),
      );
      continue;
    }
    ...
    if (renderable.geometry.kind === "polyline") {
      points.push(
        ...renderable.geometry.points.map((point) => toVector3(point)),
      );
    }
  }
