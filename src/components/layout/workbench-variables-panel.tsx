...
function getVariableResultPresentations(
  ...
  for (const variable of variables) {
    ...
    if (evaluationError) {
      presentations.set(variable.variableId, { kind: 'error', text: '???', message: evaluationError })
      continue
    }
...
export function WorkbenchVariablesPanel({
  ...
  const variables = useMemo(() => snapshot?.document.variables ?? [], [snapshot])
  const variableResultPresentations = useMemo(
    () => getVariableResultPresentations(variables, invalidVariableValueIds, invalidVariableValueMessages),
  ...
  const getVariableDraft = (variable: DocumentVariableRecord) =>
    variableDrafts[variable.variableId] ?? { name: variable.name, valueText: variable.valueText }
  ...
  return (
    <aside aria-label="Variables" data-workbench-variables-panel style={PANEL_STYLE}>
      <header
        className="flex items-center justify-between px-4 pt-3.5 pb-2.5"
        style={{ borderBottom: '1px solid var(--workbench-glass-divider)' }}
      >
        <span className="text-[12.5px] font-semibold" style={{ letterSpacing: '-0.005em', color: 'var(--workbench-shell-text)' }}>
          Variables
        </span>
        <ActionIcon
          variant="subtle"
      ...
      <div className="flex-1 overflow-y-auto py-1.5">
        {variables.map((variable) => {
          const draft = getVariableDraft(variable)
          const accessibleVariableName = variable.name.trim() || 'Unnamed variable'
          const variableResult = variableResultPresentations.get(variable.variableId) ?? {
            kind: 'error' as const,
            text: '???' as const,
            message: 'Variable expression result is unavailable.',
              ...
              {isEditingVariable ? (
                ...
                >
                  ...
                  <span className="ml-auto flex min-w-0 max-w-[68%] shrink-0 items-center justify-end gap-1.5">
                    ...
                    <span aria-hidden="true" className="shrink-0 text-[12px] leading-4 text-[var(--workbench-shell-text-dim)]">
                      =
                    </span>
                    {variableResult.kind === 'error' ? (
                      <Tooltip
