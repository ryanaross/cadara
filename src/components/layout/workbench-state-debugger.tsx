...
function DebuggerRow(props: { label: string; value: string | number }) {
  return (
    <div>
      {props.label}:{" "}
      <span className="text-[var(--cad-foreground)]">{props.value}</span>
    </div>
  );
}
...
export function WorkbenchStateDebugger({
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-semibold text-[var(--cad-muted-foreground)]">
          State Debugger
        </span>
        <span
          className="text-[var(--cad-muted)]"
          data-workbench-state-debugger-summary
        >
        ...
        <button
          ...
          aria-label={
            isExpanded ? "Collapse state debugger" : "Expand state debugger"
          }
        ...
        >
          ...
          <div className="border-t border-[var(--cad-border)] pt-2">
            <DebuggerRow
              ...
              value={state.selectionFilterLabel}
            />
          ...
          <div className="border-t border-[var(--cad-border)] pt-2">
            ...
            <DebuggerRow
              ...
              value={state.selectionDetail.label}
            />
            <DebuggerRow label="Kind" value={state.selectionDetail.kindLabel} />
            <DebuggerRow
              label="Owner"
              value={state.selectionDetail.ownerLabel}
            />
            <DebuggerRow
              label="Related"
              value={
                state.selectionDetail.relatedLabels.length > 0
                  ? state.selectionDetail.relatedLabels.join(", ")
                  : "none"
              }
            />
            <DebuggerRow
              label="Target"
              value={state.selectionDetail.targetLabel}
            />
            <DebuggerRow
              label="Section offset"
              value={state.sectionOffset ?? "none"}
            />
            <DebuggerRow
              label="Section side"
              value={state.sectionRetainedSide ?? "none"}
            />
          </div>
          ...
          <details className="border-t border-[var(--cad-border)] pt-2">
            ...
            <div className="mt-2 grid gap-2">
              <div className="grid gap-1">
                <DebuggerRow
                  label="Bodies"
                  value={state.topologyDebug.bodyCount}
                />
                <DebuggerRow
                  label="Live topology refs"
                  value={state.topologyDebug.liveTopologyReferences}
                />
                <DebuggerRow
                  label="Invalid topology refs"
                  value={state.topologyDebug.invalidatedTopologyReferences}
                />
              </div>
                ...
                {state.topologyDebug.bodies.length > 0 ? (
                  state.topologyDebug.bodies.map((body) => (
                    <div key={body.bodyId}>
                      ...
                      : {body.faces}F/{body.edges}E/{body.vertices}V,{" "}
                      {body.liveReferences} live, {body.invalidatedReferences}{" "}
                      invalid
                    </div>
                  ))
                ) : (
                ...
                {state.topologyDebug.invalidations.length > 0 ? (
                  state.topologyDebug.invalidations.map((invalidation) => (
                    <div key={invalidation.reason}>
                      ...
                      : {invalidation.count}
                      {invalidation.examples.length > 0
                        ? ` (${invalidation.examples.join(", ")})`
                        : ""}
                    </div>
                  ))
                ) : (
