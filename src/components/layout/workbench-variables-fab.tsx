 ...
 * never overlaps it.
 */
export function WorkbenchVariablesFab({ open, onToggle }: WorkbenchVariablesFabProps) {
  return (
    <button
      type="button"
      ...
      onMouseEnter={(event) => {
        ...
        event.currentTarget.style.background = 'var(--workbench-spark-accent-hover)'
      }}
      onMouseLeave={(event) => {
      ...
      style={FAB_BASE}
    >
      <WorkbenchIcon name={open ? 'close' : 'variables'} className="h-[22px] w-[22px]" />
    </button>
  )
}
