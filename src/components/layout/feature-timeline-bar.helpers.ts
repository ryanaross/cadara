...
export function getDocumentHistoryMenuEntryDescriptors(input: {
  ...
  const rollToEndDisabled =
    cursorDisabled ||
    isDocumentHistoryCursorIndexAtTail(historyLength, cursorIndex);
...
export function getHistoryItemOrderEntry(
  item: DocumentHistoryItemRecord,
): DocumentHistoryOrderEntry {
  ...
  return (
    ...
    left.every(
      (entry, index) =>
        ...
        getDocumentHistoryOrderEntryKey(right[index]!),
    )
  );
}
...
export function resolveTimelineReorderDrop(
  ...
  const beforeItem = beforeItemRecord
    ? getHistoryItemOrderEntry(beforeItemRecord)
    : null;
  if (
    beforeItem &&
    getDocumentHistoryOrderEntryKey(beforeItem) ===
      getDocumentHistoryOrderEntryKey(item)
  ) {
    return null;
  }
