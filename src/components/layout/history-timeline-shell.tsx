...
export function HistoryTimelineShell({
  ...
  const [collapsed, setCollapsed] = useState(() => readHistoryTimelineCollapsedPreference())
  const panelMotion = getHistoryTimelinePanelMotionState(collapsed)
  const toggleLabel = collapsed ? 'Show history timeline' : 'Hide history timeline'
        ...
        <Tooltip label={toggleLabel} position="top" withArrow={false}>
          <ActionIcon
            ...
            style={{
              ...
              transition: 'transform 160ms cubic-bezier(0.25, 1, 0.5, 1), background-color 160ms cubic-bezier(0.25, 1, 0.5, 1), border-color 160ms cubic-bezier(0.25, 1, 0.5, 1), color 160ms cubic-bezier(0.25, 1, 0.5, 1)',
            }}
            onMouseEnter={(event) => {
              event.currentTarget.style.transform = 'translateY(-1px)'
              event.currentTarget.style.backgroundColor = 'var(--workbench-shell-control-surface-hover)'
            }}
            onMouseLeave={(event) => {
              event.currentTarget.style.transform = 'translateY(0)'
              event.currentTarget.style.backgroundColor = 'var(--workbench-glass-fill-strong)'
            }}
            variant="subtle"
          >
            <WorkbenchIcon
