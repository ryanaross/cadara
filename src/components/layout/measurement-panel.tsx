...
export function MeasurementPanel({ measurement }: MeasurementPanelProps) {
  ...
  return (
    ...
    >
      <Text size="11px" fw={600} tt="uppercase" c="var(--workbench-shell-text-dim)" className="tracking-[0.20em]">
        Measure
      </Text>
      <Text size="sm" fw={600} c="var(--cad-foreground)" className="mt-1">
        {measurement.title}
      </Text>
      <Text size="11px" c="var(--workbench-shell-text-muted)" className="mt-0.5">
      ...
      <div className="mt-3 grid gap-1.5">
        {measurement.rows.map((row) => (
          <div key={row.id} className="flex items-baseline justify-between gap-3">
            ...
            <span className="text-right font-mono text-[12px] text-[var(--cad-foreground)]">{row.value}</span>
          </div>
        ))}
      </div>

      {measurement.note ? (
        <Text size="11px" c="var(--workbench-shell-text-muted)" className={measurement.rows.length > 0 ? 'mt-3' : 'mt-2'}>
          {measurement.note}
        </Text>
      ) : null}
    </Paper>
