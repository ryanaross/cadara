...
function getTimelineReorderBoundaryXs(slotRects: readonly TimelineSlotRect[]) {
  ...
  for (let index = 0; index < slotRects.length - 1; index += 1) {
    boundaries.push(
      (slotRects[index]!.center + slotRects[index + 1]!.center) / 2,
    );
  }
...
function getTrackRelativeClientX(
  track: HTMLDivElement | null,
...
function HistoryTimelineSurface({
  ...
  const [repairTooltip, setRepairTooltip] = useState<RepairTooltipState | null>(
    null,
  );
  ...
  const resolvedFocusedItemIndex =
    items.length === 0
      ? -1
  ...
  const resolveNearestCursorIndex = useCallback(
    (pointerClientX: number) => {
  ...
  return (
    ...
    >
      {repairTooltip && typeof document !== "undefined"
        ? createPortal(
            <span
              id={tooltipId}
              role="tooltip"
              ...
              style={{
                ...
                backgroundColor: "var(--workbench-shell-overlay-strong)",
                boxShadow:
                  "var(--workbench-panel-shadow), inset 0 0 0 1px var(--workbench-shell-danger-border)",
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          aria-describedby={
                            item.errorMessage ? tooltipId : undefined
                          }
                          onFocus={(event) => {
                            setFocusedItemIndex(index);
                            showRepairTooltip(
                          ...
                          onKeyDown={(event) => {
                            const nextIndex = getNextHistoryTreeFocusIndex(
                              index,
                              event.key,
                              items.length,
                            );
                        ...
                        >
                          {item.icon}
                          <span
                            className={`max-w-[9ch] truncate text-[11px] ${item.isSuppressed ? "line-through decoration-[var(--workbench-shell-text-muted)]" : ""}`}
                          >
                            {item.label}
                          </span>
                        </button>
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    background:
                      "color-mix(in oklch, var(--workbench-shell-accent) 35%, transparent)",
                    boxShadow: "var(--workbench-shell-scrubber-track-glow)",
                  }}
                />
                <button
                  ...
                  className={`absolute top-1/2 z-10 h-[18px] w-[18px] rounded-full border-2 transition left-[max(9px,var(--timeline-cursor-left,0px))] ${
                    dragCursorIndex === null && !cursorDisabled
                      ? "hover:[--timeline-thumb-scale:1.08]"
                      : ""
                  }`}
                  ...
                  style={{
                    ...
                    transition:
                      "box-shadow 160ms ease, transform 120ms cubic-bezier(.2,.7,.3,1)",
                    backgroundColor: "var(--workbench-shell-overlay-strong)",
...
export function FeatureTimelineBar({
  ...
  const [itemDragState, setItemDragState] = useState<ItemDragState | null>(
    null,
  );
  const suppressNextItemClickRef = useRef(false);
  const documentCursorActionsDisabled = cursorDisabled || !onCursorRequested;
  const getPositionCursor = useCallback(
    (index: number): DocumentFeatureCursor => {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const distance = Math.hypot(
        event.clientX - itemDragState.startX,
        event.clientY - itemDragState.startY,
      );
      const active =
        itemDragState.active || distance >= ITEM_REORDER_DRAG_THRESHOLD_PX;
      if (!active) {
        return;
      }

      const nearestIndex = getNearestTimelineAnchorIndex(
        getTimelineReorderBoundaryXs(
          getTimelineSlotRects(itemRefs, trackRef.current),
        ),
      ...
      setItemDragState((current) =>
        current && current.pointerId === event.pointerId
          ? {
              ...
              active: true,
              dropCursorIndex:
                nearestIndex >= -1 ? nearestIndex : current.dropCursorIndex,
            }
          : current,
      );
    };
    ...
    const handlePointerUp = (event: PointerEvent) => {
      ...
      setItemDragState((current) => {
        ...
        if (current.active && !reorderDisabled) {
          const drop = resolveTimelineReorderDrop(
            historyItems,
            current.item,
            current.dropCursorIndex,
          );
          if (drop) {
  ...
  const visualItems = useMemo<HistoryTimelineVisualItem[]>(
    () =>
      historyItems.map((item, index) => {
        const target = item.target;
        const itemToolIcon = getDocumentHistoryItemToolIcon(
          item,
          snapshot?.document.features ?? [],
        );
        const targetKey = getPrimitiveRefKey(target);
        const isSelected = visibleSelection.some(
          (entry) => getPrimitiveRefKey(entry) === targetKey,
        );
        const isAllowed = selectionFilterAllowsTarget(
          selectionFilter,
          selection,
          target,
          selectionCatalog,
        ...
        const isDraggingItem =
          itemDragState?.active && itemDragState.item.id === item.id;
        const isHistoryHighlighted =
          item.kind === "feature" &&
          historyHighlightFeatureIdSet.has(item.featureId);
        const itemFeatureDiagnostics =
          item.kind === "feature"
        ...
        const menuItems: WorkbenchContextMenuEntry[] =
          getDocumentHistoryMenuEntryDescriptors({
            ...
            canChangeSketchPlane:
              item.kind === "sketch" &&
              canReassignCommittedSketchPlane(snapshot ?? null, item.sketchId),
          }).map((entry) => {
            switch (entry.id) {
              case "edit":
                return {
              ...
              case "change-sketch-plane":
                return {
                  ...
                  onSelect: () =>
                    item.kind === "sketch"
                      ? onChangeSketchPlaneTarget?.(item.target)
                      : undefined,
                };
              case "export":
                return {
                  ...
                  icon: (
                    <WorkbenchIcon name="download" className="h-3.5 w-3.5" />
                  ),
                  onSelect: () =>
                    item.kind === "sketch" ? onExportItem(item) : undefined,
                };
              case "suppress":
                return {
                  kind: "item" as const,
              ...
              case "roll-history-here":
                return {
                  ...
                  icon: (
                    <WorkbenchIcon name="history" className="h-3.5 w-3.5" />
                  ),
              ...
              case "roll-to-end":
                return {
                  ...
                  onSelect: () =>
                    onCursorRequested?.(
                      getPositionCursor(historyItems.length - 1),
                    ),
                };
  ...
  return (
    <HistoryTimelineSurface
      ...
      onCursorRequested={(index) =>
        onCursorRequested?.(getPositionCursor(index))
      }
      getCursorAriaLabel={(index) =>
        getTimelineCursorAriaLabel(historyItems, index)
      }
