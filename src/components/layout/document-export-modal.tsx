  ...
  if (field.kind === "enum") {
    return (
      <Select
        ...
        onChange={(value) => {
          if (value !== null) {
            onPatch({ [field.patch.patchKey]: value });
          }
        }}
      />
    );
  ...
  if (field.kind === "custom" && field.rendererId === "checkbox") {
    ...
    return (
      <Checkbox
        ...
        onChange={(event) =>
          onPatch({ [payload.patchKey]: event.currentTarget.checked })
        }
      />
    );
  }
...
function DocumentExportModalContent({
  ...
  const formatData = providers.map((p) => ({
    label: p.label,
    value: p.formatId,
  }));
  ...
  return (
    <Stack gap="md">
      ...
      <Group justify="flex-end" gap="xs">
        <Button
          variant="subtle"
          leftSection={<WorkbenchIcon name="close" size={14} />}
          onClick={onClose}
          disabled={pending}
        >
          Cancel
        ...
        <Button
          ...
          loading={pending}
          disabled={!target || !hasCompatibleProviders}
        >
          Export
        </Button>
      </Group>
