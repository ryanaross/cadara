...
describe('formatBuildMetadata', () => {
  test('includes version, commit, and visible runtime mode', () => {
    expect(formatBuildMetadata({ version: '1.2.3', commit: 'abc1234', mode: 'dev' })).toBe('v1.2.3 abc1234 dev')
  })

  test('omits production mode', () => {
    expect(formatBuildMetadata({ version: '1.2.3', commit: 'abc1234', mode: null })).toBe('v1.2.3 abc1234')
  })
})
