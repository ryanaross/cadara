...
function renderBar(state: WorkbenchTabsState, props: Partial<Parameters<typeof DocumentTabsBar>[0]> = {}) {
...
test('document-tabs-bar renders one tab per document with role=tab and the tablist landmark', () => {
  const state: WorkbenchTabsState = {
    tabs: [tab(docA, { title: 'Bracket v3' }), tab(docB, { title: 'shaft.cadara' })],
    activeDocumentId: docA,
  }

  const markup = renderBar(state)

  expectTrue(markup.includes('aria-label="Open documents"'), 'tablist should expose accessible label.')
  expectTrue(markup.includes('role="tablist"'), 'strip should declare role=tablist.')
  expectTrue(markup.split('role="tab"').length - 1 === 2, 'one role=tab element per open document.')
  expectTrue(markup.includes('Bracket v3'), 'tab labels should render verbatim.')
  expectTrue(markup.includes('shaft.cadara'), 'second tab label should render verbatim.')
})
...
test('document-tabs-bar renders the storage glyph corresponding to each tab', () => {
  const state: WorkbenchTabsState = {
    tabs: [
      ...
      tab(docB, { storageKind: 'filesystem', storageDescriptor: 'shaft.cadara' }),
    ],
    activeDocumentId: docA,
  }
...
test('document-tabs-bar surfaces the storage descriptor in the accessible label', () => {
  const state: WorkbenchTabsState = {
    tabs: [tab(docA, { storageKind: 'filesystem', title: 'shaft', storageDescriptor: 'shaft.cadara' })],
    activeDocumentId: docA,
  }
...
test('document-tabs-bar does not let the only tab be dragged or closed', () => {
  ...
  expectTrue(!markup.includes('draggable="true"'), 'single tab should not be draggable.')
  expectTrue(!markup.includes('data-document-tab-close'), 'single tab should not show close.')
})
