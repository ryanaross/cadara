...
export const HISTORY_TIMELINE_COLLAPSED_STORAGE_KEY = 'cad.workbench.historyTimelineCollapsed.v1'

export function getHistoryTimelinePanelMotionState(collapsed: boolean): HistoryTimelinePanelMotionState {
  return {
    ...
    style: {
      ...
      transition: 'transform 220ms cubic-bezier(0.25, 1, 0.5, 1), opacity 140ms cubic-bezier(0.25, 1, 0.5, 1)',
    },
  }
}
...
export function readHistoryTimelineCollapsedPreference(
  ...
  try {
    ...
    console.warn('History timeline collapsed preference could not be read.', error)
    return false
  }
}
...
export function writeHistoryTimelineCollapsedPreference(
  ...
  try {
    storage.setItem(HISTORY_TIMELINE_COLLAPSED_STORAGE_KEY, collapsed ? 'true' : 'false')
  } catch (error) {
    console.warn('History timeline collapsed preference could not be saved.', error)
  }
}
