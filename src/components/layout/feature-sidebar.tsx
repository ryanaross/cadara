...
import { featureTreeSelections, objectSelections } from '@/domain/editor/mock-document'
...
export function FeatureSidebar({ onSelectTarget }: FeatureSidebarProps) {
  const {
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="space-y-1 px-3 py-3">
            {featureTreeItems.map((item) => {
              ...
              const target = featureTreeSelections[item.id]
              const isSelected =
                target !== undefined &&
                selection.some((entry) => getPrimitiveRefKey(entry) === getPrimitiveRefKey(target))
              const isAllowed = target ? selectionFilterAllowsTarget(selectionFilter, target) : false
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="space-y-1 px-3 py-3">
            {partStudioObjects.map((item) => {
              const target = objectSelections[item.id]
              const isSelected =
                target !== undefined &&
                selection.some((entry) => getPrimitiveRefKey(entry) === getPrimitiveRefKey(target))
              const isAllowed = target ? selectionFilterAllowsTarget(selectionFilter, target) : false
      ...
      <section className="border-t border-[var(--cad-border)] px-4 py-3">
        ...
        <p className="mt-1 text-xs text-[var(--cad-muted-foreground)]">
          ...
          <span className="text-[var(--cad-foreground)]">
            {activeEditSession?.draftLabel ?? 'No feature selected'}
          </span>
        </p>
        <p className="mt-1 text-xs text-[var(--cad-muted-foreground)]">
          Selection:{' '}
