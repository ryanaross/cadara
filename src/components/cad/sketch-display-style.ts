...
export function shouldDepthTestSketchDisplayMarker(
  renderable: SketchSessionDisplayRenderable,
) {
  return renderable.markerLayer !== "overlay";
}

export function getSketchDisplayMarkerRenderOrder(
  renderable: SketchSessionDisplayRenderable,
) {
  return renderable.markerLayer === "overlay" ? 5 : 4;
}

export function shouldApplySketchDisplayStyles(
  mode: ToolbarMode,
  hasSketchSession: boolean,
) {
...
export function getSketchDisplayPolylineMaterialConfig(
  ...
  const linePattern = isDiagnostic
    ? "solid"
    : hasAuthoredDash
      ? "dashed"
  ...
  return {
    ...
    opacity: isDiagnostic
      ? 1
      : applyStyles
        ? (renderable.strokeStyle?.opacity ?? defaultOpacity)
        : defaultOpacity,
    lineWidth: isDiagnostic
      ? 1
      : applyStyles
        ? (renderable.strokeStyle?.width ?? 1)
        : 1,
    lineCap: isDiagnostic
      ? "round"
      : applyStyles
        ? (renderable.strokeStyle?.lineCap ?? "round")
        : "round",
    lineJoin: isDiagnostic
      ? "round"
      : applyStyles
        ? (renderable.strokeStyle?.lineJoin ?? "round")
        : "round",
    miterLimit: isDiagnostic
      ? 4
      : applyStyles
        ? (renderable.strokeStyle?.miterLimit ?? 4)
        : 4,
    dashSize: isDiagnostic
      ? 0.24
      : applyStyles
        ? (renderable.strokeStyle?.dashSize ?? 0.24)
        : 0.24,
    gapSize: isDiagnostic
      ? 0.14
      : applyStyles
        ? (renderable.strokeStyle?.gapSize ?? 0.14)
        : 0.14,
  };
}
...
export function getActiveSketchPolylineStrokeGeometryConfig(
  ...
  if (
    !shouldUseSketchStrokeMeshGeometry(renderable, materialConfig, applyStyles)
  ) {
...
export function getActiveSketchMarkerWorldRadii(
  ...
  const visiblePixelRadius = clamp(
    renderable.geometry.displayRadius *
      ACTIVE_SKETCH_MARKER_RADIUS_PIXELS_PER_DISPLAY_UNIT,
    markerBounds.min,
    markerBounds.max,
  );
  ...
  return {
    ...
    visibleRadius: Math.max(
      visiblePixelRadius * worldUnitsPerPixel,
      Number.EPSILON,
    ),
...
export function buildSketchPolylineStrokeGeometry(
  input: BuildSketchPolylineStrokeGeometryInput,
): THREE.BufferGeometry {
  const frame = createStrokeProjectionFrame(
    input.points,
    input.sketchPlaneFrame,
  );
  const projectedPoints = input.points.map((point) =>
    projectWorldPointToStrokeFrame(point, frame),
  );
  const strokeWidth = Math.max(
    input.materialConfig.lineWidth * input.worldUnitsPerPixel,
    1e-6,
  );
  const dashSize = Math.max(
    input.materialConfig.dashSize * input.worldUnitsPerPixel,
    0,
  );
  const gapSize = Math.max(
    input.materialConfig.gapSize * input.worldUnitsPerPixel,
    0,
  );
  ...
  const strokeSegments =
    input.materialConfig.linePattern === "dashed" && cappedDashPattern
  ...
  for (const segment of strokeSegments) {
    ...
    const segmentGeometry = SVGLoader.pointsToStroke(
      [...segment],
      strokeStyle,
      16,
      1e-6,
    ...
    for (let index = 0; index < segmentPosition.count; index += 1) {
      const worldPoint = unprojectStrokeFramePoint(
        new THREE.Vector2(
          segmentPosition.getX(index),
          segmentPosition.getY(index),
        ),
        frame,
      );
...
export function buildSketchGradientMeshMaterial(
    ...
    uniforms: {
      ...
      gradientVector: {
        value: new THREE.Vector3(
          Math.cos(config.fill.angleRadians),
          Math.sin(config.fill.angleRadians),
          0,
        ),
      },
...
export function updateSketchGradientMaterialFrame(
  ...
  if (
    ...
    config.fill.kind !== "linearGradient"
  ) {
    return;
  }
  ...
  const direction = getGradientDirection(
    config.fill.angleRadians,
    config.sketchPlaneFrame,
  );
  const halfLength =
    Math.max(
  ...
  material.uniforms.gradientLengthSquared.value = Math.max(
    gradientVector.lengthSq(),
    1e-12,
  );
}

function getSvgStrokePathPoints(
  points: readonly THREE.Vector2[],
  isClosed: boolean,
): THREE.Vector2[] {
...
function createStrokeProjectionFrame(
  ...
  const normal =
    getFirstNonCollinearNormal(points, xAxis) ?? getFallbackNormal(xAxis);
  const yAxis = normal.clone().cross(xAxis).normalize();
  return { origin, xAxis, yAxis };
}

function projectWorldPointToStrokeFrame(
  point: readonly [number, number, number],
  frame: StrokeProjectionFrame,
) {
  const delta = vectorFromTuple(point).sub(frame.origin);
  return new THREE.Vector2(delta.dot(frame.xAxis), delta.dot(frame.yAxis));
}

function unprojectStrokeFramePoint(
  point: THREE.Vector2,
  frame: StrokeProjectionFrame,
) {
  return frame.origin
...
function getFirstSegmentDirection(
  points: readonly (readonly [number, number, number])[],
) {
  for (let index = 1; index < points.length; index += 1) {
    const direction = vectorFromTuple(points[index]!).sub(
      vectorFromTuple(points[index - 1]!),
    );
...
export function getSketchDisplayMarkerMaterialConfig(
  ...
  return {
    color: applyStyles
      ? (renderable.paintStyle?.color ??
        renderable.strokeStyle?.color ??
        defaultColor)
      : defaultColor,
    transparent:
      applyStyles &&
      (renderable.paintStyle?.opacity !== undefined ||
        renderable.strokeStyle?.opacity !== undefined),
    opacity: applyStyles
      ? (renderable.paintStyle?.opacity ?? renderable.strokeStyle?.opacity ?? 1)
      : 1,
  };
}
