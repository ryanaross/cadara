...
export function getSketchSpecialModeSegmentProjectionId(id: string, point: 'start' | 'end') {
  return `sketch-special-segment:${id}:${point}`
}

export function getSketchSpecialModeOverlayTone(overlay: SketchSpecialModeViewportOverlay) {
  switch (overlay.kind) {
    case 'diagnostic':
