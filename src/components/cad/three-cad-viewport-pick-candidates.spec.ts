...
import type { RegionId, RenderableId, SketchEntityId, SketchId, SketchPointId } from '@/contracts/shared/ids'
...
test('src/components/cad/three-cad-viewport-pick-candidates.spec.ts', () => {
  ...
  expectTrue(
    axisCandidates[0]?.target.kind === 'sketchDatumReference' && axisCandidates[0].target.datumId === 'xAxis',
...
test('active sketch curves are collected as screen-space semantic candidates', () => {
  ...
  const lineTarget = session.definition.entities.find((entry) => entry.entityId === 'sketch_entity_line')!.target
  const constructionTarget = session.definition.entities.find((entry) => entry.entityId === 'sketch_entity_construction')!.target
  ...
  expectTrue(
    constructionCandidates.some((candidate) => candidate.target === constructionTarget),
...
function makeCurveSession(sketchId: SketchId) {
  ...
  const line = createLineEntityDefinition(sketchId, 'sketch_entity_line' as SketchEntityId, 'Line', points[0]!.pointId, points[1]!.pointId)
  const constructionLine = createLineEntityDefinition(sketchId, 'sketch_entity_construction' as SketchEntityId, 'Construction', points[2]!.pointId, points[3]!.pointId, true)
  const circle = createCircleEntityDefinition(sketchId, 'sketch_entity_circle' as SketchEntityId, 'Circle', points[4]!.pointId, 2)
  const arc = createArcEntityDefinition(sketchId, 'sketch_entity_arc' as SketchEntityId, 'Arc', points[4]!.pointId, points[5]!.pointId, points[6]!.pointId, 'counterClockwise')
  const spline = createSplineEntityDefinition(sketchId, 'sketch_entity_spline' as SketchEntityId, 'Spline', [points[7]!.pointId, points[8]!.pointId, points[9]!.pointId])
  const definition: SketchDefinition = {
    ...createNewSketchSession(createStandardPlaneDefinition('xy')).definition,
    pointIds: points.map((entry) => entry.pointId),
    points,
    entityIds: [line.entityId, constructionLine.entityId, circle.entityId, arc.entityId, spline.entityId],
