...
export const VIEW_CUBE_CORNER_CUT_SIZE = 0.3;
...
export function createViewCubeScene(): ViewCubeSceneState {
  ...
  const faceFillGeometry = createViewCubeFaceGeometry(
    VIEW_CUBE_BODY_HALF_SIZE,
    VIEW_CUBE_CORNER_CUT_SIZE,
  );
  const faceFillMaterial = new THREE.MeshBasicMaterial({
    color: 0x314255,
  });
  const faceOutlineGeometry = createViewCubeFaceOutlineGeometry(
    VIEW_CUBE_BODY_HALF_SIZE,
    VIEW_CUBE_CORNER_CUT_SIZE,
  );
  const faceHitGeometry = createViewCubeFaceGeometry(
    VIEW_CUBE_BODY_HALF_SIZE,
    VIEW_CUBE_CORNER_CUT_SIZE,
  );
  ...
  for (const faceTarget of VIEW_CUBE_FACE_TARGETS) {
    ...
    const faceSurfacePosition = faceNormal
      .clone()
      .multiplyScalar(VIEW_CUBE_BODY_HALF_SIZE);
    const faceControlPosition = faceNormal
      .clone()
      .multiplyScalar(VIEW_CUBE_BODY_HALF_SIZE + VIEW_CUBE_SURFACE_OFFSET);
    ...
    label.mesh.position.copy(
      faceNormal
        .clone()
        .multiplyScalar(VIEW_CUBE_BODY_HALF_SIZE + VIEW_CUBE_LABEL_OFFSET),
    );
  ...
  for (const cornerTarget of VIEW_CUBE_CORNER_TARGETS) {
    ...
    const cornerFaceGeometry = createViewCubeCornerFaceGeometry(
      cornerDirection,
      0,
    );
    ...
    const cornerOutline = new THREE.LineLoop(
      cornerFaceOutlineGeometry,
      cornerFaceOutlineMaterial,
    );
...
export function createViewCubeFaceGeometry(
  halfSize: number,
  ...
  geometry.setIndex([0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5, 0, 5, 6, 0, 6, 7]);

  return geometry;
}

export function createViewCubeFaceOutlineGeometry(
  halfSize: number,
  cornerCutSize: number,
) {
  return new THREE.BufferGeometry().setFromPoints(
    createViewCubeFacePoints(halfSize, cornerCutSize),
  );
}

export function createViewCubeFacePoints(
  halfSize: number,
  cornerCutSize: number,
) {
  return [
...
export function createViewCubeCornerFaceGeometry(
  cornerDirection: THREE.Vector3,
...
export function createViewCubeCornerFaceOutlineGeometry(
  cornerDirection: THREE.Vector3,
  surfaceOffset: number,
) {
  return new THREE.BufferGeometry().setFromPoints(
...
export function createViewCubeCornerFacePoints(
  cornerDirection: THREE.Vector3,
  surfaceOffset: number,
) {
  ...
  const normalOffset = new THREE.Vector3(x, y, z)
    .normalize()
  ...
  const points = [
    new THREE.Vector3(x * (halfSize - cutSize), y * halfSize, z * halfSize).add(
      normalOffset,
    ),
    new THREE.Vector3(x * halfSize, y * (halfSize - cutSize), z * halfSize).add(
      normalOffset,
    ),
    new THREE.Vector3(x * halfSize, y * halfSize, z * (halfSize - cutSize)).add(
      normalOffset,
    ),
  ];
...
export function createViewCubeLabelMesh(
  label: string,
...
export function resolveViewCubeLabelColors() {
  ...
  return {
    outlineColor:
      rootStyles.getPropertyValue("--workbench-viewport-background").trim() ||
      "black",
    textColor:
      rootStyles.getPropertyValue("--workbench-shell-text").trim() || "white",
  };
}

export function createViewCubePlaneQuaternion(
  normal: THREE.Vector3,
  up: THREE.Vector3,
) {
...
export function updateViewCubeVisibility(
  ...
  viewCubeScene.cornerVisuals.forEach((cornerVisual) => {
    cornerVisual.outlineMaterial.opacity =
      cornerVisual.presetId === hoveredPresetId ? 1 : 0.4;
  });
}
...
export function resolveViewCubePresetId(object: THREE.Object3D | undefined) {
  ...
  return typeof presetId === "string"
    ? (presetId as ViewNavigationPresetId)
    : null;
}
