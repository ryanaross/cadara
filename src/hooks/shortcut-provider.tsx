...
export function ShortcutProvider({
  ...
  const registry = useMemo(() => createShortcutCommandRegistry(commands), [commands])
  const resolvedRepository = useMemo(
    () => repository === undefined ? createDefaultShortcutProfileRepository() : repository,
    [repository],
  )
  ...
  useEffect(() => {
    ...
    void resolvedRepository?.load().then((loadedOverrides) => {
      ...
      const nextOverrides = pruneUnknownShortcutOverrides(loadedOverrides, registry)
      setOverrides((current) =>
        shortcutProfileOverridesEqual(current, nextOverrides) ? current : nextOverrides,
      )
    })
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      resolver.handleKeyDown(event, {
        ...
        isCommandEnabled: (command) => commandHandlers[command.id]?.isEnabled?.() ?? Boolean(commandHandlers[command.id]),
        isTextEditingTarget,
      })
    }
  ...
  const saveOverrides = useCallback((nextOverrides: ShortcutProfileOverrides) => {
  ...
  const getConflictsForOverrides = useCallback(
    (nextOverrides: ShortcutProfileOverrides) =>
      detectShortcutConflicts(registry, createEffectiveKeymap(registry, nextOverrides)),
    [registry],
  )
  ...
  const value = useMemo(
    () => ({
      ...
      commands,
      effectiveKeymap,
      getPrimaryShortcut: (commandId: ShortcutCommandId) => getPrimaryShortcut(effectiveKeymap, commandId),
      registry: registry as ShortcutCommandRegistry,
      overrides,
      setCommandShortcuts: (commandId: ShortcutCommandId, shortcuts: readonly string[]) => {
        const nextOverrides = setCommandShortcutOverride(overrides, commandId, shortcuts)
      ...
      resetCommandShortcuts: (commandId: ShortcutCommandId) => {
        const result = validateShortcutOverrideUpdate(registry, resetCommandShortcut(overrides, commandId))
        if (result.nextOverrides) {
          saveOverrides(result.nextOverrides)
        }
        return result.conflicts
    ...
    [activeScopes, commands, effectiveKeymap, getConflictsForOverrides, overrides, registry, saveOverrides],
  )
...
function shortcutProfileOverridesEqual(
  ...
  return leftEntries.every(([commandId, leftOverride]) => {
    ...
    if (!rightOverride || leftOverride.shortcuts.length !== rightOverride.shortcuts.length) {
      return false
    }

    return leftOverride.shortcuts.every((shortcut, index) => shortcut === rightOverride.shortcuts[index])
  })
}
