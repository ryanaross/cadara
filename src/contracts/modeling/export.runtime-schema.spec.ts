...
test('src/contracts/modeling/export.runtime-schema.spec.ts', () => {  const stlDefaults = stlExportProvider.getDefaultOptions()
  ...
  expectTrue(stlDefaults.encoding === 'binary', 'STL export should default to binary encoding.')
  expectTrue(stlDefaults.meshAccuracy.chordTolerance > 0, 'STL defaults should include positive mesh tolerance.')
  expectTrue(threeMfDefaults.includeMetadata, '3MF export should include metadata by default.')
  expectTrue(
  ...
  expectTrue(stepDefaults.schema === 'AP242', 'STEP export should default to AP242.')
  expectTrue(cadaraDefaults.pretty, 'cadara export should default to readable JSON.')
  ...
  expectTrue(parsedStep.format === 'step', 'Export request parsing should preserve the format.')
})
