...
const geometryAssetSchemaVersionSchema =
  literalVersionSchema<GeometryAssetSchemaVersion>(
    GEOMETRY_ASSET_SCHEMA_VERSION,
    "schemaVersion",
    "Unsupported geometry asset schema version",
  );

const geometryAssetManifestSchemaVersionSchema =
  literalVersionSchema<GeometryAssetManifestSchemaVersion>(
    GEOMETRY_ASSET_MANIFEST_SCHEMA_VERSION,
    "schemaVersion",
...
export const geometryAssetHashSchema = z
  ...
  .regex(
    /^sha256:[a-f0-9]{64}$/,
    "Geometry asset hash must be a sha256:<hex> content hash.",
  )
...
function validateWeightedCurve(
  ...
  if (
    ...
    value.knots.length !== value.multiplicities.length
  ) {
    ctx.addIssue({
      code: "custom",
...
function validateWeightedSurface(
  ...
  if (
    value.uKnots &&
    value.uMultiplicities &&
    value.uKnots.length !== value.uMultiplicities.length
  ) {
    ctx.addIssue({
      code: "custom",
  ...
  if (
    value.vKnots &&
    value.vMultiplicities &&
    value.vKnots.length !== value.vMultiplicities.length
  ) {
    ctx.addIssue({
      code: "custom",
...
const cadaraBrepTopologySchema = z
  .object({
    vertices: z.array(
      z
        .object({
  ...
  .superRefine((topology, ctx) => {
    ...
    for (const [index, coedge] of topology.coedges.entries()) {
      if (
        coedge.curve2d.kind === "bezier" ||
        coedge.curve2d.kind === "bSpline"
      ) {
        validateWeightedCurve(coedge.curve2d, ctx, [
          "coedges",
          index,
          "curve2d",
        ]);
    ...
    for (const [index, face] of topology.faces.entries()) {
      ...
      if (
        ...
        (face.surface.basisCurve.kind === "bezier" ||
          face.surface.basisCurve.kind === "bSpline")
      ) {
        validateWeightedCurve(face.surface.basisCurve, ctx, [
          "faces",
          index,
          "surface",
          "basisCurve",
        ...
        for (const vertexIndex of triangle) {
          if (vertexIndex >= face.meshVertices.length) {
            ctx.addIssue({
              code: "custom",
              message:
                "Cadara B-rep face triangle references a missing vertex.",
...
export const geometryAssetRecordSchema = z
  .object({
    ...
    byteLength: z
      .number()
      .int()
      .positive("Geometry asset byte length must be positive."),
    format: z.union([
      z.literal("step"),
      z.literal("stl"),
      z.literal("3mf"),
    ...
    provenance: z
      .object({
        ...
        sourceFormat: z
          .union([z.literal("step"), z.literal("stl"), z.literal("3mf")])
          .optional(),
...
export const geometryAssetManifestSchema = z
  .object({
  ...
  .transform(
    (value) => normalizeGeometryAssetManifest(value) as GeometryAssetManifest,
  );

export const legacyGeometryAssetManifestSchema = z
  .array(geometryAssetRecordSchema)
...
function sameAssetContentMetadata(
  ...
  return (
    ...
    (left.provenance.sourceName ?? null) ===
      (right.provenance.sourceName ?? null) &&
    (left.provenance.selectedFileName ?? null) ===
      (right.provenance.selectedFileName ?? null) &&
    (left.provenance.stepDocumentName ?? null) ===
      (right.provenance.stepDocumentName ?? null) &&
    (left.provenance.sourceHash ?? null) ===
      (right.provenance.sourceHash ?? null) &&
    (left.provenance.sourceFormat ?? null) ===
      (right.provenance.sourceFormat ?? null) &&
    (left.provenance.sourceStored ?? null) ===
      (right.provenance.sourceStored ?? null) &&
    (left.provenance.generator ?? null) ===
      (right.provenance.generator ?? null) &&
