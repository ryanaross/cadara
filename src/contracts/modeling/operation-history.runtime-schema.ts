...
import { OPERATION_HISTORY_SCHEMA_VERSION, type OperationHistorySchemaVersion } from '@/contracts/shared/versioning'
import {
...
const advancedOperationIntents = new Set(['create', 'add', 'subtract', 'intersect'])
...
const operationHistorySchemaVersionSchema = literalVersionSchema<OperationHistorySchemaVersion>(
  OPERATION_HISTORY_SCHEMA_VERSION,
  'schemaVersion',
  'Unsupported operation history schema version',
)

const persistedCommitSketchPayloadSchema = z.object({
...
const rawFeatureDefinitionSchema = z.unknown().superRefine((value, ctx) => {
  if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
    ...
    if ((definition.kind === 'extrude' || definition.kind === 'revolve') && typeof definition.parameters === 'object' && definition.parameters !== null && !Array.isArray(definition.parameters)) {
      ...
      if (!Array.isArray(parameters.profiles) || parameters.profiles.length === 0) {
        ctx.addIssue({
          code: 'custom',
    ...
    if (typeof definition.kind === 'string' && advancedFeatureKinds.has(definition.kind) && typeof definition.parameters === 'object' && definition.parameters !== null && !Array.isArray(definition.parameters)) {
      ...
      for (const participant of parameters.participants) {
        if (
          ...
          || !advancedParticipantRoles.has((participant as Record<string, unknown>).role as string)
...
const persistedReorderDocumentHistoryPayloadSchema = z.object({
...
const persistedSetFeatureCursorPayloadSchema = z.object({
...
const operationHistoryEntrySchema = z.object({
  ...
  switch (value.kind) {
    case 'commitSketch':
      return { kind: value.kind, payload: persistedCommitSketchPayloadSchema.parse(value.payload) }
    case 'createFeature':
    ...
    case 'reorderDocumentHistory':
      return { kind: value.kind, payload: persistedReorderDocumentHistoryPayloadSchema.parse(value.payload) }
    case 'setFeatureCursor':
...
export const operationHistoryPayloadSchema = z.object({
...
function mapIssueToValidationFailure(issue: ZodIssue): Exclude<OperationHistoryValidationResult, { ok: true }> {
  ...
  if (issue.message.startsWith('Unsupported operation history schema version')) {
    return {
      ok: false,
  ...
  if (issue.message === 'Entry payload contains transport-only request metadata.') {
    return {
      ok: false,
  ...
  if (issue.message.includes('inconsistent point sketch IDs') || issue.message.includes('inconsistent entity sketch IDs')) {
    return {
      ok: false,
  ...
  if (String(issue.path[2] ?? '') === 'definition' && String(issue.path[0] ?? '') === 'payload') {
    return {
      ok: false,
...
export function parseOperationHistoryPayload(value: unknown): OperationHistoryValidationResult {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    return {
      ok: false,
