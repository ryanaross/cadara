...
const persistedSketchHistoryCursorSchema = z.discriminatedUnion('kind', [
  z.object({ kind: z.literal('empty') }).strict(),
  z.object({ kind: z.literal('item'), itemId: stringSchema }).strict(),
]).transform((value) => value as PersistedSketchHistoryCursor)

const persistedSketchHistoryOperationSchema = z.object({
  itemId: stringSchema,
...
}).strict().transform((value) => value as PersistedSketchHistoryOperation)

const persistedSketchCommitRequestSchema = z.object({
...
}).strict().transform((value) => value as PersistedSketchDraftSession['commitRequest'])

export const persistedSketchDraftSessionSchema = z.object({
  sketchId: sketchIdSchema.nullable(),
  sketchLabel: stringSchema,
  plane: sketchPlaneDefinitionSchema,
  definition: sketchDefinitionSchema,
...
}).strict().transform((value) => value as PersistedSketchDraftSession)

const authoredDocumentStackSchema = z.array(authoredModelDocumentSchema)
const draftHistoryEntrySchema = z.object({
...
export const documentLocalDurableHistoryStateSchema = z.object({
...
}).strict().transform((value) => value as DocumentLocalDurableHistoryState)
...
export function parseDocumentLocalDurableHistoryState(value: unknown) {
  ...
  if (!result.success) {
    return {
      ok: false as const,
      message: result.error.issues[0]?.message ?? 'Durable history payload is invalid.',
    }
  }
...
export function createDurableHistoryAvailability(input: DurableHistoryAvailability): DurableHistoryAvailability {
  return {
