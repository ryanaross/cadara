...
import { contractVersionSchema, documentIdSchema, literalVersionSchema, point2dSchema, requestIdSchema, revisionIdSchema, sketchIdSchema, sketchPointIdSchema } from '@/contracts/shared/runtime-schema'
...
const interactiveSketchSolveSessionIdSchema = z.string()
  .regex(/^interactive_sketch_solve_.+$/, 'Interactive solve session ID is invalid.')
...
export const startInteractiveSketchSolveSessionRequestSchema = sketchSolverEnvelopeSchema.extend({
  plane: sketchPlaneFrameSchema,
  definition: sketchDefinitionSchema,
  tolerances: z.unknown(),
  partialSolvePolicy: z.union([z.literal('bestEffort'), z.literal('failOnConflict')]),
...
}).transform((value) => value as unknown as StartInteractiveSketchSolveSessionRequest)

export const updateInteractiveSketchSolveSessionRequestSchema = sketchSolverEnvelopeSchema.extend({
  sessionId: interactiveSketchSolveSessionIdSchema,
  dragTarget: solverDraggedSketchPointTargetSchema,
}).transform((value) => value as unknown as UpdateInteractiveSketchSolveSessionRequest)

export const finalizeInteractiveSketchSolveSessionRequestSchema = sketchSolverEnvelopeSchema.extend({
  sessionId: interactiveSketchSolveSessionIdSchema,
}).transform((value) => value as unknown as FinalizeInteractiveSketchSolveSessionRequest)

export const disposeInteractiveSketchSolveSessionRequestSchema = sketchSolverEnvelopeSchema.extend({
  sessionId: interactiveSketchSolveSessionIdSchema,
}).transform((value) => value as unknown as DisposeInteractiveSketchSolveSessionRequest)
