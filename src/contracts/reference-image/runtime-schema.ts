...
export const referenceImagePayloadSchema = z.object({
  mediaType: z
    .string()
    .trim()
    .min(1, "Reference-image media type must not be empty."),
  fileName: z.string().trim().min(1).optional(),
  pixelWidth: positiveNumberSchema(
    "Reference-image pixel width must be positive.",
  ),
  pixelHeight: positiveNumberSchema(
    "Reference-image pixel height must be positive.",
  ),
  base64Data: z
    .string()
    .trim()
    .min(1, "Reference-image base64 payload must not be empty.")
...
export const referenceImageCalibrationScaleModeSchema = z
  .union([z.literal("lockedAspect"), z.literal("independent")])
  .transform((value) => value as ReferenceImageCalibrationScaleMode);

export const referenceImageCalibrationAnchorSchema = z
  .object({
    anchorId: z
      .string()
      .trim()
      .min(1, "Reference-image anchor id must not be empty."),
    label: z
      .string()
      .trim()
      .min(1, "Reference-image anchor label must not be empty."),
    uv: point2dSchema,
    pointId: z
      .string()
      .trim()
      .min(1, "Reference-image anchors must bind to a local point id."),
  })
  .strict()
  .transform((value) => value as ReferenceImageCalibrationAnchor);

export const referenceImageCalibrationStateSchema = z
  .object({
...
export const referenceImageOperationStateSchema = z
  .object({
