...
test('src/contracts/performance/telemetry.spec.ts preserves no-op results and filters expensive attributes', async () => {
  ...
  expectTrue(result.ok === true, 'No-op telemetry should preserve successful operation results.')
  ...
  try {
    ...
    expectTrue(error === rejected, 'No-op telemetry should preserve the exact thrown error.')
  }
  ...
  expectTrue(filtered['cadara.operation'] === 'solveSketch', 'Allowed low-cardinality attributes should be retained.')
  expectTrue(filtered['cadara.constraint_count'] === 4, 'Cheap count attributes should be retained.')
  expectTrue(!('mesh_triangle_total' in filtered), 'Mesh triangle totals should not be allowed telemetry attributes.')
  expectTrue(!('automerge_version_count' in filtered), 'Full Automerge version counts should not be allowed telemetry attributes.')
  expectTrue(!('solver_iteration_count' in filtered), 'Solver iteration counts should not be allowed telemetry attributes.')
  expectTrue(!('pointer_move_x' in filtered), 'Pointer-move attributes should not be allowed telemetry attributes.')
})
...
test('src/contracts/performance/telemetry.spec.ts records Sentry spans with filtered attributes', () => {
  ...
  recordPerformanceMark(telemetry, makeDescriptor('sentry', {
  ...
  expectTrue(started.attributes?.['cadara.operation'] === 'getDocumentSnapshot', 'Sentry spans should receive operation attributes.')
  expectTrue(!('pointer_move_x' in (started.attributes ?? {})), 'Sentry spans should not receive high-frequency attributes.')
  expectTrue(endedAttributes[0]?.['cadara.result'] === 'success', 'Sentry span end should include result classification.')
})

test('src/contracts/performance/telemetry.spec.ts classifies revision results and keeps dev tracing opt-in conservative', () => {
  expectTrue(
    classifyRevisionResult({ revisionState: { kind: 'accepted' } }) === 'success',
    'Accepted revision results should classify as success.',
  )
  expectTrue(
    classifyRevisionResult({ revisionState: { kind: 'rejected' } }) === 'rejected',
    'Rejected revision results should classify as rejected.',
  )
  expectTrue(
    classifyRevisionResult({ revisionState: { kind: 'conflict' } }) === 'conflict',
  ...
  expectTrue(
    !shouldEnablePerformanceTelemetry({ isProduction: false, search: '?cadEnableSentry=1' }),
