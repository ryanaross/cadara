...
export const renderExportSchemaVersionSchema = literalVersionSchema<RenderExportSchemaVersion>(
...
const renderBindingSchema = z.object({
  ...
  topology: z.union([z.literal('face'), z.literal('edge'), z.literal('vertex')]).nullable(),
...
const renderGeometrySchema = z.discriminatedUnion('kind', [
  z.object({
    ...
    triangleIndices: z.array(z.tuple([z.number().int(), z.number().int(), z.number().int()])),
  }),
  z.object({
  ...
  z.object({
    ...
    displayRadius: positiveNumberSchema('Render marker radius must be positive.'),
  }),
])

const renderableEntityRecordSchema = z.object({
...
export const renderExportSchema = z.object({
