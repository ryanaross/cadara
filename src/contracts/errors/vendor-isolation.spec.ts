...
test('src/contracts/errors/vendor-isolation.spec.ts', () => {
  const violations = collectSourceFiles(join(process.cwd(), 'src'))
    ...
    .filter((filePath) => !filePath.endsWith('.spec.ts') && !filePath.endsWith('.spec.tsx'))
    .filter((filePath) => !allowedVendorImportFiles.has(filePath))
    .filter((filePath) => readFileSync(filePath, 'utf8').includes('@sentry/browser'))

  if (violations.length > 0) {
    throw new Error(`Sentry browser SDK imports must stay isolated in the reporter adapter.\n${violations.join('\n')}`)
  }
})
