...
test('src/contracts/errors/app-error.spec.ts', () => {  const cause = new Error('Kernel refused the operation.')
  ...
  expectTrue(normalized.message === cause.message, 'Normalization should preserve Error messages.')
  expectTrue(normalized.cause === cause, 'Normalization should preserve the original cause.')
  expectTrue(normalized.requestId === 'request_preview-1', 'Normalization should preserve request ids.')
  expectTrue(
  ...
  expectTrue(nonError.message === 'Non-Error throw fell back.', 'Non-Error throws should use fallback messages.')
  expectTrue(nonError.cause === 'bad value', 'Non-Error throws should still be retained as causes.')
  ...
  expectTrue(malformed.message === 'Malformed marker fell back.', 'Malformed marked objects should not escape normalization.')
  expectTrue(malformed.cause === malformedMarkedValue, 'Malformed marked objects should still be retained as causes.')
  ...
  expectTrue(zodError.code === 'app/validation-failed', 'Zod failures should get validation codes.')
  expectTrue(zodError.message.length > 0, 'Zod failures should expose a human message.')
  ...
  expectTrue(diagnosticError.message === 'Variable x references missing.', 'Diagnostic messages should be preserved.')
  expectTrue(
  ...
  expectTrue(modelingDiagnostic.severity === 'error', 'Fatal app errors should become error diagnostics.')
  ...
  expectTrue(report !== null, 'Test reporter should keep the first deduped report.')
  expectTrue(duplicate === null, 'Test reporter should suppress duplicate dedupe keys.')
  expectTrue(testReporter.reports.length === 1, 'Test reporter should store reports.')
  ...
  expectTrue(String(consoleRecords[0]?.[0]) === '[app-error]', 'Console reporter should emit actionable records.')
})
