...
export const SENTRY_DSN =
  "https://0a4eee874e00861be4c30cb81fe15ce1@o4511242392961024.ingest.de.sentry.io/4511333375148112";
...
export function initializeSentryErrorReporting(
  options: SentryInitializationOptions = {},
) {
...
export async function checkSentryDsnReachability(
  options: {
    dsn?: string;
    fetchLike?: FetchLike;
  } = {},
...
export function shouldEnablePerformanceTelemetry(input: {
  ...
  return (
    params.get("cadEnableSentry") === "1" &&
    params.get("cadEnablePerfTelemetry") === "1"
  );
}
...
function createSentryPerformanceSpan(
  ...
  const span = client.startInactiveSpan?.({
    ...
    op: descriptor.op,
    attributes: toSentrySpanAttributes(
      filterPerformanceSpanAttributes({
  ...
  return {
    ...
    setAttributes(attributes) {
      span.setAttributes(
        toSentrySpanAttributes(filterPerformanceSpanAttributes(attributes)),
      );
    },
    end(attributes = {}) {
...
function toSentrySpanAttributes(
  attributes: Record<string, PerformanceSpanAttributeValue | undefined>,
) {
  return Object.fromEntries(
    Object.entries(attributes).filter(
      (
        ...
        string,
        Exclude<PerformanceSpanAttributeValue, null | undefined>,
      ] => entry[1] !== null && entry[1] !== undefined,
    ),
  );
}

function toSentrySpanAttributeValue(
