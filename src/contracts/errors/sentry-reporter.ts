...
export function createSentryErrorReporter(
  ...
  return {
    report(error, metadata) {
      ...
      try {
        const captureContext = createSentryCaptureContext(
          error,
          record.metadata,
        );
...
function createSentryCaptureContext(
  ...
  const activeDocumentContext =
    createSentryActiveDocumentContext(activeDocument);
  const externalTags = metadata.externalTracking?.tags ?? {};
  const contextRecord = Object.fromEntries(
    error.context.map((entry) => [entry.key, entry.value]),
  );
  const tags: Record<string, string> = {
    ...
    "active_document.available": String(
      activeDocument.availability === "loaded",
    ),
  };
  ...
  const captureContext: CaptureContext = {
    ...
    extra: {
      ...
      activeDocumentPayload:
        activeDocument.availability === "loaded" &&
        activeDocument.payloadStatus === "attached"
          ? activeDocument.document
          : null,
      causeStack:
        error.cause instanceof Error ? (error.cause.stack ?? null) : null,
    },
  };
...
function createSentryActiveDocumentContext(
