...
function makeDefinition(parts: MutableSketchDefinitionParts): SketchDefinition {
  return {
    ...
    constraintIds: parts.constraints.map(
...
function createBenchmarkPlaneDefinition(
...
export function createGridFixture(
  ...
  const pointIdAt = (row: number, column: number) =>
    createPointId(`${name}_r${row}_c${column}`);
  ...
  for (let row = 0; row <= rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      entities.push(line);
      constraints.push(
        makeHorizontalConstraint(`${name}_h${row}_${column}`, line.entityId),
      );
    }
  }
  ...
  for (let column = 0; column <= columns; column += 1) {
    for (let row = 0; row < rows; row += 1) {
      ...
      entities.push(line);
      constraints.push(
        makeVerticalConstraint(`${name}_v${column}_${row}`, line.entityId),
      );
    }
  }
  ...
  for (let column = 0; column < columns; column += 1) {
    dimensions.push(
      makeHorizontalDistanceDimension(
...
function createConstraintChainFixture(
  name: string,
  segmentCount: number,
): SketchSolverBenchmarkFixture {
  const sketchId = `sketch_benchmark_${name.replaceAll("-", "_")}` as SketchId;
  ...
  for (let index = 0; index <= segmentCount; index += 1) {
    points.push(
      makePoint(sketchId, `${name}_p${index}`, `P${index}`, [
        index,
        index % 2 === 0 ? 0 : 0.02,
      ]),
    );
  ...
  for (let index = 0; index < segmentCount; index += 1) {
    ...
    entities.push(line);
    constraints.push(
      makeHorizontalConstraint(`${name}_h${index}`, line.entityId),
    );
    dimensions.push({
...
function createIndependentComponentFixture(): SketchSolverBenchmarkFixture {
  const first = createConstraintChainFixture("independent-a", 5).sketch
    .definition;
  const second = createConstraintChainFixture("independent-b", 5).sketch
    .definition;
...
function createThreeBranchFallbackFixture(): SketchSolverBenchmarkFixture {
  ...
  const anchor1 = makePoint(
    sketchId,
    "branch_anchor_1",
    "Anchor 1",
    [-6.45492548301263e-11, -20.00000000144171],
  );
  ...
  const p6 = makePoint(
    ...
    "branch_p6",
    "P6",
    [16.420739052696923, -15.60007626054567],
  );
  const p8 = makePoint(
    sketchId,
    "branch_p8",
    "P8",
    [32.39551360005226, -21.414418708092832],
  );
  const p12 = makePoint(
    sketchId,
    "branch_p12",
    "P12",
    [16.420739050742696, 4.399923751375514],
  );
  const p13 = makePoint(
    sketchId,
    "branch_p13",
    "P13",
    [32.39551359627179, -1.4144187060831115],
  );
  const p19 = makePoint(
    sketchId,
    "branch_p19",
    "P19",
    [32.395513600164705, -21.414418707726117],
  );
  const p21 = makePoint(
    sketchId,
    "branch_p21",
    "P21",
    [15.974774547466922, -25.81434244596269],
  );
  ...
  const fixture = makeFixture(
    ...
    makeAuthoredSketchRecord(
      sketchId,
      "Benchmark three-branch fallback",
      makeDefinition({
...
export function evaluateSketchSolverBenchmarkFixture(
  ...
  const dragResult = dragPoint
    ? updateCompiledSketchSolveSession(
        session,
        {
          ...
          position: fixture.interactiveDragTarget?.position ?? [
            dragPoint.position[0] + 0.05,
            dragPoint.position[1],
          ],
        },
  ...
  return {
    ...
    loopCount: extracted.regions.reduce(
      (total, region) => total + region.loops.length,
      0,
    ),
