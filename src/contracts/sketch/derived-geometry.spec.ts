...
test('evaluateSketchDerivations mirrors geometry and reverses mirrored arc sweep direction', () => {
  const definition = makeSketchDefinition({
    ...
    entities: [
      ...
      makeArc('mirror_arc', 'mirror_center', 'mirror_start', 'mirror_end', 'clockwise'),
    ],
...
test('evaluateSketchDerivations applies linear, circular, and transform relationships through the exported seam', () => {
  const definition = makeSketchDefinition({
    ...
    entities: [
      ...
      makeSpline('seed_spline', ['spline_seed_a', 'spline_seed_b', 'spline_seed_c']),
      makeSpline('rotated_spline', ['spline_out_a', 'spline_out_b', 'spline_out_c']),
    ],
    derivedRelationships: [
      makeRelationship({
        kind: 'linearPattern',
  ...
  assertPoint(pointPosition(result.definition, 'pattern_out'), [0, 2], 'Circular patterns should rotate points around the requested center.')
...
function makePoint(pointId: string, position: SketchPoint2D): SketchPointDefinition {
  return {
...
function makeLine(entityId: string, startPointId: string, endPointId: string): SketchEntityDefinition {
  return {
    kind: 'lineSegment',
...
function makePointEntity(entityId: string, pointId: string): SketchEntityDefinition {
  return {
    kind: 'point',
    entityId: entityId as SketchEntityDefinition['entityId'],
    label: entityId,
...
function makeCircle(entityId: string, centerPointId: string, radius: number): SketchEntityDefinition {
  return {
    kind: 'circle',
    entityId: entityId as SketchEntityDefinition['entityId'],
    label: entityId,
...
function makeSpline(entityId: string, fitPointIds: string[]): SketchEntityDefinition {
  return {
    kind: 'spline',
    entityId: entityId as SketchEntityDefinition['entityId'],
    label: entityId,
    ...
    isConstruction: false,
    fitPointIds: fitPointIds as SketchEntityDefinition extends { kind: 'spline'; fitPointIds: infer T } ? T : never,
    degree: 3,
  }
}

function makeRelationship(definition: SketchDerivationDefinition): SketchDerivationDefinition {
  return definition
}
