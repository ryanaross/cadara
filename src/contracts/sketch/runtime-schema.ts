...
export const sketchSchemaVersionSchema = z.literal(
  "sketch-definition/v1alpha1",
);
export const solvedSketchSchemaVersionSchema = z.literal(
  "solved-sketch/v1alpha1",
);

const projectedSketchGeometryRefSchema = z
  .object({
    kind: z
      .union([
...
const sketchReferenceDefinitionSchema = z
...
const sketchStyleDefinitionSchema = z.object({
  fillMode: z
    .union([z.literal("none"), z.literal("solid"), z.literal("gradient")])
    .optional(),
  ...
  strokeCap: z
    .union([z.literal("butt"), z.literal("round"), z.literal("square")])
    .optional(),
  strokeJoin: z
    .union([z.literal("miter"), z.literal("round"), z.literal("bevel")])
    .optional(),
...
const sketchPointDefinitionSchema = z
  .object({
...
const sketchEntityDefinitionSchema = z
  .discriminatedUnion("kind", [
    z.object({
    ...
    z.object({
      ...
      minorRadius: positiveNumberSchema(
        "Ellipse minor radius must be positive.",
      ),
      style: sketchStyleDefinitionSchema.optional(),
    }),
    z.object({
      ...
      endPointId: sketchPointIdSchema,
      minorRadius: positiveNumberSchema(
        "Elliptical arc minor radius must be positive.",
      ),
      sweepDirection: z.union([
        z.literal("clockwise"),
        z.literal("counterClockwise"),
...
const referencePointConstraintOperandSchema = z.union([
  sketchDatumConstraintOperandSchema,
  projectedSketchGeometryConstraintOperandSchema,
]);

const referenceCurveConstraintOperandSchema = z.union([
  sketchDatumConstraintOperandSchema,
  projectedSketchGeometryConstraintOperandSchema,
]);

const dimensionPointConstraintOperandSchema = z.union([
  localSketchPointConstraintOperandSchema,
  sketchDatumConstraintOperandSchema,
  projectedSketchGeometryConstraintOperandSchema,
]);

const dimensionCurveConstraintOperandSchema = z.union([
  localSketchEntityConstraintOperandSchema,
  sketchDatumConstraintOperandSchema,
  projectedSketchGeometryConstraintOperandSchema,
]);
...
const constraintDefinitionSchema = z
  .discriminatedUnion("kind", [
    ...
    z.object({
      ...
      projectedPoint: referencePointConstraintOperandSchema,
    }),
    z.object({
      constraintId: constraintIdSchema,
      ...
      label: z.string(),
      point: localSketchPointConstraintOperandSchema,
      projectedCurve: referenceCurveConstraintOperandSchema,
    }),
    z.object({
      constraintId: constraintIdSchema,
...
const dimensionDefinitionSchema = z
  .discriminatedUnion("kind", [
    ...
    z.object({
      dimensionId: dimensionIdSchema,
      kind: z.literal("pointDatumDistance"),
      label: z.string(),
      axis: z.union([
        z.literal("aligned"),
        z.literal("horizontal"),
      ...
      point: localSketchPointConstraintOperandSchema,
      datum: sketchDatumConstraintOperandSchema,
      value: z.number(),
      annotationPlacement: dimensionLineAnnotationPlacementSchema.optional(),
    }),
    z.object({
    ...
    z.object({
      ...
      label: z.string(),
      lines: z.tuple([
        dimensionCurveConstraintOperandSchema,
        dimensionCurveConstraintOperandSchema,
      ]),
    ...
    z.object({
      ...
      label: z.string(),
      line: dimensionCurveConstraintOperandSchema,
      point: dimensionPointConstraintOperandSchema,
      value: positiveNumberSchema(
        "Line-point distance dimension must be positive.",
      ),
      annotationPlacement: dimensionLineAnnotationPlacementSchema.optional(),
    }),
    z.object({
      ...
      valueRadians: positiveNumberSchema(
        "Line angle dimension must be positive.",
      ),
...
const sketchStyleRecordSchema = z
  .object({
...
const sketchDerivationBaseSchema = {
  derivationId: z
    .string()
    .regex(/^sketch_derivation_.+$/, "Sketch derivation ID is invalid."),
  label: z.string(),
  seedEntityIds: z.array(sketchEntityIdSchema),
  outputs: z.array(sketchDerivedEntityOutputSchema),
} as const;

const sketchDerivationDefinitionSchema = z
  .discriminatedUnion("kind", [
    z.object({
...
const sketchAuthoringOperationSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    const editsOperationTarget =
      value.targets.edited?.some((target) => target.kind === "operation") ===
      true;
    ...
    if (!allowsOwnedState && value.ownedState !== undefined) {
      context.addIssue({
        ...
        path: ["ownedState"],
        message:
          "Only referenceImage creation rows and edit rows targeting operations may persist operation-owned state.",
      });
    }
  })
  .transform(({ createdGraph, removedGraph, ownedState, ...value }) => {
    const operation: Omit<
      SketchAuthoringOperation,
      "createdGraph" | "removedGraph" | "ownedState"
    > &
      Partial<
        Pick<
          SketchAuthoringOperation,
...
export const sketchDefinitionSchema = z
  .object({
...
const regionRecordSchema = z
  .object({
    regionId: regionIdSchema,
    label: z.string(),
    target: durableRefSchema,
    sourceSketch: z.object({
      kind: z.literal("sketch"),
      sketchId: sketchIdSchema,
    }),
  })
  .passthrough()
  .transform((value) => value as unknown as RegionRecord);
...
const solvedSketchEntityGeometrySchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    ...
    solvedRadius: positiveNumberSchema(
      "Solved circle radius must be positive.",
    ),
  }),
  z.object({
    entityId: sketchEntityIdSchema,
  ...
  z.object({
    ...
    minorRadius: positiveNumberSchema(
      "Solved ellipse minor radius must be positive.",
    ),
  }),
  z.object({
    entityId: sketchEntityIdSchema,
    ...
    endPosition: point2dSchema,
    minorRadius: positiveNumberSchema(
      "Solved elliptical arc minor radius must be positive.",
    ),
    sweepDirection: z.union([
      z.literal("clockwise"),
      z.literal("counterClockwise"),
  ...
  z.object({
    ...
    text: z
      .string()
      .trim()
      .min(1, "Solved profile text content must not be empty."),
    height: positiveNumberSchema(
      "Solved profile text height must be positive.",
    ),
    rotationRadians: z.number(),
    horizontalAlign: z.union([
      z.literal("left"),
...
export const sketchRecordSchema = z
  ...
  .passthrough()
  .transform((value) => value as unknown as SketchRecord);

export const solvedSketchSnapshotSchema = z
  .object({
    schemaVersion: solvedSketchSchemaVersionSchema,
    status: z.object({
      solveState: z.union([
        z.literal("notEvaluated"),
        z.literal("solved"),
        z.literal("partiallySolved"),
        z.literal("failed"),
      ]),
    ...
    constraintStatuses: z.array(
      z
        .object({
          constraintId: constraintIdSchema,
          status: z.union([
            z.literal("satisfied"),
            z.literal("unsatisfied"),
            z.literal("conflicting"),
          ]),
        })
        .passthrough(),
    ),
    dimensionStatuses: z.array(
      z
        .object({
          dimensionId: dimensionIdSchema,
          status: z.union([
  ...
  .passthrough()
  .transform((value) => value as unknown as SolvedSketchSnapshot);
