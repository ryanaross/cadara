...
function getEntityPointIds(entity: SketchEntityDefinition): readonly SketchPointId[] {
  switch (entity.kind) {
    case 'lineSegment':
...
function rotateAround(point: SketchPoint2D, origin: SketchPoint2D, angle: number, scale = 1): SketchPoint2D {
...
function reflectAcrossLine(point: SketchPoint2D, start: SketchPoint2D, end: SketchPoint2D): SketchPoint2D | null {
...
function getRelationshipTransform(
  ...
  switch (relationship.kind) {
    case 'mirror':
      return getMirrorTransform(relationship, entityById, pointById, diagnostics)
    case 'linearPattern':
      return (point) => [
    ...
    case 'circularPattern':
      return (point) => rotateAround(point, relationship.center, relationship.angleRadians * instanceIndex)
    case 'transform':
      return (point) => {
        const rotated = rotateAround(point, relationship.origin, relationship.rotationRadians, relationship.scale)
        return [
...
function transformedEntity(
  ...
  if (seed.kind === 'circle' && output.kind === 'circle') {
    return {
      ...output,
      radius: relationship.kind === 'transform' ? seed.radius * Math.abs(relationship.scale) : seed.radius,
    }
  }
  ...
  if (seed.kind === 'arc' && output.kind === 'arc') {
    return {
      ...output,
      sweepDirection: relationship.kind === 'mirror'
        ? seed.sweepDirection === 'clockwise' ? 'counterClockwise' : 'clockwise'
        : seed.sweepDirection,
    }
  }
...
export function evaluateSketchDerivations(definition: SketchDefinition): SketchDerivationEvaluationResult {
  ...
  const pointById = new Map(definition.points.map((point) => [point.pointId, point]))
  ...
  const replacePoint = (pointId: SketchPointId, position: SketchPoint2D) => {
    ...
    nextPoints = nextPoints.map((point) => point.pointId === pointId ? next : point)
  }

  const replaceEntity = (entity: SketchEntityDefinition) => {
    entityById.set(entity.entityId, entity)
    nextEntities = nextEntities.map((entry) => entry.entityId === entity.entityId ? entity : entry)
  }
  ...
  for (const relationship of relationships) {
    for (const output of relationship.outputs) {
      ...
      const seedPointIds = output.seedPointIds.length > 0 ? output.seedPointIds : getEntityPointIds(seed)
