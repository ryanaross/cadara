...
test('handleWorkbenchFailure keeps expected user-visible failures out of telemetry', () => {
  ...
  expectTrue(reporter.reports.length === 0, 'Expected workbench failures should not be reported.')
})
...
test('handleWorkbenchFailure reports classified defects separately from notification rendering', () => {
  ...
  expectTrue(notifications[0] === 'Open linked document failed.', 'Reportable failures may also show a user message.')
  expectTrue(
    reporter.reports[0]?.error === error
      && reporter.reports[0]?.metadata.source === 'workbench.file.openLinked'
      && reporter.reports[0]?.metadata.dedupeKey === 'workbench.file.openLinked:IndexedDB is unavailable.',
