...
export function clampWorkbenchSidebarWidth(
  width: number,
  containerWidth: number,
) {
  const boundedContainerWidth =
  ...
  const maxWidth = Math.max(
    ...
    Math.min(
      ...
      boundedContainerWidth - MIN_WORKBENCH_VIEWPORT_WIDTH,
    ),
  );
...
export function getWorkbenchSidebarWidthFromPointer(
  ...
  return clampWorkbenchSidebarWidth(
    pointerClientX - containerLeft,
    containerWidth,
  );
}
