...
import {
  createEditorDebugTraceRecorder,
} from '@/application/editor/editor-debug-trace'
...
import type { EditorEvent, EditorEffectRuntime } from '@/domain/editor/state-machine'
...
function waitForTraceEntries(entries: EditorRuntimeTraceEntry[], count: number): Promise<void> {
...
test('src/application/editor/editor-debug-trace.spec.ts keeps a bounded runtime trace ring buffer', () => {
  ...
  expectTrue(snapshot.maxEntries === 2, 'Trace recorder should report its configured capacity.')
  expectTrue(snapshot.totalEntries === 3, 'Trace recorder should report the total recorded entry count.')
  expectTrue(snapshot.droppedEntries === 1, 'Trace recorder should count entries dropped from the ring buffer.')
  expectTrue(snapshot.entries.length === 2, 'Trace recorder should keep only the bounded recent entries.')
  expectTrue(snapshot.entries[0]?.sequence === 2, 'Trace recorder should evict the oldest entry first.')
  expectTrue(snapshot.entries[1]?.sequence === 3, 'Trace recorder should retain the newest entry.')
})
...
test('src/application/editor/editor-debug-trace.spec.ts observes event, effect completion, and follow-up transition sequencing', async () => {
  ...
  expectTrue(dispatchedTrace?.kind === 'event-dispatched', 'Trace observers should see the initial session.started dispatch.')
  if (dispatchedTrace?.kind === 'event-dispatched') {
    expectTrue(dispatchedTrace.event.type === 'session.started', 'Trace observers should receive the dispatched event type.')
    expectTrue(dispatchedTrace.emittedEffects[0]?.type === 'document.fetchSnapshot', 'Dispatched events should report emitted effects.')
  }
  expectTrue(startedTrace?.kind === 'effect-started', 'Trace observers should see effect execution start.')
  expectTrue(completedTrace?.kind === 'effect-completed', 'Trace observers should see completed effects.')
  if (completedTrace?.kind === 'effect-completed') {
    expectTrue(completedTrace.completion.type === 'effect.snapshotLoaded', 'Completed effects should report the follow-up event type.')
    expectTrue(completedTrace.state.revisionId === snapshot.document.revisionId, 'Completed effects should report the accepted state summary.')
  }
...
test('src/application/editor/editor-debug-trace.spec.ts records failed effects without changing runtime ownership', async () => {
  ...
  expectTrue(failedTrace?.kind === 'effect-failed', 'Trace observers should receive failed effect entries.')
  if (failedTrace?.kind === 'effect-failed') {
    expectTrue(failedTrace.error.message === 'Trace this failure.', 'Failed effect entries should summarize the surfaced error.')
    expectTrue(failedTrace.failure.type === 'effect.snapshotFailed', 'Failed effect entries should report the synthesized failure event.')
  }
