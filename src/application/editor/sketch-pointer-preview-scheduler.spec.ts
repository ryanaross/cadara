...
test('src/application/editor/sketch-pointer-preview-scheduler.spec.ts coalesces pointer previews to the latest frame event', () => {
  ...
  expectTrue(dispatched.length === 0, 'Pointer preview moves should wait for the scheduled animation frame.')
  expectTrue(frameCallbacks.size === 1, 'Pointer preview moves should share one pending animation frame.')

  frameCallbacks.get(1)?.(0)

  expectTrue(dispatched.length === 1, 'One coalesced pointer preview should dispatch for the frame.')
  expectTrue(
...
test('src/application/editor/sketch-pointer-preview-scheduler.spec.ts flushes pending pointer preview before acceptance events', () => {
  ...
  expectTrue(cancelledFrames[0] === 7, 'Flushing should cancel the stale scheduled frame.')
  expectTrue(dispatched.length === 2, 'Flush should dispatch the pending pointer before the acceptance event.')
  expectTrue(dispatched[0]?.type === 'sketch.pointerMoved', 'The pending pointer preview should dispatch first.')
  expectTrue(dispatched[1]?.type === 'sketch.pointerReleased', 'The acceptance event should dispatch after the flush.')
})
