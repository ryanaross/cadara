...
import type { DocumentHistoryOrderEntry, ModelingDiagnostic } from '@/contracts/modeling/schema'
...
function createImportProvider(
  overrides: Partial<ImportProvider<{ units: 'mm' }, { enabled: boolean }, FeatureEditorFormSchema>> = {},
): ImportProvider<{ units: 'mm' }, { enabled: boolean }, FeatureEditorFormSchema> {
  return {
    id: 'step',
    label: 'STEP',
...
function createOwner(input: {
  ...
  importProvider?: ImportProvider<unknown, unknown, FeatureEditorFormSchema> | null
}) {
...
test('document owner preserves rejected and errored variable mutations without refreshing', async () => {
  const snapshot = await createSeedDocumentSnapshot()
  const dispatched: EditorEvent[] = []
  const rejectedDiagnostic = createDiagnostic('Variable expressions must stay valid.')
...
test('document owner routes rename operations for bodies, features, and sketches', async () => {
  ...
  const renameBodyCalls: Array<{ bodyId: string; bodyLabel: string; baseRevisionId: string }> = []
  const updateFeatureCalls: Array<{ featureId: string; featureLabel: string; definition: typeof feature.definition }> = []
  const commitSketchCalls: Array<{ sketchId: string; sketchLabel: string; solverCorrelation: ModelingCommitSketchCorrelation | null }> = []
...
test('document owner enforces rename and delete guardrails', async () => {
  ...
  const owner = createOwner({
    ...
    modelingService: {
      ...
      async deleteTarget() {
        throw new Error('Delete guardrails should stop before the modeling service.')
      },
    },
  })
    ...
    await owner.deleteTarget(
      {
        ...
        referenceId: 'ref_projected' as PrimitiveRef['kind'] extends never ? never : never,
  ...
  try {
    await owner.renameTarget(
      { kind: 'feature', featureId: 'feature_missing' as typeof snapshot.document.features[number]['featureId'] },
      'Missing feature',
      { operation: 'Rename feature', fallbackMessage: 'Rename feature failed.' },
    )
  } catch (error: unknown) {
  ...
  try {
    await owner.renameTarget(
      { kind: 'sketch', sketchId: 'sketch_missing' as typeof snapshot.document.sketches[number]['sketchId'] },
      'Missing sketch',
      { operation: 'Rename sketch', fallbackMessage: 'Rename sketch failed.' },
    )
  } catch (error: unknown) {
  ...
  try {
    await owner.renameTarget(
      { kind: 'edge', bodyId: 'body_part-1', edgeId: 'edge_outer-0' } as PrimitiveRef,
      'Unsupported rename',
...
test('document owner forwards history reorder, variable update, and durable delete mutations', async () => {
  const snapshot = await createSeedDocumentSnapshot()
  const nextSnapshot = await createSeedDocumentSnapshot()
  const historyItem: DocumentHistoryOrderEntry = { kind: 'sketch', sketchId: snapshot.document.sketches[0]!.sketchId }
  const deleteCalls: PrimitiveRef[] = []
  const variableUpdates: Array<{ variableId: string; name: string; valueText: string }> = []
  const reorderCalls: Array<{ item: DocumentHistoryOrderEntry; beforeItem: DocumentHistoryOrderEntry | null }> = []
  ...
  const variableId = ('variable_test' as typeof snapshot.document.variables[number]['variableId'])
  ...
  const reorderResult = await owner.reorderDocumentHistory(
    historyItem,
    null,
    { operation: 'Reorder history', fallbackMessage: 'Reorder history failed.' },
  )
...
test('document owner handles import provider lookup, diagnostic failures, and successful replay', async () => {
  ...
  const successfulOwner = createOwner({
    ...
    modelingService: {
      ...
      async createFeature() {
        throw new Error('No feature mutations were prepared for this import test.')
      },
      async commitSketch() {
        throw new Error('No sketch mutations were prepared for this import test.')
      },
    },
  })
  ...
  const diagnosticsOwner = createOwner({
    ...
    importProvider: createImportProvider({
      prepare: async () => ({
        diagnostics: [{ code: 'import-blocked', severity: 'error', message: 'Geometry could not be reconstructed.' }],
      }),
    }),
    modelingService: {
      currentDocumentId: snapshot.document.documentId,
      ...
      async addDocumentVariable() {
        throw new Error('Blocked imports should stop before replaying actions.')
      },
      async createFeature() {
        throw new Error('Blocked imports should stop before replaying actions.')
      },
