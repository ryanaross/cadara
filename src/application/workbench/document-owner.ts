...
export function createWorkbenchDocumentOwner({
	...
	>(result: AppResult<T>, options: AcceptedMutationOptions): Promise<AcceptedDocumentMutation<T>> {
		if (result.isErr()) {
	...
	async function deleteTarget(target: PrimitiveRef, options: AcceptedMutationOptions) {
		...
		switch (target.kind) {
			...
			case "sketch": {
				...
				const result = await modelingService.commitSketch({
					...
					solverCorrelation:
						modelingService.sketchSolver?.createCommitCorrelation(requestId) ?? null,
				});
				return acceptMutation(result, options);
			}
			default:
