...
function tab(documentId: DocumentId, title = documentId, storageKind: WorkbenchTab['storageKind'] = 'browser'): WorkbenchTab {
...
describe('reduceWorkbenchTabs', () => {
  describe('open', () => {
    ...
    it('activates the new tab when activate=true', () => {
      const state = createInitialWorkbenchTabsState(tab(docA))
      const next = reduceWorkbenchTabs(state, { type: 'open', tab: tab(docB), activate: true })
    ...
    it('updates an existing tab in place rather than duplicating', () => {
      ...
      const next = reduceWorkbenchTabs(state, { type: 'open', tab: tab(docA, 'New title') })
  ...
  describe('close', () => {
    it('refuses to close the only tab', () => {
      const state = createInitialWorkbenchTabsState(tab(docA))
      const next = reduceWorkbenchTabs(state, { type: 'close', documentId: docA })
    ...
    it('promotes the right-hand neighbor when closing the active tab', () => {
      ...
      const next = reduceWorkbenchTabs(seeded, { type: 'close', documentId: docA })
    ...
    it('falls back to the left-hand neighbor when closing the rightmost active tab', () => {
      ...
      const next = reduceWorkbenchTabs(seeded, { type: 'close', documentId: docC })
    ...
    it('keeps the active id when closing a non-active tab', () => {
      const seeded = reduceWorkbenchTabs(createInitialWorkbenchTabsState(tab(docA)), {
        type: 'open',
        tab: tab(docB),
      })

      const next = reduceWorkbenchTabs(seeded, { type: 'close', documentId: docB })
  ...
  describe('activate', () => {
    it('switches the active id when the target exists', () => {
      ...
      const next = reduceWorkbenchTabs(seeded, { type: 'activate', documentId: docB })
    ...
    it('is a no-op when the target is not in the tab list', () => {
      const state = createInitialWorkbenchTabsState(tab(docA))
      const next = reduceWorkbenchTabs(state, { type: 'activate', documentId: docB })
  ...
  describe('reorder', () => {
    it('moves the tab to the requested index', () => {
      ...
      const next = reduceWorkbenchTabs(seeded, { type: 'reorder', documentId: docC, toIndex: 0 })

      expect(next.tabs.map((entry) => entry.documentId)).toEqual([docC, docA, docB])
    })
    ...
    it('clamps an out-of-bounds index instead of throwing', () => {
      ...
      const next = reduceWorkbenchTabs(seeded, { type: 'reorder', documentId: docA, toIndex: 999 })
  ...
  describe('rename / updateStorage', () => {
    it('renames a tab', () => {
      ...
      const next = reduceWorkbenchTabs(state, { type: 'rename', documentId: docA, title: 'Bracket v3' })
...
describe('reconcileWorkbenchTabsForActiveDocument', () => {
  ...
  it('keeps the persisted order and activates the loaded document when present', () => {
    const persisted = { tabs: [tab(docA), tab(docB), tab(docC)], activeDocumentId: docC }
    const next = reconcileWorkbenchTabsForActiveDocument(persisted, tab(docB))

    expect(next.tabs.map((entry) => entry.documentId)).toEqual([docA, docB, docC])
    expect(next.activeDocumentId).toBe(docB)
  })
  ...
  it('prepends the loaded document if persistence drifted', () => {
    ...
    expect(next.tabs.map((entry) => entry.documentId)).toEqual([docB, docA, docC])
    expect(next.activeDocumentId).toBe(docB)
  })
})
...
describe('workbenchTabsStorageDescriptor', () => {
  it('describes the storage backend for tooltip copy', () => {
    expect(workbenchTabsStorageDescriptor('browser', null)).toBe('Stored in this browser only')
    expect(workbenchTabsStorageDescriptor('filesystem', 'bracket.cadara')).toBe('Synced to bracket.cadara')
    expect(workbenchTabsStorageDescriptor('filesystem', null)).toBe('Synced to a local file')
    expect(workbenchTabsStorageDescriptor('cloud', 'Drive')).toBe('Cloud-synced — Drive')
