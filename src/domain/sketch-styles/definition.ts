...
export type SketchStylePatch =
  ...
  | {
      intent: typeof SKETCH_STYLE_PATCH_INTENT;
      field: "strokeCap";
      value: SketchStrokeCap;
    }
  | {
      intent: typeof SKETCH_STYLE_PATCH_INTENT;
      field: "strokeJoin";
      value: SketchStrokeJoin;
    }
  | {
      intent: typeof SKETCH_STYLE_PATCH_INTENT;
...
export type SketchStyleToolId = Extract<ToolId, "fill" | "stroke">;
...
export function buildSketchStyleControls(
  style: SketchStyleDefinition | undefined,
): readonly SketchToolControlDescriptor[] {
  ...
  return [
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "fillMode" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "fillColor" },
      },
    },
    {
    ...
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "gradientEndColor" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeEnabled" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeColor" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeWidth" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeCap" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeJoin" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeMiterLimit" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeDashSize" },
      },
    },
    {
      ...
      action: {
        type: "patch",
        patch: { intent: SKETCH_STYLE_PATCH_INTENT, field: "strokeGapSize" },
      },
    },
  ];
}
...
export function isSketchStyleToolId(
  toolId: ToolId,
): toolId is SketchStyleToolId {
...
export function buildSketchStylePresentation(
  ...
  if (!focus.target) {
    ...
    return {
      prompts: [
        {
          id: "sketch-style-target-prompt",
          text: `${definition.label}: ${isFill ? "select an enclosed region" : "select a sketch edge"}`,
        },
      ],
      selectionGuide: {
        id: "sketch-style-target-guide",
        label: isFill ? "Select an enclosed region" : "Select a sketch edge",
        acceptedKinds: isFill
          ? ["region"]
  ...
  const controls = buildSketchStyleControls(style).filter((control) =>
    definition.controlIds.includes(control.id),
  );
...
export function parseSketchStylePatch(
  patch: Record<string, unknown>,
): SketchStylePatch | null {
  if (
    patch.intent !== SKETCH_STYLE_PATCH_INTENT ||
...
function isSketchStylePatchField(
  field: string,
): field is SketchStylePatchField {
  return (
    field === "fillMode" ||
    field === "fillColor" ||
    field === "gradientStartColor" ||
...
function getSketchStyleFocusDefinition(
  toolId: SketchStyleToolId,
): SketchStyleFocusDefinition {
  switch (toolId) {
    case "fill":
      return {
        label: "Fill",
