...
export const referenceImageCalibrationModeDefinition = {
  ...
  resolveOpenRequest: ({ sketchSession, target }) =>
    ...
    collectActiveReferenceImageOperations(sketchSession.definition).some(
      ({ operation }) => operation.operationId === target.operationId,
    )
  ...
  enter: ({ sketchSession, operationTarget }) => {
    const activeOperation = collectActiveReferenceImageOperations(
      sketchSession.definition,
    ).find(
      ({ operation }) => operation.operationId === operationTarget.operationId,
    );

    if (!activeOperation) {
      throw new Error(
        `Reference-image operation ${operationTarget.operationId} is not available for calibration.`,
      );
    }
  ...
  buildPanel: ({ activeMode }) =>
    buildPanel(activeMode.state as ReferenceImageCalibrationModeState),
  buildViewport: ({ activeMode }) =>
    buildViewport(activeMode.state as ReferenceImageCalibrationModeState),
  ...
  handleEffectResult: ({ activeMode, effectId, payload }) => {
    ...
    if (
      effectId !== "replace-image" ||
      !isReferenceImagePayload(payload.image)
    ) {
      return { effect: null };
    }
  ...
  commit: ({ sketchSession, activeMode }) => {
    ...
    const createdPoints = state.draftPoints.filter(
      (point) => !existingPointIds.has(point.pointId),
    );
    const createdEntities = createdPoints.flatMap((point) => {
      const entity = createDraftPointEntity(
        state.sketchId,
        state.operationId,
        point,
      );
  ...
  getOperationOwnedStateOverride: ({ activeMode }) => ({
    operationId: (activeMode.state as ReferenceImageCalibrationModeState)
      .operationId,
    state: (activeMode.state as ReferenceImageCalibrationModeState).draftState,
    label: (activeMode.state as ReferenceImageCalibrationModeState).draftState
      .image.fileName,
  }),
...
function getCalibrationState(
  ...
  if (!calibration) {
    throw new Error(
      "Reference-image calibration state must be solved before entering calibration mode.",
    );
  }
...
function buildPanel(
  ...
  const solveFields: SketchSpecialModePanelField[] = [
    ...
    },
    {
      ...
      action: {
        kind: "patch",
        patch: { field: "showExportedAnchorsInSketch" },
      },
    },
    {
      id: "anchor-count",
  ...
  const anchorFields: SketchSpecialModePanelField[] = [
    {
      ...
      helper:
        calibration.anchors.length > 0
          ? `${calibration.anchors.length} bound anchors available.`
          : "No anchors yet.",
  ...
  const anchorButtons: SketchSpecialModePanelButton[] = [
    ...
    },
    {
      id: "rebind-anchor",
      label:
        state.pendingAnchorPlacement && state.selectedAnchorId
          ? "Picking Binding"
          : "Rebind Anchor",
  ...
  const sections: SketchSpecialModePanelSection[] = [
    {
      ...
      title: "Placement",
      description:
        "Reference-image placement is recovered from solved sketch point bindings after ordinary sketch solving.",
      fields: solveFields,
    },
    {
      id: "anchors",
      ...
      fields: [
        {
          ...
          label: "Payload",
          value:
            state.draftState.image.fileName ??
            `${state.draftState.image.pixelWidth}×${state.draftState.image.pixelHeight}`,
        },
      ],
      buttons: [
        {
    ...
    footerButtons: [
      {
        id: "cancel",
        label: "Cancel",
        action: { kind: "command", command: "cancel" },
      },
      {
        id: "commit",
        label: "Commit",
        tone: "primary",
        action: { kind: "command", command: "commit" },
      },
    ],
  };
}
...
function buildViewport(
  ...
  const solvedByAnchorId = new Map(
    calibration.solveResult.anchors.map(
      (anchor) => [anchor.anchorId, anchor.worldPosition] as const,
    ),
  );
        ...
        ? [
            {
              ...
              handle: createSketchSpecialModeHandleRef(
                state.operationId,
                anchor.anchorId,
              ),
              tone:
                state.selectedAnchorId === anchor.anchorId
                  ? ("success" as const)
                  : ("neutral" as const),
              draggable: false,
            },
  ...
  return {
    ...
    diagnostics: calibration.solveResult.diagnostics.map(
      (diagnostic, index) => ({
        id: `${diagnostic.code}:${index}`,
        message: diagnostic.message,
        severity: diagnostic.severity,
...
function handleClick(
  ...
  if (state.pendingAnchorPlacement) {
    if (state.selectedAnchorId && target?.kind === "sketchPoint") {
      const boundPoint = definitionPoints.find(
        (candidate) => candidate.pointId === target.pointId,
      );
      return boundPoint
        ? {
            state: solveModeState({
              ...
              draftState: {
                ...
                calibration: {
                  ...
                  anchors: getCalibrationState(state.draftState).anchors.map(
                    (anchor) =>
    ...
    if (
      !uv ||
      target?.kind !== "sketchOperation" ||
      target.operationId !== state.operationId
    ) {
      return { state };
    }
    ...
    if (state.selectedAnchorId) {
      ...
      return {
        state: solveModeState({
          ...state,
          draftPoints: upsertDraftPoint(
            removeUnusedDraftPoint(state),
            nextPoint,
          ),
          draftState: {
            ...state.draftState,
    ...
    const nextAnchorOrdinal = getNextAnchorOrdinal(
      state.operationId,
      calibration.anchors,
    );
...
function patchDraftState(
  ...
  if (
    field === "scaleMode" &&
    (value === "lockedAspect" || value === "independent")
  ) {
  ...
  if (field === "selectedAnchorId") {
    return {
      ...
      selectedAnchorId:
        typeof value === "string" &&
        calibration.anchors.some((anchor) => anchor.anchorId === value)
          ? value
          : null,
      pendingAnchorPlacement: false,
    };
...
function removeSelectedAnchor(state: ReferenceImageCalibrationModeState) {
  ...
  const calibration = getCalibrationState(state.draftState);
  const anchorToRemove = calibration.anchors.find(
    (anchor) => anchor.anchorId === state.selectedAnchorId,
  );
  if (!anchorToRemove) {
    return state;
  }

  const nextAnchors = calibration.anchors.filter(
    (anchor) => anchor.anchorId !== state.selectedAnchorId,
  );
  const nextDraftPoints = state.draftPoints.filter(
    (point) =>
...
function getSelectedAnchor(state: ReferenceImageCalibrationModeState) {
  return (
    getCalibrationState(state.draftState).anchors.find(
      (anchor) => anchor.anchorId === state.selectedAnchorId,
    ) ?? null
  );
}
...
function collectDraftPoints(
  ...
  const pointLookup = new Map(
    definitionPoints.map((point) => [point.pointId, point] as const),
  );
...
function upsertDraftPoint(
  ...
  const nextPoints = draftPoints.filter(
...
function removeUnusedDraftPoint(state: ReferenceImageCalibrationModeState) {
  ...
  const remainingAnchors = getCalibrationState(state.draftState).anchors.filter(
    (anchor) =>
      anchor.anchorId !== selectedAnchor.anchorId &&
      anchor.pointId === selectedAnchor.pointId,
  );
  return remainingAnchors.length > 0
    ? state.draftPoints
    : state.draftPoints.filter(
        (point) => point.pointId !== selectedAnchor.pointId,
      );
}
...
function createDraftPointEntity(
  ...
  const entityId =
    `sketch_entity_${operationId.replace(/[^a-zA-Z0-9]+/g, "_")}_${point.pointId.replace(/[^a-zA-Z0-9]+/g, "_")}_point` as SketchEntityId;
  return {
    kind: "point",
    entityId,
    label: point.label,
...
function findNearestAnchor(
  ...
  const solvedByAnchorId = new Map(
    getCalibrationState(state.draftState).solveResult.anchors.map(
      (anchor) => [anchor.anchorId, anchor.worldPosition] as const,
    ),
  );
...
function getNextAnchorOrdinal(
  ...
  return (
    anchors.reduce((maxOrdinal, anchor) => {
