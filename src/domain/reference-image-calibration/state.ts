...
export function solveReferenceImageOperationState(
  ...
  const calibration =
    state.calibration ?? createDefaultReferenceImageCalibrationState();
  const diagnostics = collectCalibrationDiagnostics(
    calibration,
    input?.pointPositionsById,
  );
  ...
  const combinedDiagnostics = [...diagnostics, ...solveResult.diagnostics];
    ...
    calibration: {
      ...
      solveResult: {
        ...
        anchors: calibration.anchors.flatMap((anchor) => {
          const solvedAnchor = solveResult.anchors.find(
...
export function replaceReferenceImagePayloadPreservingCalibration(input: {
  ...
  const calibration =
    input.state.calibration ?? createDefaultReferenceImageCalibrationState();
...
function clampUv(uv: SketchPoint2D): SketchPoint2D {
  return [Math.min(1, Math.max(0, uv[0])), Math.min(1, Math.max(0, uv[1]))];
}
