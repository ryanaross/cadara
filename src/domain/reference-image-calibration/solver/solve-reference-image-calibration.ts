...
export function solveReferenceImageCalibration(
  ...
  const targets = input.anchors.filter(
    (anchor) => anchor.worldPosition !== null,
  );
  const anchorMap = new Map(
    input.anchors.map((anchor) => [anchor.anchorId, anchor] as const),
  );
  ...
  if (targets.length === 0) {
    diagnostics.push({
      ...
      severity: "warning",
      message:
        "Add at least one anchored calibration point to solve the reference image.",
    });
  }
  ...
  const solvedAnchors: ReferenceImageCalibrationSolvedAnchor[] =
  ...
  if (hasDegeneratePlacement(placement)) {
    diagnostics.push({
      ...
      severity: "warning",
      message:
        "Reference-image calibration collapsed to an invalid size; add stronger anchors or relax conflicting constraints.",
    });
  }

  const underconstrainedMessage = getUnderconstrainedCalibrationMessage(
    input,
    placement,
    aspectRatio,
    anchorMap,
  ...
  for (const anchor of input.anchors) {
    ...
    const solved = solvedAnchors.find(
  ...
  for (const constraint of input.constraints) {
    const first = solvedAnchors.find(
      (anchor) => anchor.anchorId === constraint.firstAnchorId,
    )?.worldPosition;
    const second = solvedAnchors.find(
      (anchor) => anchor.anchorId === constraint.secondAnchorId,
    )?.worldPosition;
    if (!first || !second) {
      diagnostics.push({
    ...
    const delta = Math.abs(
      distanceBetween(first, second) - constraint.distance,
    );
...
function findBestPlacement(
  ...
  aspectRatio: number,
  anchorMap: ReadonlyMap<
    string,
    ReferenceImageCalibrationSolverInput["anchors"][number]
  >,
) {
  const candidates = buildInitialPlacementCandidates(
    input,
    targets,
    aspectRatio,
  );
...
function buildInitialPlacementCandidates(
  ...
  const leastSquaresPlacement = estimateLeastSquaresPlacement(
    input,
    targets,
    aspectRatio,
  );
  ...
  if (input.scaleMode === "lockedAspect") {
    const pairwisePlacement = estimateLockedAspectPairPlacement(
      targets,
      aspectRatio,
    );
...
function estimateAxisAlignedPlacement(
  ...
  const width = Math.max(
    (Math.max(...xTargets) - Math.min(...xTargets)) / uSpan,
    EPSILON,
  );
  const height = Math.max(
    (Math.max(...yTargets) - Math.min(...yTargets)) / vSpan,
    EPSILON,
  );
  ...
  if (input.scaleMode === "lockedAspect") {
    const lockedHeight = Math.max(
      height,
      width / Math.max(aspectRatio, EPSILON),
    );
    return {
...
function estimateLeastSquaresPlacement(
  ...
  const coefficients = solveLeastSquaresSystem(
    targets.map((anchor) => ({
      uv: anchor.uv,
  ...
  if (input.scaleMode === "lockedAspect") {
    const scale = Math.max(
      (width / Math.max(aspectRatio, EPSILON) + height) * 0.5,
      EPSILON,
    );
    return {
...
function estimateLockedAspectPairPlacement(
  ...
  const rotationRadians = normalizeAngle(
    Math.atan2(worldDelta[1], worldDelta[0]) -
      Math.atan2(
        first.uv[1] - second.uv[1],
        (second.uv[0] - first.uv[0]) * aspectRatio,
      ),
...
function estimatePlacementCenter(
  ...
  return [
    estimatedCenters.reduce((sum, center) => sum + center[0], 0) /
      estimatedCenters.length,
    estimatedCenters.reduce((sum, center) => sum + center[1], 0) /
      estimatedCenters.length,
  ];
}
...
function evaluateLoss(
  ...
  for (const constraint of input.constraints) {
    ...
    const distanceError =
      distanceBetween(firstPosition, secondPosition) - constraint.distance;
  ...
  const regularization = [
    ...
    shortestAngleDifference(
      ...
      input.initialPlacement.rotationRadians,
    ),
...
function minimizeLoss(
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration += 1) {
    ...
    const candidate = current.map(
      (value, index) => value - gradient[index]! * learningRate,
    );
...
function independentVectorToPlacement(
  vector: readonly number[],
): ReferenceImagePlacement {
  return {
    center: [vector[0] ?? 0, vector[1] ?? 0],
...
function getUnderconstrainedCalibrationMessage(
  ...
  const residualCount =
    input.anchors.filter((anchor) => anchor.worldPosition !== null).length * 2 +
    input.constraints.filter(
      (constraint) =>
        anchorMap.has(constraint.firstAnchorId) &&
        anchorMap.has(constraint.secondAnchorId),
    ).length;
  if (residualCount === 0) {
  ...
  const columns = vector.map((value, index) => {
    ...
    const previousResiduals = buildResidualVector(
      input,
      previousPlacement,
      anchorMap,
    );
    return nextResiduals.map(
      (nextValue, residualIndex) =>
  ...
  if (input.scaleMode === "independent") {
    ...
    const spansU =
      uValues.length > 1 &&
      Math.max(...uValues) - Math.min(...uValues) > EPSILON;
    const spansV =
      vValues.length > 1 &&
      Math.max(...vValues) - Math.min(...vValues) > EPSILON;
    if (!spansU || !spansV) {
...
function buildResidualVector(
  ...
  for (const anchor of input.anchors) {
    ...
    const solved = mapAnchorToWorld(anchor.uv, placement);
    residuals.push(
      solved[0] - anchor.worldPosition[0],
      solved[1] - anchor.worldPosition[1],
    );
  }
  ...
  for (const column of columns) {
    ...
    for (const basisVector of basis) {
      ...
      for (let index = 0; index < orthogonalized.length; index += 1) {
        orthogonalized[index] =
          (orthogonalized[index] ?? 0) - projection * (basisVector[index] ?? 0);
      }
    }
...
function stabilizePlacement(
  ...
  return changed
    ? {
        ...
        center:
          targets.length > 0
            ? estimatePlacementCenter(targets, stabilized)
            : stabilized.center,
      }
    : stabilized;
}
...
function solveLeastSquaresSystem(
  ...
  for (const target of targets) {
    ...
    for (let rowIndex = 0; rowIndex < row.length; rowIndex += 1) {
      rightX[rowIndex] =
        (rightX[rowIndex] ?? 0) + row[rowIndex]! * target.worldPosition[0];
      rightY[rowIndex] =
        (rightY[rowIndex] ?? 0) + row[rowIndex]! * target.worldPosition[1];
      for (let columnIndex = 0; columnIndex < row.length; columnIndex += 1) {
        normal[rowIndex]![columnIndex] =
          (normal[rowIndex]![columnIndex] ?? 0) +
...
function solveLinearSystem(
  ...
  for (let pivotIndex = 0; pivotIndex < augmented.length; pivotIndex += 1) {
    ...
    for (
      let rowIndex = pivotIndex + 1;
      rowIndex < augmented.length;
      rowIndex += 1
    ) {
    ...
    for (
      ...
      columnIndex < augmented[pivotIndex]!.length;
      columnIndex += 1
    ) {
      augmented[pivotIndex]![columnIndex] =
    ...
    for (let rowIndex = 0; rowIndex < augmented.length; rowIndex += 1) {
      ...
      for (
        let columnIndex = pivotIndex;
        columnIndex < augmented[rowIndex]!.length;
        columnIndex += 1
      ) {
...
function averageAngles(angles: readonly number[]) {
  const vector = angles.reduce(
    (sum, angle) =>
      [sum[0] + Math.cos(angle), sum[1] + Math.sin(angle)] as SketchPoint2D,
...
function dot(first: readonly number[], second: readonly number[]) {
  return first.reduce(
    (sum, value, index) => sum + value * (second[index] ?? 0),
    0,
  );
}
