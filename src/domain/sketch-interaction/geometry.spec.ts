...
import type { SketchDefinition, SketchPoint2D } from '@/contracts/sketch/schema'
import type { ProjectedGeometryId, ReferenceId, SketchEntityId, SketchId, SketchPointId } from '@/contracts/shared/ids'
...
test('collectSketchInteractionGeometry preserves local, projected, datum, and advanced sketch targets', () => {
  ...
  const point = (suffix: string, position: SketchPoint2D) =>
    createPointDefinition(sketchId, `sketch_point_${suffix}` as SketchPointId, suffix, position)
  const points = [
    point('line_a', [-3, 0]),
    point('line_b', [-1, 0]),
    point('center', [0, 0]),
  ...
  const entities = [
    createLineEntityDefinition(sketchId, 'sketch_entity_line' as SketchEntityId, 'Line', points[0]!.pointId, points[1]!.pointId),
    createCircleEntityDefinition(sketchId, 'sketch_entity_circle' as SketchEntityId, 'Circle', points[2]!.pointId, 1.5),
    createArcEntityDefinition(sketchId, 'sketch_entity_arc' as SketchEntityId, 'Arc', points[2]!.pointId, points[3]!.pointId, points[4]!.pointId, 'counterClockwise'),
    createSplineEntityDefinition(sketchId, 'sketch_entity_spline' as SketchEntityId, 'Spline', [points[5]!.pointId, points[6]!.pointId, points[7]!.pointId]),
    createEllipseEntityDefinition(sketchId, 'sketch_entity_ellipse' as SketchEntityId, 'Ellipse', points[8]!.pointId, points[9]!.pointId, 1),
    createEllipticalArcEntityDefinition(sketchId, 'sketch_entity_elliptical_arc' as SketchEntityId, 'Elliptical arc', points[10]!.pointId, points[11]!.pointId, points[12]!.pointId, points[13]!.pointId, 1, 'counterClockwise'),
    createConicEntityDefinition(sketchId, 'sketch_entity_conic' as SketchEntityId, 'Conic', points[14]!.pointId, points[15]!.pointId, points[16]!.pointId, 0.5),
    createBezierCurveEntityDefinition(sketchId, 'sketch_entity_bezier' as SketchEntityId, 'Bezier', [points[17]!.pointId, points[18]!.pointId, points[19]!.pointId, points[20]!.pointId], 3),
    createProfileTextEntityDefinition(sketchId, 'sketch_entity_text' as SketchEntityId, 'Text', points[21]!.pointId, 'CAD', 1, 0, 'left', 'baseline'),
  ]
  ...
  expectTrue(byKind.has('point'), 'Local point definitions and datum origin should be available as semantic interaction points.')
  expectTrue(byKind.has('lineSegment'), 'Local lines, projected lines, and datum axes should be available as semantic line segments.')
  expectTrue(byKind.has('circle'), 'Local and projected circles should be available as semantic circles.')
  expectTrue(byKind.has('arc'), 'Local and projected arcs should be available as semantic arcs.')
  expectTrue(byKind.has('spline'), 'Local and projected splines should be available as semantic splines.')
  expectTrue(byKind.has('sampledCurve'), 'Advanced local curves should be available as sampled semantic curves.')
  expectTrue(
...
test('flattenSketchInteractionCurve respects arc sweeps and closes closed curves', () => {
  ...
  const arc = createArcEntityDefinition(sketchId, 'sketch_entity_arc' as SketchEntityId, 'Arc', center.pointId, start.pointId, end.pointId, 'counterClockwise')
  const circle = createCircleEntityDefinition(sketchId, 'sketch_entity_circle' as SketchEntityId, 'Circle', center.pointId, 2)
  const missingLine = createLineEntityDefinition(sketchId, 'sketch_entity_missing_line' as SketchEntityId, 'Missing line', start.pointId, 'sketch_point_missing' as SketchPointId)
  const definition: SketchDefinition = {
    ...createNewSketchSession(createStandardPlaneDefinition('xy')).definition,
    pointIds: points.map((entry) => entry.pointId),
    points,
  ...
  const circleGeometry = geometry.find((entry) => entry.target === circle.target)

  expectTrue(arcGeometry !== undefined && isSketchInteractionCurveGeometry(arcGeometry), 'The authored arc should produce curve geometry.')
  expectTrue(circleGeometry !== undefined && isSketchInteractionCurveGeometry(circleGeometry), 'The authored circle should produce curve geometry.')
...
function nearestDistance(points: readonly SketchPoint2D[], target: SketchPoint2D) {
  return points.reduce(
    (nearest, point) => Math.min(nearest, Math.hypot(point[0] - target[0], point[1] - target[1])),
