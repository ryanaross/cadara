...
export type PreviewBodyRelationshipKind = 'intersects' | 'coplanarContact' | 'none' | 'unknown'
...
export function classifyPreviewBooleanTarget(input: {
  ...
  settings?: Pick<ModelingDocumentSettings, 'modelingTolerance' | 'angularToleranceRadians'> | null
  ...
  const previewMeshes = input.previewRenderables.filter((record) => record.geometry.kind === 'mesh')
  const bodyMeshesByBodyId = groupBodyMeshes(input.bodyRenderables, candidateBodyIds)
  ...
  const intersectingBodyId = selectBestCandidate(intersectingCandidates, tolerance)
  ...
  const previewPlanes = previewMeshes.flatMap((record) => createPlaneSample(record, null))
  const coplanarCandidates: CandidateScore[] = []
  for (const bodyId of candidateBodyIds) {
    const score = scoreCoplanarContact(
      previewPlanes,
      (bodyMeshesByBodyId.get(bodyId) ?? []).flatMap((record) => createPlaneSample(record, bodyId)),
      tolerance,
      angularTolerance,
    )
    if (score !== null) {
...
export function isBooleanOperationPatch(patch: FeatureDraftPatch) {
  return Object.hasOwn(patch, 'operation') || Object.hasOwn(patch, 'operationIntent')
}

export function isBooleanTargetPatch(patch: FeatureDraftPatch) {
  return Object.hasOwn(patch, 'booleanScope')
    || Object.hasOwn(patch, 'booleanTargetBodyId')
    || Object.hasOwn(patch, 'booleanTargetBodyIds')
    || Object.hasOwn(patch, 'targetBodyTargets')
}

export function featureSupportsBooleanTargetPreselection(featureType: FeatureEditSessionState['featureType']) {
...
function createPreselectionPatch(
  ...
  if (featureType === 'sweep' || featureType === 'loft' || featureType === 'thicken') {
    const operationIntent = advancedOperationIntentForRelationship(relationship)
    return operationIntent === 'create' || !relationship.bodyId
      ? { operationIntent, targetBodyTargets: [] }
      : { operationIntent, targetBodyTargets: [{ kind: 'body', bodyId: relationship.bodyId }] }
  }

  return null
}

function basicOperationForRelationship(relationship: PreviewBodyRelationship): FeatureBooleanOperation {
...
function groupBodyMeshes(
  ...
  for (const record of renderables) {
    if (record.geometry.kind !== 'mesh' || !record.ownerBodyId || !candidateBodyIdSet.has(record.ownerBodyId)) {
      continue
    }
...
function scoreCoplanarContact(
  ...
  for (const preview of previewPlanes) {
    for (const body of bodyPlanes) {
      ...
      if (Math.abs(distanceToPlane(preview.point, body.point, body.normal)) > tolerance) {
        continue
      }

      const overlap = bounds2Overlap(preview.projectedBounds, body.projectedBounds)
...
function createPlaneSample(record: RenderableEntityRecord, bodyId: BodyId | null): PlaneSample[] {
  if (record.geometry.kind !== 'mesh' || record.binding.semanticClass !== 'planarFace') {
    return []
  }
  ...
  const normal = findMeshNormal(record.geometry.vertexPositions, record.geometry.triangleIndices)
  if (!normal) {
    return []
  }
...
function boundsFromRenderable(record: RenderableEntityRecord): Bounds3 | null {
  if (record.geometry.kind !== 'mesh' || record.geometry.vertexPositions.length === 0) {
    return null
  }
...
function projectBounds(bounds: Bounds3, normal: [number, number, number]): Bounds2 {
  const dominantAxis = dominantNormalAxis(normal)
  if (dominantAxis === 0) {
    return { min: [bounds.min[1], bounds.min[2]], max: [bounds.max[1], bounds.max[2]] }
  }
  if (dominantAxis === 1) {
...
function dominantNormalAxis(normal: [number, number, number]) {
  const absolute = normal.map((value) => Math.abs(value)) as [number, number, number]
...
function selectBestCandidate(candidates: readonly CandidateScore[], tolerance: number): BodyId | 'ambiguous' | null {
  if (candidates.length === 0) {
    return null
  }

  const ranked = [...candidates].sort((left, right) => right.score - left.score || left.bodyId.localeCompare(right.bodyId))
  const best = ranked[0]!
  const second = ranked[1]
  if (second && Math.abs(best.score - second.score) <= Math.max(tolerance, Math.abs(best.score) * 1e-6)) {
    return 'ambiguous'
  }
...
function subtract(left: RenderPoint3D, right: RenderPoint3D): [number, number, number] {
