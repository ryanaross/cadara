...
test('src/domain/editor/sketch-session-history.spec.ts', () => {  function addLine(
  ...
  expectTrue(fullItems.length === 2, 'Sketch history should include one row per authored operation.')
  expectTrue(fullItems.every((item) => item.kind === 'operation'), 'Sketch history should render operation rows only.')
  expectTrue(getSketchHistoryCursorIndex(fullItems, session.historyCursor) === 1, 'Sketch cursor should advance to the newest operation.')
  expectTrue(
  ...
  expectTrue(getNextSketchHistoryCursor(session) === null, 'Next sketch cursor should be unavailable at the tail.')

  const rolledBack = moveSketchHistoryCursor(session, getSketchHistoryCursorForIndex(fullItems, 0))
  expectTrue(rolledBack.definition.entityIds.length === 1, 'Rolling back should filter displayed sketch entities after the cursor.')
  expectTrue(session.fullDefinition.entityIds.length === 2, 'Rolling back must not mutate the prior full draft definition.')
  expectTrue(
  ...
  expectTrue(getPreviousSketchHistoryCursor(beforeFirst) === null, 'Undo should be unavailable before the first sketch item.')
  expectTrue(
  ...
  expectTrue(inserted.fullDefinition.entityIds.length === 2, 'Inserting after a rolled-back cursor should replace after-cursor sketch items.')
  expectTrue(inserted.definition.entityIds.length === 2, 'Displayed sketch definition should include the inserted tail item.')
  expectTrue(
  ...
  expectTrue(cursorRepair.fullDefinition.authoringOperations?.length === 1, 'Deleting a history row should remove the targeted authored operation.')
  expectTrue(
  ...
  expectTrue(singleRowId, 'Single-row history delete fixture should create one authored operation.')
  const emptied = deleteSketchHistoryOperation(singleRowWithLine, singleRowId)
  expectTrue(emptied.historyCursor.kind === 'empty', 'Deleting the last history row should repair the cursor to the empty position.')
  expectTrue(emptied.definition.entityIds.length === 0, 'Deleting the final history row should clear the rebuilt sketch graph.')
  ...
  expectTrue(deletedEntityId, 'Delete-row fixture should expose one authored entity.')
  ...
  expectTrue(deleteRowId, 'Live deletion should append a durable delete row before it can be removed from history.')
  ...
  expectTrue(restored.definition.entityIds.includes(deletedEntityId), 'Deleting a delete row from history should restore the geometry it had removed.')
})
