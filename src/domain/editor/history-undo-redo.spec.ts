...
test('src/contracts/editor/history-undo-redo.spec.ts', async () => {  function addLine(
  ...
  async function createLoadedIdleState() {
    ...
    expectTrue(fetchEffect?.type === 'document.fetchSnapshot', 'Session start should request a snapshot.')
  ...
  async function testSketchUndoRedo() {
    ...
    expectTrue(getEditorHistoryAvailability(state).canUndo, 'Sketch undo should be available after authoring two items.')
    expectTrue(!getEditorHistoryAvailability(state).canRedo, 'Sketch redo should be unavailable at the history tail.')

    const undone = transitionEditorState(state, { type: 'history.undoRequested' })
    expectTrue(undone.effects.length === 0, 'Sketch undo should not emit document cursor effects.')
    expectTrue(undone.state.kind === 'editingSketch', 'Sketch undo should keep the sketch session active.')
    expectTrue(undone.state.session.definition.entityIds.length === 1, 'Sketch undo should hide after-cursor geometry.')
    expectTrue(getEditorHistoryAvailability(undone.state).canRedo, 'Sketch redo should become available after undo.')

    const redone = transitionEditorState(undone.state, { type: 'tool.activated', toolId: 'redo' })
    expectTrue(redone.effects.length === 0, 'Sketch redo should not emit document cursor effects.')
    expectTrue(redone.state.kind === 'editingSketch', 'Sketch redo should keep the sketch session active.')
    expectTrue(redone.state.session.definition.entityIds.length === 2, 'Sketch redo should restore visible geometry through the cursor.')
  }
  ...
  function testSketchGeometryDeletionUndoRestoresDependentConstraints() {
    ...
    expectTrue(dependentConstraintId, 'Deletion undo fixture should create a dependent constraint.')
    ...
    expectTrue(deleted.state.kind === 'editingSketch', 'Geometry deletion should keep the sketch session active.')
    expectTrue(!deleted.state.session.definition.entityIds.includes(entityId), 'Geometry deletion should remove the selected entity.')
    expectTrue(
    ...
    expectTrue(deleted.state.selection.length === 0, 'Geometry deletion should clear selection.')
    expectTrue(deleted.state.hoverTarget === null, 'Geometry deletion should clear hover state.')

    const undone = transitionEditorState(deleted.state, { type: 'tool.activated', toolId: 'undo' })
    expectTrue(undone.effects.length === 0, 'Sketch deletion undo should remain sketch-local.')
    expectTrue(undone.state.kind === 'editingSketch', 'Sketch deletion undo should keep the sketch session active.')
    expectTrue(
  ...
  async function testIdleDocumentHistoryAvailabilityAndCursorRequest() {
    ...
    expectTrue(previousCursor, 'Loaded document fixture should have a previous document cursor.')

    expectTrue(getEditorHistoryAvailability(state).canUndo, 'Idle editor runtime should expose document cursor undo.')
    expectTrue(!getEditorHistoryAvailability(state).canRedo, 'Idle editor runtime should disable redo at the document tail.')
    ...
    expectTrue(requested.effects.length === 1, 'Document cursor requests should emit one runtime effect.')
    expectTrue(requested.effects[0]?.type === 'document.moveHistoryCursor', 'Document cursor requests should use the editor cursor effect.')
    expectTrue(
    ...
    expectTrue(duplicate.effects.length === 0, 'A second cursor move should not be emitted while the first is pending.')
  }
  ...
  function testFeatureEditingDoesNotExposeHistory() {
    ...
    expectTrue(!getEditorHistoryAvailability(state).canUndo, 'Selection commands should not expose undo.')
    expectTrue(!getEditorHistoryAvailability(state).canRedo, 'Selection commands should not expose redo.')
    expectTrue(
