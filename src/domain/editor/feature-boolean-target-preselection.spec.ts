...
test('feature boolean target preselection classifies and applies conservative defaults', () => {
  const tolerance = { modelingTolerance: 0.001, angularToleranceRadians: 0.0001 }

  const intersecting = classifyPreviewBooleanTarget({
    previewRenderables: [boxMesh('preview_intersecting', null, [0.5, 0.5, 0.5], [1.5, 1.5, 1.5])],
  ...
  const coplanar = classifyPreviewBooleanTarget({
    previewRenderables: [planarFaceMesh('preview_coplanar', null, 0, [0.25, 0.25], [0.75, 0.75])],
    bodyRenderables: [planarFaceMesh('body_face_a', 'body_a', 0, [0, 0], [1, 1])],
    candidateBodyIds: ['body_a' as BodyId],
    settings: tolerance,
  })
  expectTrue(
  ...
  const coplanarSession = applyFeatureBooleanTargetPreselection(createExtrudeSession(), coplanar)
  expectTrue(
  ...
  const noRelationship = classifyPreviewBooleanTarget({
    previewRenderables: [planarFaceMesh('preview_far', null, 4, [0, 0], [1, 1])],
    bodyRenderables: [planarFaceMesh('body_face_a', 'body_a', 0, [0, 0], [1, 1])],
    candidateBodyIds: ['body_a' as BodyId],
    settings: tolerance,
  })
  const standaloneSession = applyFeatureBooleanTargetPreselection(createExtrudeSession(), noRelationship)
  expectTrue(
...
test('feature boolean target preselection maps advanced intents and preserves manual overrides', () => {
  const advancedSession = applyFeatureBooleanTargetPreselection(
    createFeatureEditSession({
      featureType: 'thicken',
      selectedTarget: { kind: 'face', bodyId: 'body_seed' as BodyId, faceId: 'face_seed' as FaceId },
    }),
  ...
  const preserved = applyFeatureBooleanTargetPreselection(
    manual,
    { kind: 'intersects', bodyId: 'body_b' as BodyId },
  )
...
function boxMesh(
  ...
  return meshRecord(id, ownerBodyId, [
    [min[0], min[1], min[2]],
    [max[0], min[1], min[2]],
    [max[0], max[1], min[2]],
    [min[0], max[1], min[2]],
