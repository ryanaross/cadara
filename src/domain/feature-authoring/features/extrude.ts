...
function isEndConditionKind(value: unknown): value is ExtrudeEndCondition['kind'] {
  return value === 'blind' || value === 'upToNext' || value === 'upToFace' || value === 'upToPart' || value === 'upToVertex' || value === 'throughAll'
}
...
function ensureEndSupportsMode(mode: 'oneSide' | 'symmetric' | 'twoSide', end: ExtrudeEndCondition) {
  return mode === 'symmetric' && end.kind !== 'blind' && end.kind !== 'throughAll'
    ? { ...DEFAULT_FIRST_END, direction: end.direction, draftAngle: end.draftAngle }
    : end
}
...
function patchEnd(end: ExtrudeEndCondition, patch: Record<string, unknown>, prefix: 'first' | 'second'): ExtrudeEndCondition {
  ...
  const offsetDistanceKey = prefix === 'first' ? 'upToOffsetDistance' : 'secondUpToOffsetDistance'
  ...
  const direction = isExtrudeDirection(patch[directionKey]) ? patch[directionKey] : 'direction' in end ? end.direction : 'positive'
  const draftAngle = acceptAuthoredPatch(patch[draftKey], 'draftAngle' in end ? end.draftAngle ?? 0 : 0, (value): value is number => typeof value === 'number')
  ...
  const offset = isFiniteAuthoredNumber(offsetDistance) ? { distance: offsetDistance, direction: offsetDirection } : undefined

  if (nextKind === 'blind') {
    return {
      kind: 'blind',
      direction,
      distance: acceptAuthoredPatch(patch[depthKey], end.kind === 'blind' ? end.distance : 12, (value): value is number => typeof value === 'number'),
      draftAngle,
    }
  }
  ...
  if (nextKind === 'upToNext') {
    return offset ? { kind: 'upToNext', direction, offset, draftAngle } : { kind: 'upToNext', direction, draftAngle }
  }
  ...
  if (nextKind === 'upToFace') {
    return { kind: 'upToFace', direction, draftAngle, ...(offset ? { offset } : {}), target: target?.kind === 'face' ? target : { kind: 'face', bodyId: '' as never, faceId: '' as never } }
  }
  if (nextKind === 'upToPart') {
    return { kind: 'upToPart', direction, draftAngle, ...(offset ? { offset } : {}), target: target?.kind === 'body' ? target : { kind: 'body', bodyId: '' as never } }
  }
  return { kind: 'upToVertex', direction, draftAngle, ...(offset ? { offset } : {}), target: target?.kind === 'vertex' ? target : { kind: 'vertex', bodyId: '' as never, vertexId: '' as never } }
}
...
function endHasValidScalars(end: ExtrudeEndCondition) {
  return (end.kind !== 'blind' || isPositiveAuthoredNumber(end.distance))
    && (!('draftAngle' in end) || end.draftAngle === undefined || isFiniteAuthoredNumber(end.draftAngle))
    && (!('offset' in end) || end.offset === undefined || isFiniteAuthoredNumber(end.offset.distance))
}
  ...
  switch (end.kind) {
    ...
    case 'upToVertex':
      return {
        ...end,
        draftAngle: authoredDefinitionValue(end.draftAngle ?? 0, 0),
        ...(end.offset ? { offset: { ...end.offset, distance: authoredDefinitionValue(end.offset.distance, 0) } } : {}),
      } as ExtrudeEndCondition
    case 'throughAll':
      return { ...end, draftAngle: authoredDefinitionValue(end.draftAngle ?? 0, 0) }
  }
}
...
function endFields(prefix: 'first' | 'second', end: ExtrudeEndCondition): FeatureEditorFormField[] {
  ...
  const fields: FeatureEditorFormField[] = [
    {
      ...
      value: end.kind,
      options: ['blind', 'upToNext', 'upToFace', 'upToPart', 'upToVertex', 'throughAll'].map((value) => ({ value, label: endConditionLabel(value as ExtrudeEndCondition['kind']) })),
  ...
  if (end.kind === 'blind') {
    fields.push({
      ...
      authoredValue: expressionCapableAuthoredValue(end.distance, { kind: 'positiveNumber' }),
      error: isPositiveAuthoredNumber(end.distance) ? null : { message: 'Depth must be greater than zero.' },
  ...
  if (end.kind === 'upToFace' || end.kind === 'upToPart' || end.kind === 'upToVertex') {
    ...
    fields.push({
      ...
      picker: {
        ...
        selectionFilter: createSingleTargetSelectionFilter(extrudeSelectionFilter, {
          id: `${idPrefix}-up-to-target`,
          label: `${labelPrefix}${endConditionLabel(end.kind)} target`,
          targetKind,
        }),
    ...
    fields.push(
      {
        ...
        input: 'number',
        step: 0.1,
        authoredValue: expressionCapableAuthoredValue(end.offset?.distance ?? 0, { kind: 'finiteNumber' }),
        error: end.offset?.distance === undefined || isFiniteAuthoredNumber(end.offset.distance) ? null : { message: 'Offset must be finite.' },
  ...
  fields.push({
    ...
    value: authoredNumberFormValue(end.draftAngle ?? 0, (value) => value * (180 / Math.PI)),
    input: 'angleDegrees',
    step: 1,
    authoredValue: expressionCapableAuthoredValue(end.draftAngle ?? 0, { kind: 'angle' }),
    error: end.draftAngle === undefined || isFiniteAuthoredNumber(end.draftAngle) ? null : { message: 'Draft angle must be finite.' },
...
export const extrudeAuthoringDefinition = {
  ...
  getMissingInputsDiagnostics(input) {
    ...
    if (!endHasValidScalars(input.draft.firstEnd) || !endHasRequiredTarget(input.draft.firstEnd)) {
      diagnostics.push(createMissingInputDiagnostic({
        ...
        message: 'Complete the active extrude end condition before previewing extrude.',
      }))
    }
    ...
    if (
      ...
      diagnostics.push(createMissingInputDiagnostic({
        ...
        message: 'Complete the second extrude end condition before previewing extrude.',
      }))
    }
  ...
  buildDefinition(draft) {
    ...
    const extent: ExtrudeFeatureExtent = draft.extentMode === 'twoSide'
      ? { mode: 'twoSide', firstEnd: definitionEnd(firstEnd), secondEnd: definitionEnd(draft.secondEnd) }
      : draft.extentMode === 'symmetric'
        ? { mode: 'symmetric', end: definitionEnd(firstEnd) as Extract<ExtrudeEndCondition, { kind: 'blind' | 'throughAll' }> }
