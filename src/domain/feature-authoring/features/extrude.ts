...
function patchEnd(
	...
	if (nextKind === "upToPart") {
		return {
			...
			draftAngle,
			...(offset ? { offset } : {}),
			target: target?.kind === "body" ? target : { kind: "body", bodyId: "" as never },
		};
	}
	return {
		kind: "upToVertex",
	...
	switch (end.kind) {
		...
		case "upToVertex":
			return {
				...
				draftAngle: authoredDefinitionValue(end.draftAngle ?? 0, 0),
				...(end.offset
					? { offset: { ...end.offset, distance: authoredDefinitionValue(end.offset.distance, 0) } }
					: {}),
...
function endFields(prefix: "first" | "second", end: ExtrudeEndCondition): FeatureEditorFormField[] {
	...
	const fields: FeatureEditorFormField[] = [
		{
			...
			value: end.kind,
			options: ["blind", "upToNext", "upToFace", "upToPart", "upToVertex", "throughAll"].map(
				(value) => ({ value, label: endConditionLabel(value as ExtrudeEndCondition["kind"]) }),
			),
	...
	if (end.kind === "upToFace" || end.kind === "upToPart" || end.kind === "upToVertex") {
		const targetKind =
			end.kind === "upToFace" ? "face" : end.kind === "upToPart" ? "body" : "vertex";
		fields.push({
		...
		fields.push(
			{
				...
				input: "number",
				step: 0.1,
				authoredValue: expressionCapableAuthoredValue(end.offset?.distance ?? 0, {
					kind: "finiteNumber",
				}),
				error:
					end.offset?.distance === undefined || isFiniteAuthoredNumber(end.offset.distance)
						? null
...
export const extrudeAuthoringDefinition = {
					...
					: Array.isArray(patch.profileTargets)
						? patch.profileTargets.filter(
								(entry): entry is (typeof draft.profileTargets)[number] =>
									asExtrudeProfileRef(entry as Parameters<typeof asExtrudeProfileRef>[0]) !== null,
							)
	...
	buildDefinition(draft) {
		...
		return draft.profileTargets.length > 0 &&
			...
			(draft.extentMode !== "twoSide" ||
				(endHasValidScalars(draft.secondEnd) && endHasRequiredTarget(draft.secondEnd)))
			? {
					kind: "extrude",
					featureTypeVersion: EXTRUDE_FEATURE_SCHEMA_VERSION,
					parameters: {
			...
			sections: [
				...
				},
				{
					...
					fields: [
						...
						...endFields(
							"first",
							ensureEndSupportsMode(session.draft.extentMode, session.draft.firstEnd),
						),
