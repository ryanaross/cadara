...
class InstrumentedSketchSolverAdapter implements SketchSolverAdapter {
  ...
  private readonly interactiveGestures = new Map<string, {
    startedAt: number
    updateCount: number
  ...
  projectExternalReferences(request: ProjectSketchExternalReferencesRequest) {
    return this.measure('projectExternalReferences', request, () => this.inner.projectExternalReferences(request), (result) => ({
  ...
  validateSketch(request: ValidateSketchRequest) {
    return this.measure('validateSketch', request, () => this.inner.validateSketch(request), (result) => ({
  ...
  solveSketch(request: SolveSketchRequest) {
    return this.measure('solveSketch', request, () => this.inner.solveSketch(request), (result) => ({
      ...definitionAttributes(request),
      ...
      'cadara.diagnostic_count': result.diagnostics.length + (result.regionResult?.diagnostics.length ?? 0),
    }))
  }

  async startInteractiveSolveSession(request: StartInteractiveSketchSolveSessionRequest) {
    const result = await this.measure('startInteractiveSolveSession', request, () =>
      this.inner.startInteractiveSolveSession(request), (response) => ({
        ...definitionAttributes(request),
        'cadara.projected_reference_count': request.projectedReferences.length,
  ...
  async updateInteractiveSolveSession(request: UpdateInteractiveSketchSolveSessionRequest) {
    const startedAt = getPerformanceNow()
    const result = await this.measure('updateInteractiveSolveSession', request, () =>
      this.inner.updateInteractiveSolveSession(request), (response) => ({
        'cadara.result': response.result.kind === 'blocked' ? 'blocked' : 'success',
    ...
    if (gesture) {
      ...
      gesture.maxUpdateDurationMs = Math.max(gesture.maxUpdateDurationMs, getPerformanceNow() - startedAt)
    }
    return result
  }

  async finalizeInteractiveSolveSession(request: FinalizeInteractiveSketchSolveSessionRequest) {
    const result = await this.measure('finalizeInteractiveSolveSession', request, () =>
      this.inner.finalizeInteractiveSolveSession(request), (response) => ({
        'cadara.solve_state': response.status?.solveState ?? 'cancelled',
        'cadara.constraint_state': response.status?.constraintState ?? 'cancelled',
        'cadara.diagnostic_count': response.diagnostics.length,
      }))
  ...
  async disposeInteractiveSolveSession(request: DisposeInteractiveSketchSolveSessionRequest) {
    const result = await this.measure('disposeInteractiveSolveSession', request, () =>
      this.inner.disposeInteractiveSolveSession(request), (response) => ({
  ...
  deriveSketchRegions(request: DeriveSketchRegionsRequest) {
    return this.measure('deriveSketchRegions', request, () => this.inner.deriveSketchRegions(request), (result) => ({
      ...definitionAttributes(request),
      'cadara.projected_reference_count': request.projectedReferences.length,
      'cadara.diagnostic_count': result.diagnostics.length,
    }))
  ...
  resolveSketchReference(request: ResolveSketchReferenceRequest) {
    return this.measure('resolveSketchReference', request, () => this.inner.resolveSketchReference(request), (result) => ({
  ...
  private recordGesture(sessionId: string, result: 'success' | 'cancelled' | 'failure') {
...
function definitionAttributes(request: { definition?: {
