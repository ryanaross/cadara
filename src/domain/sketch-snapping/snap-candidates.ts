...
export function collectSketchSnapGeometries(input: {
  ...
  const points = new Map(input.definition.points.map((point) => [point.pointId, point]))
  const centerPointIds = new Set(input.definition.entities.flatMap((entity) =>
    entity.kind === 'circle' || entity.kind === 'arc' ? [entity.centerPointId] : [],
  ))
...
function collectCurveCandidates(
  ...
  for (const geometry of geometries) {
    if (geometry.kind === 'lineSegment') {
      const point = projectPointToSegment(pointer, geometry.start, geometry.end)
      candidates.push(createCandidate({
        kind: 'nearestOnLine',
        point,
        pointer,
    ...
    if (geometry.kind === 'circle') {
      const point = nearestPointOnCircle(pointer, geometry.center, geometry.radius)
      candidates.push(createCandidate({
...
function collectActiveAnchorCandidates(input: ResolveSketchSnapInput, tolerance: number): SketchSnapCandidate[] {
  if (!input.activeAnchor) {
    return []
  }

  const pointer = input.pointer
  const activeAnchorSource: SketchSnapSourceRef = { kind: 'transientAnchor', id: 'active-start' }
  const candidates: SketchSnapCandidate[] = [
    createCandidate({
  ...
  for (const geometry of input.geometries) {
    if (geometry.kind === 'lineSegment') {
      const foot = projectPointToFiniteSegment(input.activeAnchor, geometry.start, geometry.end)
    ...
    if (geometry.kind === 'circle' || geometry.kind === 'arc') {
      for (const point of tangentPointsFromPoint(input.activeAnchor, geometry)) {
        candidates.push(createCandidate({
          kind: 'tangent',
          point,
          pointer,
...
function createCandidate(input: {
  ...
  const sources = [...input.sources].sort((left, right) => sourceKey(left).localeCompare(sourceKey(right)))
  const key = [
    input.kind,
...
function selectActiveCandidate(
  ...
  const previous = candidates.find((candidate) => candidate.key === activeCandidateKey)
...
function compareCandidate(left: SketchSnapCandidate, right: SketchSnapCandidate) {
  return compareDistance(left.distance, right.distance)
    || sourcePreference(left) - sourcePreference(right)
    || left.priority - right.priority
    || left.key.localeCompare(right.key)
}
...
function getCandidatePriority(kind: SketchSnapCandidateKind, activeTool: SketchToolId | null | undefined) {
...
function getCandidatePreview(kind: SketchSnapCandidateKind): SketchSnapPreviewMetadata {
  switch (kind) {
    case 'endpoint':
...
function projectPointToSegmentParameter(point: SketchPoint2D, start: SketchPoint2D, end: SketchPoint2D) {
...
function nearestPointOnCircle(point: SketchPoint2D, center: SketchPoint2D, radius: number): SketchPoint2D {
...
function nearestPointOnArc(point: SketchPoint2D, arc: Extract<SketchSnapGeometry, { kind: 'arc' }>): SketchPoint2D {
...
function tangentPointsFromPoint(
  ...
  const radius = geometry.kind === 'circle' ? geometry.radius : arcRadius(geometry)
...
): geometry is Extract<SketchSnapGeometry, { kind: 'lineSegment' | 'circle' | 'arc' }> {
  return geometry.kind === 'lineSegment' || geometry.kind === 'circle' || geometry.kind === 'arc'
}

function getCurveIntersections(
  left: Extract<SketchSnapGeometry, { kind: 'lineSegment' | 'circle' | 'arc' }>,
  right: Extract<SketchSnapGeometry, { kind: 'lineSegment' | 'circle' | 'arc' }>,
): SketchPoint2D[] {
  ...
  if (right.kind === 'lineSegment') {
    return lineCircleIntersections(right, left).filter((point) => pointLiesOnGeometry(point, left))
  }

  return circleCircleIntersections(left, right)
    .filter((point) => pointLiesOnGeometry(point, left) && pointLiesOnGeometry(point, right))
}
...
function lineLineIntersection(
  ...
  const p = left.start
  const r: SketchPoint2D = [left.end[0] - left.start[0], left.end[1] - left.start[1]]
  const q = right.start
  const s: SketchPoint2D = [right.end[0] - right.start[0], right.end[1] - right.start[1]]
...
function lineCircleIntersections(
  ...
  const radius = circleLike.kind === 'circle' ? circleLike.radius : arcRadius(circleLike)
  ...
  return [(-b - sqrt) / (2 * a), (-b + sqrt) / (2 * a)]
    ...
    .map((t) => [line.start[0] + dx * t, line.start[1] + dy * t] as SketchPoint2D)
}
...
function circleCircleIntersections(
  ...
  const a = (leftRadius * leftRadius - rightRadius * rightRadius + distance * distance) / (2 * distance)
...
function pointLiesOnGeometry(
  point: SketchPoint2D,
  geometry: Extract<SketchSnapGeometry, { kind: 'lineSegment' | 'circle' | 'arc' }>,
) {
  if (geometry.kind === 'lineSegment') {
    return distanceBetween(projectPointToSegment(point, geometry.start, geometry.end), point) <= 1e-6
  }

  if (geometry.kind === 'circle') {
    return Math.abs(distanceBetween(point, geometry.center) - geometry.radius) <= 1e-6
  }

  return pointLiesOnArc(point, geometry)
}

function pointLiesOnArc(point: SketchPoint2D, arc: Extract<SketchSnapGeometry, { kind: 'arc' }>) {
  const radius = arcRadius(arc)
  ...
  const start = normalizeAngle(Math.atan2(arc.start[1] - arc.center[1], arc.start[0] - arc.center[0]))
  const end = normalizeAngle(Math.atan2(arc.end[1] - arc.center[1], arc.end[0] - arc.center[0]))
  const angle = normalizeAngle(Math.atan2(point[1] - arc.center[1], point[0] - arc.center[0]))
