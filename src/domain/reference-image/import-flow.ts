...
export async function readReferenceImagePayload(
  file: File,
...
export type SketchImageImportFlowResult =
  ...
  | {
      ...
      reopenRequest: Extract<
        EditorEvent,
        { type: "authoring.reopenRequested" }
      >;
    };

type SketchImageImportCommitInput = NonNullable<
...
type SketchImageImportModelingService = Pick<
  ModelingService,
  "getCurrentDocumentSnapshot" | "sketchSolver"
> & {
  commitSketch(
    input: SketchImageImportCommitInput,
  ): ReturnType<ModelingService["commitSketch"]>;
};
...
function resolveSketchImageImportCommitBasis(input: {
  ...
  if (
    ...
    return {
      ...
      baseRepositoryHeads:
        input.baseRepositoryHeads ?? input.snapshot.provenance?.repositoryHeads,
    };
  }
...
function getSketchImageImportFailureMessage(
  result: ModelingCommitSketchResult,
) {
...
export async function runSketchImageImportFlow(input: {
  ...
  const requestId =
    input.requestId ?? ("request_sketch-reference-image-import" as RequestId);
  ...
  if (input.payloads) {
    ...
    try {
      payloads = await Promise.all(
        pickerResult.files.map((file) => readPayload(file)),
      );
    } catch (error: unknown) {
      return {
        kind: "failed",
