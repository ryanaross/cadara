...
function getNativeMeshPayloadBuilder(oc: OpenCascadeInstance) {
  return (oc as unknown as OpenCascadeNativeTopologyKernelHost)
    .CadaraBuildNativeMeshExportPayload?.BuildJson;
}
  ...
  try {
    ...
    return triangleIndices.map((triangle) => {
      ...
      if (!first || !second || !third) {
        throw new Error(
          `Native mesh triangle references missing vertex index ${triangle.join(",")}.`,
        );
      }

      const vertices: [MeshPoint, MeshPoint, MeshPoint] = [
        first,
        second,
        third,
      ];
...
function createOccMeshTessellationCapability(
  state: OccAuthoringState,
): MeshTessellationCapability {
  return {
    tessellate(
...
function createOccBRepWriterCapability(
  state: OccAuthoringState,
): BRepWriterCapability {
  return {
    writeStep(
      ...
      if (
        !oc.Interface_Static.IsPresent("write.step.schema") ||
        !oc.Interface_Static.IsPresent("write.step.unit")
      ) {
        return {
          diagnostic: createExportDiagnostic(
            "occ-export-writer-unavailable",
      ...
      const schemaConfigured = oc.Interface_Static.SetCVal(
        "write.step.schema",
        mapStepSchema(options.schema),
      );
      const unitConfigured = oc.Interface_Static.SetCVal(
        "write.step.unit",
        options.unit === "millimeter" ? "MM" : options.unit,
      );
      ...
      const transferMode = oc.STEPControl_StepModelType
        .STEPControl_AsIs as unknown as OpenCascadeInstance["STEPControl_StepModelType"];
      ...
      try {
        ...
        if (
          !isOccDoneStatus(writeStatus) ||
          !oc.FS.analyzePath(outputPath).exists
        ) {
          return {
            diagnostic: createExportDiagnostic(
              "occ-export-writer-failed",
        ...
        const payload = oc.FS.readFile(outputPath, {
          encoding: "utf8",
        }) as string;
...
function createOccSketchVectorCapability(
  state: OccAuthoringState,
): SketchVectorExportCapability {
  return {
...
export function createOccExportCapabilities(
  state: OccAuthoringState,
): ExportCapabilities {
  return {
