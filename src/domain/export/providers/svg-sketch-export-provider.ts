...
import type { SketchVectorEntity, SketchVectorExportModel, SketchVectorRegion, SketchVectorStyle } from '@/contracts/export/sketch-vector'
...
function arcPath(entity: Extract<SketchVectorEntity, { kind: 'arc' }>, coordinates: SvgCoordinateSystem) {
  ...
  const rawDelta = entity.sweepDirection === 'counterClockwise'
...
function bezierPath(points: readonly SketchPoint2D[], degree: 2 | 3, coordinates: SvgCoordinateSystem) {
...
function entityPath(entity: SketchVectorEntity, coordinates: SvgCoordinateSystem): string | null {
  switch (entity.kind) {
    case 'lineSegment':
    ...
    case 'spline':
      ...
      return [
        ...
        ...entity.points.slice(1).map((point) => `L ${formatPoint(point, coordinates)}`),
      ].join(' ')
    case 'bezierCurve':
      return bezierPath(entity.controlPoints, entity.degree, coordinates)
    case 'conic':
      return bezierPath([entity.start, entity.control, entity.end], 2, coordinates)
  }
}

function appendStrokeAttributes(attributes: string[], style: SketchVectorStyle | null) {
  const stroke = style?.stroke
  if (!stroke) {
    attributes.push('stroke="black"', 'stroke-width="1"', 'vector-effect="non-scaling-stroke"', 'fill="none"')
    return
  }
  ...
  if ((stroke.dashSize ?? 0) > 0 && (stroke.gapSize ?? 0) > 0) {
    attributes.push(`stroke-dasharray="${formatNumber(stroke.dashSize!)} ${formatNumber(stroke.gapSize!)}"`)
  }
}
...
function appendFillAttributes(
  ...
  if (style.fill.kind === 'solid') {
    attributes.push(`fill="${escapeXml(style.fill.color)}"`, `fill-opacity="${formatNumber(style.fill.opacity)}"`)
    return
  }
...
function entityBounds(entity: SketchVectorEntity): SketchPoint2D[] {
  switch (entity.kind) {
    ...
    case 'circle':
      return [[entity.center[0] - entity.radius, entity.center[1] - entity.radius], [entity.center[0] + entity.radius, entity.center[1] + entity.radius]]
    case 'arc':
      return [entity.start, entity.end, [entity.center[0] - entity.radius, entity.center[1] - entity.radius], [entity.center[0] + entity.radius, entity.center[1] + entity.radius]]
    case 'spline':
...
function computeCoordinateSystem(model: SketchVectorExportModel): SvgCoordinateSystem {
...
function regionPath(region: SketchVectorRegion, model: SketchVectorExportModel, coordinates: SvgCoordinateSystem) {
  const byEntityId = new Map(model.entities.map((entity) => [entity.entityId, entity]))
  ...
  for (const loop of region.loops) {
    ...
    if (points.length > 2) {
      ...
      commands.push(...points.slice(1).map((point) => `L ${formatPoint(point, coordinates)}`))
      commands.push('Z')
    }
  }
...
function serializeSvg(model: SketchVectorExportModel): ExportResult {
  ...
  for (const region of model.regions) {
    ...
    regionElements.push(`<path d="${escapeXml(path)}" ${attributes.join(' ')}/>`)
  }
  ...
  for (const entity of model.entities) {
    ...
    entityElements.push(`<path d="${escapeXml(path)}" ${attributes.join(' ')}/>`)
  }
  ...
  if (regionElements.length === 0 && entityElements.length === 0) {
    return {
      ...
      diagnostics: [{
        ...
        severity: 'error',
        message: 'SVG export could not produce any supported sketch geometry.',
...
export const svgSketchExportProvider: ExportProvider<SvgSketchExportOptions, FeatureEditorFormSchema> = {
  id: 'sketch-svg',
  label: 'SVG',
  formatId: 'svg',
  fileExtension: 'svg',
  ...
  async export(input: ExportProviderInput<SvgSketchExportOptions>): Promise<ExportResult> {
    const result = await input.capabilities.sketchVector.resolveSketchVectorModel(input.target)
