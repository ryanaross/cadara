...
import type { DocumentId, RevisionId, SketchEntityId, SketchPointId } from '@/contracts/shared/ids'
...
function resolveLocalFill(style: SketchStyleDefinition | undefined): SketchStyleFill {
...
function resolveLocalStroke(style: SketchStyleDefinition | undefined): SketchStyleStroke | null {
...
export function resolveAuthoredSketchVectorStyle(style: SketchStyleDefinition | SketchStyleRecord | undefined): SketchVectorStyle | null {
  if (!style) {
    return null
  }
  ...
  if ('styleId' in style) {
    return {
      fill: style.fill,
      stroke: style.stroke.opacity > 0 && style.stroke.width > 0 ? style.stroke : null,
    }
  }
...
function buildEntity(
  ...
  if (solvedEntity) {
    ...
    if (solvedEntity.kind === 'arc') {
      return {
        ...
        radius: distance(solvedEntity.centerPosition, solvedEntity.startPosition),
  ...
  switch (entity.kind) {
    case 'lineSegment': {
      const start = getPoint(pointMap, entity.startPointId, diagnostics, target)
      const end = getPoint(pointMap, entity.endPointId, diagnostics, target)
      return start && end ? { kind: entity.kind, entityId: entity.entityId, label: entity.label, start, end, isConstruction: entity.isConstruction, style } : null
    }
    case 'circle': {
      const center = getPoint(pointMap, entity.centerPointId, diagnostics, target)
      return center ? { kind: entity.kind, entityId: entity.entityId, label: entity.label, center, radius: entity.radius, isConstruction: entity.isConstruction, style } : null
    }
    case 'arc': {
      const center = getPoint(pointMap, entity.centerPointId, diagnostics, target)
      const start = getPoint(pointMap, entity.startPointId, diagnostics, target)
    ...
    case 'spline': {
      ...
      if (points.length < 2) {
        diagnostics.push(createDiagnostic('sketch-vector-unsupported-entity', `Spline ${entity.entityId} needs at least two exportable points.`, target))
        return null
      }
      return { kind: entity.kind, entityId: entity.entityId, label: entity.label, points, degree: entity.degree, isConstruction: entity.isConstruction, style }
    }
    case 'bezierCurve': {
      ...
      if (controlPoints.length < 3) {
        diagnostics.push(createDiagnostic('sketch-vector-unsupported-entity', `Bezier curve ${entity.entityId} needs at least three exportable control points.`, target))
        return null
      }
      return { kind: entity.kind, entityId: entity.entityId, label: entity.label, controlPoints, degree: entity.degree, isConstruction: entity.isConstruction, style }
    }
    case 'conic': {
      const start = getPoint(pointMap, entity.startPointId, diagnostics, target)
      const control = getPoint(pointMap, entity.controlPointId, diagnostics, target)
      const end = getPoint(pointMap, entity.endPointId, diagnostics, target)
      return start && control && end ? { kind: entity.kind, entityId: entity.entityId, label: entity.label, start, control, end, rho: entity.rho, isConstruction: entity.isConstruction, style } : null
    }
    case 'point':
    case 'ellipse':
    case 'ellipticalArc':
...
function buildRegionLoop(loop: RegionRecord['loops'][number], entityIds: ReadonlySet<SketchEntityId>): SketchVectorRegionLoop {
  return {
    role: loop.role,
    isClosed: loop.isClosed,
    boundaryPointIds: loop.boundaryPointIds,
    segments: loop.segments
      .filter((segment) => segment.source.kind === 'entity' && entityIds.has(segment.source.entityId))
      .map((segment) => ({
        entityId: segment.source.kind === 'entity' ? segment.source.entityId : ('' as SketchEntityId),
...
function buildRegions(
  ...
  return regions
    ...
    .map((region) => {
      const styleRecord = definition.styles?.find((style) => style.target.kind === 'region' && style.target.regionId === region.regionId)
      return {
        regionId: region.regionId,
        label: region.label,
        loops: region.loops.filter((loop) => loop.isClosed).map((loop) => buildRegionLoop(loop, entityIds)),
        style: resolveAuthoredSketchVectorStyle(styleRecord),
      }
    })
}

export function buildSketchVectorExportModel(input: BuildSketchVectorExportModelInput): SketchVectorExportModel | { diagnostic: DocumentExportDiagnostic } {
  ...
  const sketch = input.sketches.find((candidate) => candidate.sketchId === sketchTarget.sketchId)
  if (!sketch) {
    return {
      diagnostic: createDiagnostic(
  ...
  const solvedPoints = new Map(sketch.sketch.solvedSnapshot.solvedPoints.map((point) => [point.pointId, point.solvedPosition] as const))
  const solvedEntities = new Map(sketch.sketch.solvedSnapshot.solvedEntities.map((entity) => [entity.entityId, entity] as const))
  ...
  for (const point of sketch.sketch.definition.points) {
    ...
    points.set(point.pointId, solvedPoints.get(point.pointId) ?? point.position)
  }

  const entities = sketch.sketch.definition.entities
    .map((entity) => buildEntity(entity, solvedEntities.get(entity.entityId), points, input.target, diagnostics))
    .filter((entity): entity is SketchVectorEntity => entity !== null)
  const entityIds = new Set(entities.map((entity) => entity.entityId))
  const regions = buildRegions(sketch.sketch.definition, sketch.sketch.regions, entityIds)
