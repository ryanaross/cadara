...
import { createCadaraFacetedBrepTopologyFromTriangles, encodeGeometryAssetData } from '@/contracts/modeling/geometry-assets'
import { hashGeometryAssetBytes } from '@/domain/modeling/geometry-asset-store'

export function createDeterministicCadaraBrepData(byteLength = 64 * 1024, seed = 17): CadaraBrepGeometryAssetData {
  return {
    kind: 'cadaraBrep',
    ...
    bodies: [
      {
        ...
        topology: createCadaraFacetedBrepTopologyFromTriangles(createDeterministicCubeTriangles(seed), `test_${seed}_${byteLength}`),
      },
    ],
  }
}
...
export function createDeterministicGeometryAssetBytes(byteLength = 64 * 1024, seed = 17) {
  return encodeGeometryAssetData(createDeterministicCadaraBrepData(byteLength, seed))
}
