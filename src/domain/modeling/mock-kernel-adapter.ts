...
function getMockExportExtension(format: DocumentExportFormat) {
  switch (format) {
    case 'stl':
      return 'stl'
    case 'step':
...
function getMockExportMimeType(format: DocumentExportFormat) {
  switch (format) {
    case 'stl':
      return 'model/stl'
    case 'step':
...
function createMockExportFilename(targetLabel: string, format: DocumentExportFormat) {
  ...
  return `${slug}.${getMockExportExtension(format)}`
}

function createMockGeometryPayload(request: DocumentExportRequest) {
  const text = [
    `cadara mock ${request.format} export`,
    `target=${getPrimitiveRefKey(request.target)}`,
    `options=${JSON.stringify(request.options)}`,
  ].join('\n')

  if (
    (request.format === 'stl' && request.options.encoding === 'binary')
    || request.format === '3mf'
  ) {
...
async function buildSnapshot(solverAdapter: SketchSolverAdapter): Promise<DocumentSnapshot> {
  ...
  const document: DocumentSnapshot['document'] = {
    ...
    capabilities: {
      supportedFeatureKinds: ['extrude', 'fillet', 'stepImport', 'sweep', 'loft', 'chamfer', 'thicken', 'combine', 'split', 'deleteSolid'],
      previewableFeatureKinds: ['extrude', 'stepImport', 'sweep', 'loft', 'chamfer', 'thicken', 'combine', 'split', 'deleteSolid'],
...
export class MockKernelAdapter implements ModelingKernelAdapter {
  ...
  async exportDocument(request: DocumentExportRequest): Promise<DocumentExportResult> {
      ...
      return {
        ...
        diagnostics: [
          createExportDiagnostic(
            'mock-cadara-export-service-only',
    ...
    if (request.baseRevisionId !== this.currentRevisionId) {
      return {
        ok: false,
        format: request.format,
        diagnostics: [
          createExportDiagnostic(
            ...
            `Export request revision ${request.baseRevisionId} does not match current revision ${this.currentRevisionId}.`,
            request.target,
          ),
        ],
      }
    ...
    if (
      ...
      return {
        ...
        diagnostics: [
          createExportDiagnostic(
            'mock-export-unexportable-target',
    ...
    return {
      ok: true,
      format: request.format,
      filename: createMockExportFilename(request.targetLabel, request.format),
      extension: getMockExportExtension(request.format),
      mimeType: getMockExportMimeType(request.format),
      payload: createMockGeometryPayload(request),
      diagnostics: [],
    }
  }
