...
export function withContractVersion<TRequest extends { contractVersion: typeof CONTRACT_VERSION }>(
...
export function collectSketchDependencyIds(value: unknown, dependencyIds: Set<SketchId>) {
...
export function collectMissingFeatureSketchDependencyIds(document: AuthoredModelDocument) {
...
export function stripRepositoryMutationBasis<T extends { baseRepositoryHeads?: readonly string[] }>(
  input: T,
...
export function stringifyCadaraDocument(document: KernelDocumentSnapshot, pretty: boolean) {
...
export function createDocumentRepositoryDiagnostic(status: Extract<DocumentRepositoryRestoreStatus, { kind: 'failed' }>): ModelingDiagnostic {
  return {
...
export function modelingMutationResultToAppResult<T extends ModelingMutationBoundaryResult>(
  result: T,
  input: {
    operation: string
...
export function runModelingMutationBoundary<T extends ModelingMutationBoundaryResult>(input: {
  operation: string
  fallbackMessage: string
  requestId?: RequestId
  context?: readonly AppErrorContextEntry[]
