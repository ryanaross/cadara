...
import type { ModelingFeatureSuppressionResult, ModelingSetFeatureSuppressionInput } from '@/domain/modeling/modeling-service/types'
...
class InstrumentedModelingService implements ModelingService {
  ...
  subscribeToDocumentChanges(listener: Parameters<ModelingService['subscribeToDocumentChanges']>[0]) {
    return this.inner.subscribeToDocumentChanges(listener)
  }

  waitForPersistence() {
    return this.measure('waitForPersistence', () => this.inner.waitForPersistence())
  }

  getHistoryRestoreState() {
    return this.measure('getHistoryRestoreState', () => this.inner.getHistoryRestoreState(), (result) => ({
  ...
  setViewportLodTier(tierId: Parameters<ModelingService['setViewportLodTier']>[0]) {
    return this.inner.setViewportLodTier(tierId)
  }

  getCurrentDocumentSnapshot() {
    return this.measure('getCurrentDocumentSnapshot', () => this.inner.getCurrentDocumentSnapshot(), snapshotAttributes)
  }

  createNewDocument() {
    return this.measure('createNewDocument', () => this.inner.createNewDocument(), okDiagnosticAttributes)
  }

  importDocument(input: ModelingImportDocumentInput) {
    return this.measure('importDocument', () => this.inner.importDocument(input), okDiagnosticAttributes)
  }

  renameDocument(input: Parameters<ModelingService['renameDocument']>[0]) {
    return this.measure('renameDocument', () => this.inner.renameDocument(input), okDiagnosticAttributes)
  }

  exportCurrentDocument() {
    return this.measure('exportCurrentDocument', () => this.inner.exportCurrentDocument(), okDiagnosticAttributes)
  }

  bindLocalFile(input: Parameters<ModelingService['bindLocalFile']>[0]) {
    return this.measure('bindLocalFile', () => this.inner.bindLocalFile(input), okDiagnosticAttributes)
  }

  restoreLocalFileBinding() {
    return this.measure('restoreLocalFileBinding', () => this.inner.restoreLocalFileBinding())
  }

  getLocalFileSyncStatus() {
    return this.measure('getLocalFileSyncStatus', () => this.inner.getLocalFileSyncStatus())
  }

  subscribeToLocalFileSyncStatus(listener: Parameters<ModelingService['subscribeToLocalFileSyncStatus']>[0]) {
    return this.inner.subscribeToLocalFileSyncStatus(listener)
  }

  commitSketch(input: ModelingCommitSketchInput) {
    return this.measureAppResult('commitSketch', () => this.inner.commitSketch(input), mutationAttributes)
  }

  projectSketchExternalReferences(input: Parameters<ModelingService['projectSketchExternalReferences']>[0]) {
    return this.measure('projectSketchExternalReferences', () => this.inner.projectSketchExternalReferences(input), diagnosticAttributes)
  }

  addDocumentVariable(input: ModelingAddDocumentVariableInput) {
    return this.measureAppResult('addDocumentVariable', () => this.inner.addDocumentVariable(input), mutationAttributes)
  }

  updateDocumentVariable(input: ModelingUpdateDocumentVariableInput) {
    return this.measureAppResult('updateDocumentVariable', () => this.inner.updateDocumentVariable(input), mutationAttributes)
  }

  createFeature(input: ModelingCreateFeatureInput) {
    return this.measureAppResult('createFeature', () => this.inner.createFeature(input), mutationAttributes)
  }

  updateFeature(input: ModelingUpdateFeatureInput) {
    return this.measureAppResult('updateFeature', () => this.inner.updateFeature(input), mutationAttributes)
  }

  setFeatureSuppression(input: ModelingSetFeatureSuppressionInput) {
    return this.measureAppResult('setFeatureSuppression', () => this.inner.setFeatureSuppression(input), mutationAttributes)
  }

  deleteFeature(input: ModelingDeleteFeatureInput) {
    return this.measureAppResult('deleteFeature', () => this.inner.deleteFeature(input), mutationAttributes)
  }

  deleteTarget(input: ModelingDeleteTargetInput) {
    return this.measureAppResult('deleteTarget', () => this.inner.deleteTarget(input), mutationAttributes)
  }

  renameBody(input: ModelingRenameBodyInput) {
    return this.measureAppResult('renameBody', () => this.inner.renameBody(input), mutationAttributes)
  }

  reorderFeature(input: ModelingReorderFeatureInput) {
    return this.measureAppResult('reorderFeature', () => this.inner.reorderFeature(input), mutationAttributes)
  }

  reorderDocumentHistory(input: ModelingReorderDocumentHistoryInput) {
    return this.measureAppResult('reorderDocumentHistory', () => this.inner.reorderDocumentHistory(input), mutationAttributes)
  }

  setFeatureCursor(input: ModelingSetFeatureCursorInput) {
    return this.measureAppResult('setFeatureCursor', () => this.inner.setFeatureCursor(input), mutationAttributes)
  }

  evaluatePreview(input: Parameters<ModelingService['evaluatePreview']>[0]) {
    return this.measure('evaluatePreview', () => this.inner.evaluatePreview(input), diagnosticAttributes)
  }

  exportDocument(input: ModelingExportDocumentInput) {
    return this.measure('exportDocument', () => this.inner.exportDocument(input), okDiagnosticAttributes)
  }

  resolveReference(target: Parameters<ModelingService['resolveReference']>[0]) {
    return this.measure('resolveReference', () => this.inner.resolveReference(target), diagnosticAttributes)
  }
    ...
    return action()
      ...
      .mapErr((error: AppError) => {
        span.end({
          ...
          'cadara.diagnostic_count': Number(error.code === 'modeling/diagnostic'),
        })
        return error
      })
  }
...
function snapshotAttributes(result: Awaited<ReturnType<ModelingService['getCurrentDocumentSnapshot']>>): PerformanceSpanAttributes {
  return {
...
function mutationAttributes(
  result: ModelingFeatureMutationResult
...
function diagnosticAttributes(result: { diagnostics?: readonly unknown[] }): PerformanceSpanAttributes {
  return {
    'cadara.diagnostic_count': result.diagnostics?.length ?? 0,
  }
}

function okDiagnosticAttributes(result: { ok?: boolean; diagnostics?: readonly unknown[] }): PerformanceSpanAttributes {
  return {
