...
export interface LocalFileBindingStore {
  ...
  load(
    documentId: DocumentId,
  ): Promise<LocalFileBindingStoreResult<LocalFileBindingRecord | null>>;
  save(
    record: LocalFileBindingRecord,
  ): Promise<LocalFileBindingStoreResult<LocalFileBindingMetadata>>;
...
export function createIndexedDbLocalFileBindingStore(
  ...
  async function withStore<T>(
    ...
    try {
      const database = await openLocalFileBindingDatabase(
        indexedDBFactory,
        databaseName,
        storeName,
      );
  ...
  return {
    ...
    load(documentId) {
      return withStore(
        "readonly",
        (store) =>
          store.get(documentId) as IDBRequest<LocalFileBindingRecord | null>,
      );
    },
    save(record) {
      return withStore("readwrite", (store) => {
        ...
        ) as IDBRequest<LocalFileBindingRecord>;
      }).then((result) => {
        if (!result.ok) {
          return result;
        }
    ...
    clear(documentId) {
      return withStore(
        "readwrite",
        (store) => store.delete(documentId) as unknown as IDBRequest<null>,
      );
    },
  };
}
...
function openLocalFileBindingDatabase(
  ...
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ??
          new Error("Local file binding database failed to open."),
      );
  });
}
...
function resolveIdbRequest<T>(request: IDBRequest<T>) {
  return new Promise<T>((resolve, reject) => {
    request.onsuccess = () => resolve(request.result);
    request.onerror = () =>
      reject(request.error ?? new Error("Local file binding request failed."));
  });
}
