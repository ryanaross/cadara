...
test('src/domain/modeling/performance-telemetry-wrappers.spec.ts preserves OCC worker results and rejections', async () => {
  ...
  } as unknown as OccWorkerSnapshotClient, telemetry)

  const snapshotResult = await client?.getDocumentSnapshot({} as never)
  expectTrue(snapshotResult?.snapshot.document.bodies.length === 1, 'The OCC wrapper should preserve successful payloads.')
  expectTrue(
    telemetry.finished.some((span) => span.attributes['cadara.operation'] === 'getDocumentSnapshot'
  ...
  try {
    ...
    expectTrue(error === failure, 'The OCC wrapper should preserve the exact rejection error.')
  }
  expectTrue(
    telemetry.finished.some((span) => span.attributes['cadara.operation'] === 'createFeature'
...
test('src/domain/modeling/performance-telemetry-wrappers.spec.ts records repository source and heads without changing results', async () => {
  ...
  } as unknown as DocumentRepository, telemetry)

  const result = await repository.load({ documentId: 'doc_1', seedDocument: {} as never })
  expectTrue(result.ok === true, 'The repository wrapper should preserve load results.')
  expectTrue(
    telemetry.finished.some((span) => span.attributes['cadara.operation'] === 'load'
...
test('src/domain/modeling/performance-telemetry-wrappers.spec.ts preserves modeling AppResultAsync behavior', async () => {
  const telemetry = createRecordingTelemetry()
  const appError = createAppError({ code: 'modeling/diagnostic', message: 'Rejected.' })
  const service = createInstrumentedModelingService({
    currentDocumentId: 'doc_1',
    sketchSolver: null,
    createFeature() {
      return ResultAsync.fromPromise(Promise.reject(appError), () => appError)
    },
  } as unknown as ModelingService, telemetry)

  const result = await service.createFeature({} as never)
  expectTrue(result.isErr(), 'The modeling wrapper should preserve AppResultAsync failures.')
  expectTrue(
    telemetry.finished.some((span) => span.attributes['cadara.operation'] === 'createFeature'
...
test('src/domain/modeling/performance-telemetry-wrappers.spec.ts aggregates sketch interactive drag telemetry', async () => {
  ...
  } as unknown as SketchSolverAdapter, telemetry)
  ...
  expectTrue(
    telemetry.finished.some((span) => span.attributes['cadara.operation'] === 'interactiveDragGesture'
...
function createRecordingTelemetry(): PerformanceTelemetry & {
  finished: Array<{ descriptor: unknown; attributes: PerformanceSpanAttributes }>
} {
  const finished: Array<{ descriptor: unknown; attributes: PerformanceSpanAttributes }> = []
  return {
    finished,
