...
export function resolveFeatureDefinitionValues(input: {
  ...
  for (const field of getFeatureValueExpressionFields(resolved)) {
    const authoredValue = getPathValue(
      resolved as unknown as MutableRecord,
      field.path,
    );
...
function resolveAuthoredFeatureValue(input: {
  ...
  if (!isExpressionAuthoredValue(input.value)) {
    ...
    const literalValue =
      input.value.source === "literal" ? input.value.value : input.value;
    const validation = validateFeatureValueKind(
      literalValue,
      input.field.valueKind,
    );
    ...
    return validation.ok
      ...
      : {
          ok: false,
          diagnostic: createValueKindDiagnostic(
            input.field,
  ...
  for (const symbolName of collectSymbolNames(node)) {
    if (
      input.variablesByName.has(symbolName) ||
      isKnownMathSymbol(symbolName)
    ) {
      continue;
    }
...
function createValueKindDiagnostic(
  field: FeatureValueExpressionFieldDescriptor,
  code: ReturnType<typeof validateFeatureValueKind> extends {
    ok: false;
    failure: { code: infer TCode };
  }
    ? TCode
    : string,
  message: string,
  ...
  const diagnosticCode =
    `feature-value-expression-${code}` as FeatureValueExpressionDiagnosticCode;
  return createDiagnostic(diagnosticCode, `${field.label}: ${message}`);
}

function createDiagnostic(
  code: FeatureValueExpressionDiagnosticCode,
  message: string,
): ModelingDiagnostic {
  return {
    code,
