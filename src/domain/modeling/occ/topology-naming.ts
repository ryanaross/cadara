...
type OccNamedShape = InstanceType<OpenCascadeInstance['Handle_TNaming_NamedShape']>
...
export const OCC_TOPOLOGY_NAMING_STRATEGY = 'selector-backed-ocaf-labels' as const
...
function hasShapeType(shape: OccShape, expected: OccShape) {
  ...
  if (typeof shapeType.value === 'number' && typeof expectedType.value === 'number') {
...
function createSelectorLabel(
  ...
  try {
    try {
      const selectedWithContext = selector.Select_1(selection, context, false, true)
      const selectedShapes = readNamedShape(oc, selector.NamedShape())

      if (
        selectedWithContext
        && selectedShapes.some((shape) => shape.IsSame(selection) && hasShapeType(shape, selection))
      ) {
        return label
      }
    } catch {
...
function createBodyLabel(oc: OpenCascadeInstance, document: OccDocument, shape: OccShape) {
...
function createInitialNamingState(
  ...
  for (const [faceId, face] of body.facesById) {
    ...
    selectorLabelsByKey.set(key, createSelectorLabel(oc, bodyLabel, face, body.shape))
  }
  ...
  for (const [edgeId, edge] of body.edgesById) {
    ...
    selectorLabelsByKey.set(key, createSelectorLabel(oc, bodyLabel, edge, body.shape))
  }

  for (const [vertexId, vertex] of body.verticesById) {
    const key = topologyRefKey({ kind: 'vertex', bodyId: body.bodyId, vertexId })
    topologyLabelsByKey.set(key, createPrimitiveLabel(oc, bodyLabel, vertex))
    selectorLabelsByKey.set(key, createSelectorLabel(oc, bodyLabel, vertex, body.shape))
  }
...
function listShapes(
  ...
  const shapes: OccShape[] = []
  const copy = new oc.TopTools_ListOfShape_3(list as InstanceType<OpenCascadeInstance['TopTools_ListOfShape']>)
...
function readCurrentNamedShape(
  ...
  try {
    const currentNamedShape = oc.TNaming_Tool.CurrentNamedShape_1(namedShape, validLabels)
...
export function isOccTopologyHistoryDeleted(
  ...
  return historySource.IsDeleted?.(shape) === true
...
function reconcileKind<Id extends FaceId | EdgeId | VertexId, Shape extends OccFace | OccEdge | OccVertex>(
  ...
  for (const previousId of input.previousIds) {
    ...
    const target = targetFor(input.kind as never, input.bodyId, previousId as never)
    ...
    const resolution = selectorFinalIndexes.length > 0
  ...
  for (const [index, ids] of claimsByIndex) {
    ...
    const key = topologyRefKey(targetFor(input.kind as never, input.bodyId, id as never))
    const label = input.previousLabelsByKey.get(key)
  ...
  for (const [freshId, freshShape] of input.freshShapesById) {
    ...
    const key = topologyRefKey(targetFor(input.kind as never, input.bodyId, freshId as never))
    input.nextLabelsByKey.set(key, createPrimitiveLabel(oc, input.bodyLabel, freshShape))
    input.nextSelectorLabelsByKey.set(
      key,
