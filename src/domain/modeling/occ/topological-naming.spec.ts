...
function createChamferDefinition(bodyId: BodyId, edgeId: EdgeId): FeatureDefinition {
  return {
    kind: 'chamfer',
...
function createShellJoinDefinition(bodyId: BodyId, removableFaceId: FaceId): FeatureDefinition {
  return {
    kind: 'shell',
...
function createThickenDefinition(bodyId: BodyId, faceId: FaceId): FeatureDefinition {
  return {
    kind: 'thicken',
    featureTypeVersion: ADVANCED_SOLID_FEATURE_SCHEMA_VERSION,
    parameters: {
...
function distanceSquared(left: readonly [number, number, number], right: readonly [number, number, number]) {
  return (left[0] - right[0]) ** 2
    + (left[1] - right[1]) ** 2
    + (left[2] - right[2]) ** 2
}

function pointNear(left: readonly [number, number, number], right: readonly [number, number, number]) {
...
function edgeHasEndpoints(
  ...
  const start = pointCoordinates(oc.BRep_Tool.Pnt(oc.TopExp.FirstVertex(edge, true)))
  const end = pointCoordinates(oc.BRep_Tool.Pnt(oc.TopExp.LastVertex(edge, true)))

  return (pointNear(start, first) && pointNear(end, second))
...
test('proper naming should carry untouched edge and vertex references through chained fillet and chamfer operations', async () => {
  ...
  const bodyId = bodyIdForFeature(baseFeatureId)
  const xy = createStandardPlaneDefinition('xy')
  const base = createRectangleSketch('sketch_occ_limit_fillet_chamfer_base' as SketchId, xy, {
    width: 10,
    height: 8,
  })
  const initial = createOccAuthoringState(oc, { sketches: [base.sketch] })
  ...
  const baseBody = requireBody(afterBase, bodyId)
  const stableEdgeId = findEdgeByEndpoints(oc, baseBody, [10, 8, 0], [10, 8, 6])
  ...
  const chamferEdgeId = findEdgeByEndpoints(oc, requireBody(afterFillet, bodyId), [0, 8, 0], [0, 8, 6])
...
test('proper naming should allow an old edge id to drive a downstream fillet after chained fillet and chamfer operations', async () => {
  ...
  const bodyId = bodyIdForFeature(baseFeatureId)
  const xy = createStandardPlaneDefinition('xy')
  const base = createRectangleSketch('sketch_occ_limit_downstream_old_edge' as SketchId, xy, {
    width: 10,
    height: 8,
  })
  const initial = createOccAuthoringState(oc, { sketches: [base.sketch] })
  ...
  const baseBody = requireBody(afterBase, bodyId)
  const downstreamEdgeId = findEdgeByEndpoints(oc, baseBody, [10, 8, 0], [10, 8, 6])
  const firstFilletEdgeId = findEdgeByEndpoints(oc, baseBody, [0, 0, 0], [0, 0, 6])
  const afterFillet = applyFeature(afterBase, {
...
test('proper naming should keep untouched edge and vertex references live through shell replacement', async () => {
  ...
  const bodyId = bodyIdForFeature(baseFeatureId)
  const xy = createStandardPlaneDefinition('xy')
  const base = createRectangleSketch('sketch_occ_limit_shell_stress' as SketchId, xy, {
    width: 10,
    height: 8,
  })
  const initial = createOccAuthoringState(oc, { sketches: [base.sketch] })
...
test('proper naming should carry thicken-produced topology through chained fillet and chamfer operations', async () => {
  ...
  const xy = createStandardPlaneDefinition('xy')
  const base = createRectangleSketch('sketch_occ_limit_thicken_stress' as SketchId, xy, {
    width: 10,
    height: 8,
  })
  const initial = createOccAuthoringState(oc, { sketches: [base.sketch] })
  ...
  const sourceTopFaceId = findPlanarFaceAtZ(oc, requireBody(afterBase, sourceBodyId), 6)
  ...
  const stableEdgeId = findEdgeByEndpoints(oc, thickenedBody, [10, 8, 6], [10, 8, 6.5])
  const stableVertexId = findVertexAt(oc, thickenedBody, [10, 8, 6.5])
  const filletEdgeId = findEdgeByEndpoints(oc, thickenedBody, [0, 0, 6], [0, 0, 6.5])
  ...
  const chamferEdgeId = findEdgeByEndpoints(oc, requireBody(afterFillet, thickenedBodyId), [0, 8, 6], [0, 8, 6.5])
  const afterChamfer = applyFeature(afterFillet, {
...
test('proper naming should report a deleted-topology diagnostic for a cut-away face', async () => {
  ...
  const bodyId = bodyIdForFeature(baseFeatureId)
  const xy = createStandardPlaneDefinition('xy')
  const base = createRectangleSketch('sketch_occ_limit_deleted_cut' as SketchId, xy, {
    width: 4,
    height: 3,
  })
  const initial = createOccAuthoringState(oc, { sketches: [base.sketch] })
  ...
  const removedFaceId = findPlanarFaceAtZ(oc, requireBody(afterBase, bodyId), 0)
...
test('proper naming should report an ambiguous-topology diagnostic for split successors', async () => {
  ...
  const xy = createStandardPlaneDefinition('xy')
  const target = createRectangleSketch('sketch_occ_limit_split_target' as SketchId, xy, {
    width: 6,
    height: 4,
  })
  const tool = createRectangleSketch('sketch_occ_limit_split_tool' as SketchId, xy, {
    origin: [2, 0],
    width: 2,
    height: 4,
  })
  const initial = createOccAuthoringState(oc, { sketches: [target.sketch, tool.sketch] })
  ...
  const selectedFaceId = findPlanarFaceAtZ(oc, requireBody(afterTarget, targetBodyId), 4)
...
test('proper naming should invalidate consumed Combine tool-body topology', async () => {
  ...
  const toolBodyId = bodyIdForFeature(toolFeatureId)
  const xy = createStandardPlaneDefinition('xy')
  const target = createRectangleSketch('sketch_occ_limit_combine_target' as SketchId, xy, {
    width: 4,
    height: 4,
  })
  const tool = createRectangleSketch('sketch_occ_limit_combine_tool' as SketchId, xy, {
    origin: [2, 1],
    width: 3,
    height: 2,
  })
