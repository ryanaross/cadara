import type { RenderPoint3D, RenderableEntityRecord } from '@/contracts/render/schema'
...
function createTier(
  ...
  const convertedAngular = degreesToOccAngularDeflectionRadians(angularDegreeEquivalent)
  const angularDeflectionRadians = id === 'startup'
  ...
  return {
    ...
    angularDeflectionDegreeEquivalent: angularDeflectionRadians * (180 / Math.PI),
  }
}

export const OCC_TESSELLATION_TIERS: Record<OccTessellationTierId, OccTessellationTier> = {
  startup: createTier('startup', 0.75, 1.5),
  normal: createTier('normal', 0.25, 8),
  fine: createTier('fine', 0.1, 2),
}

export function getOccTessellationTier(tierId: OccTessellationTierId = 'startup') {
  return OCC_TESSELLATION_TIERS[tierId]
}

export function selectBodyLodTier(input: BodyLodSelectionInput): OccTessellationTierId {
  if (input.bodyRadiusModelUnits <= 0 || input.cameraDistanceModelUnits <= 0) {
    return 'startup'
  }

  const projectedSize = input.bodyRadiusModelUnits / input.cameraDistanceModelUnits
...
export function selectViewportLodTierForRenderables(input: ViewportLodSelectionInput): OccTessellationTierId {
...
function collectGeometryPoints(renderable: RenderableEntityRecord): RenderPoint3D[] {
