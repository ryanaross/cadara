...
export type PackedRenderableEntityRecord =
  Omit<RenderableEntityRecord, 'geometry'> & {
...
export function packMeshGeometry(geometry: RenderMeshGeometry): PackedMeshGeometry {
  const positions = new Float32Array(geometry.vertexPositions.length * 3)
  const normals = geometry.vertexNormals ? new Float32Array(geometry.vertexNormals.length * 3) : null
...
export function unpackMeshGeometry(geometry: PackedMeshGeometry): RenderMeshGeometry {
  ...
  return {
    ...
    triangleIndices: Array.from({ length: indices.length / 3 }, (_, index) => {
      const offset = index * 3
      return [indices[offset]!, indices[offset + 1]!, indices[offset + 2]!] as const
    }),
  }
}

export function packRenderableMeshes(records: readonly RenderableEntityRecord[]) {
...
export function unpackRenderableMeshes(records: readonly PackedRenderableEntityRecord[]) {
...
export function collectPackedMeshTransferList(records: readonly PackedRenderableEntityRecord[]) {
...
export function unpackWorkspaceSnapshotRenderMeshes(snapshot: WorkspaceSnapshot | PackedWorkspaceSnapshot): WorkspaceSnapshot {
  const records = unpackRenderableMeshes(snapshot.document.render.records as PackedRenderableEntityRecord[])
...
function createPackedView(view: Float32Array | Uint32Array): PackedTypedArrayView {
  return {
...
function writePointTriplets(target: Float32Array, points: readonly RenderPoint3D[]) {
