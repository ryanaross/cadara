...
import { dot, magnitude, mapSketchPointToWorld as mapPointWithFrame, mapWorldPointToSketch as mapPointToSketchWithFrame, normalize, type Vec3 } from '@/domain/modeling/occ/math'
...
export interface ExtractedPlanarFaceData {
  ...
  plane: ReturnType<InstanceType<OpenCascadeInstance['BRepAdaptor_Surface_2']>['Plane']>
}
...
export function buildConstructionPlaneFromPlanarFace(
  ...
  const { frame } = extractPlanarFaceData(oc, face, `Face ${faceId} is not planar.`)
  return {
    support,
    frame,
    key: null,
