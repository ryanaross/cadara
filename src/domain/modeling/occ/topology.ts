...
function indexFacesByNativePayload(
  ...
  for (const record of nativePayload.topology) {
    ...
    if (record.index > faceMap.Size()) {
      faceMap.delete()
      throw new Error(`Native topology payload references missing face index ${record.index}.`)
    }
  ...
  if (faceIds.length !== faceMap.Size()) {
    ...
    faceMap.delete()
    throw new Error(`Native topology payload returned ${faceIds.length} faces for body ${bodyId}; expected ${faceCount}.`)
  }
...
function indexEdgesByNativePayload(
  ...
  for (const record of nativePayload.topology) {
    ...
    if (record.index > edgeMap.Size()) {
      edgeMap.delete()
      throw new Error(`Native topology payload references missing edge index ${record.index}.`)
    }
  ...
  if (edgeIds.length !== edgeMap.Size()) {
    ...
    edgeMap.delete()
    throw new Error(`Native topology payload returned ${edgeIds.length} edges for body ${bodyId}; expected ${edgeCount}.`)
  }
...
function indexVerticesByNativePayload(
  ...
  for (const record of nativePayload.topology) {
    ...
    if (record.index > vertexMap.Size()) {
      vertexMap.delete()
      throw new Error(`Native topology payload references missing vertex index ${record.index}.`)
    }

    const vertexId = record.id as VertexId
    vertexIds.push(vertexId)
    verticesById.set(vertexId, oc.TopoDS.Vertex_1(vertexMap.FindKey(record.index)))
  }

  if (vertexIds.length !== vertexMap.Size()) {
    const vertexCount = vertexMap.Size()
    vertexMap.delete()
    throw new Error(`Native topology payload returned ${vertexIds.length} vertices for body ${bodyId}; expected ${vertexCount}.`)
  }
...
function buildNativeTopologyPayloadForTrackedSolid(
  ...
  if (!buildJson) {
    if ((oc as OpenCascadeInstance & { CadaraAllowLegacyTopologyEnumerationForTests?: boolean })
  ...
  const nativePayload = disambiguateNativeTopologyPayloadIdentity(input.bodyId, parseNativeShimPayloadJson(buildJson(
    input.solid,
    input.bodyId,
    input.topologyToken,
  ...
  assertReleasableNativeTopologyPayload(input.bodyId, nativePayload, 'tracked solid body')
...
export function haveSameOccTopologyIds(
  ...
  return left.faceIds.length === right.faceIds.length
...
function assertReleasableNativeTopologyPayload(
  ...
  const errorDiagnostic = nativePayload.diagnostics.find((diagnostic) => diagnostic.severity === 'error')

  if (errorDiagnostic && !isPreviewBody && !defersPre8Validation) {
    throw new Error(`Native topology payload for ${operation} ${bodyId} is not releasable: ${errorDiagnostic.message}`)
  }
...
function disambiguateNativeTopologyPayloadIdentity(
  ...
  for (const record of nativePayload.topology) {
    ...
    kernelUidCounts.set(kernelUidKey, (kernelUidCounts.get(kernelUidKey) ?? 0) + 1)
  }
  ...
  const topology = nativePayload.topology.map((record) => {
    ...
    const needsDisambiguation = (idCounts.get(idKey) ?? 0) > 1
    ...
    firstReplacementByKindAndId.set(idKey, firstReplacementByKindAndId.get(idKey) ?? nextId)
...
export function trackReplacementSolidBodyFromNativePayload(
  ...
  const [solid] = solids
  const nativePayload = disambiguateNativeTopologyPayloadIdentity(input.previous.bodyId, input.nativePayload)
  assertReleasableNativeTopologyPayload(input.previous.bodyId, nativePayload, 'native replacement body')
  const faces = indexFacesByNativePayload(oc, input.previous.bodyId, solid, nativePayload)
  const edges = indexEdgesByNativePayload(oc, input.previous.bodyId, solid, nativePayload)
  const vertices = indexVerticesByNativePayload(oc, input.previous.bodyId, solid, nativePayload)
...
export function reconcileReplacementSolidBody(
  ...
  const reconciledBody = {
    ...
    nativeTopologyPayload: haveSameOccTopologyIds(reconciliation.topology, replacement.topology)
