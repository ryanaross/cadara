...
test('executeTransformFeature uses native transaction history for replacement topology', async () => {
  ...
  expectTrue(replacement != null, 'Native transform should replace the selected body.')
  expectTrue(
  ...
  assertNativeHistoryDidNotFallBack(result.historyInvalidations, 'Native transform')
})
...
test('executeShellFeature uses native shell transaction before replacement boolean composition', async () => {
  ...
  const nativeBuilder = nativeHost.CadaraExecuteNativeFeatureTransaction?.BuildShellCommittedShapeTransactionWithHistory
  let nativeCallCount = 0
  expectTrue(typeof nativeBuilder === 'function', 'Expected custom OCC runtime to expose native shell transactions.')
  nativeHost.CadaraExecuteNativeFeatureTransaction!.BuildShellCommittedShapeTransactionWithHistory = (...args) => {
    nativeCallCount += 1
    return nativeBuilder(...args)
  }
  const faceId = body.topology.faceIds[0]
  expectTrue(faceId != null, 'Expected the tracked box to expose a shell removable face target.')
  const context = createOccAuthoringState(oc, { bodies: [body] })
