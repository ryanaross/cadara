...
import type { ExportCapabilities, MeshExportAccuracy, StepWriterOptions } from '@/contracts/export/capabilities'
import type { DocumentExportDiagnostic } from '@/contracts/modeling/export'
import type { PerformanceSpanAttributes, PerformanceTelemetry } from '@/contracts/performance/telemetry'
import {
  classifyOkResult,
...
class InstrumentedOccWorkerClient implements OccWorkerSnapshotClient {
  ...
  exportAuthoredModelDocument(documentId: AuthoredModelDocument['documentId']) {
    return this.measure('exportAuthoredModelDocument', () => this.inner.exportAuthoredModelDocument(documentId))
  }

  getDocumentSnapshot(request: GetDocumentSnapshotRequest, lodTierId?: OccTessellationTierId) {
    return this.measure('getDocumentSnapshot', () => this.inner.getDocumentSnapshot(request, lodTierId), snapshotResultAttributes)
  }

  projectSketchExternalReferences(request: ProjectSketchExternalReferencesRequest) {
    return this.measure('projectSketchExternalReferences', () => this.inner.projectSketchExternalReferences(request), diagnosticResultAttributes)
  }

  commitSketch(request: CommitSketchRequest) {
    return this.measure('commitSketch', () => this.inner.commitSketch(request), diagnosticResultAttributes)
  }

  createFeature(request: CreateFeatureRequest) {
    return this.measure('createFeature', () => this.inner.createFeature(request), diagnosticResultAttributes)
  }

  updateFeature(request: UpdateFeatureRequest) {
    return this.measure('updateFeature', () => this.inner.updateFeature(request), diagnosticResultAttributes)
  }

  setFeatureSuppression(request: SetFeatureSuppressionRequest) {
    return this.measure('setFeatureSuppression', () => this.inner.setFeatureSuppression(request), diagnosticResultAttributes)
  }

  deleteFeature(request: DeleteFeatureRequest) {
    return this.measure('deleteFeature', () => this.inner.deleteFeature(request), diagnosticResultAttributes)
  }

  deleteTarget(request: DeleteDocumentTargetRequest) {
    return this.measure('deleteTarget', () => this.inner.deleteTarget(request), diagnosticResultAttributes)
  }

  renameBody(request: RenameBodyRequest) {
    return this.measure('renameBody', () => this.inner.renameBody(request), diagnosticResultAttributes)
  }

  reorderFeature(request: ReorderFeatureRequest) {
    return this.measure('reorderFeature', () => this.inner.reorderFeature(request), diagnosticResultAttributes)
  }

  reorderDocumentHistory(request: ReorderDocumentHistoryRequest) {
    return this.measure('reorderDocumentHistory', () => this.inner.reorderDocumentHistory(request), diagnosticResultAttributes)
  }

  setFeatureCursor(request: SetFeatureCursorRequest) {
    return this.measure('setFeatureCursor', () => this.inner.setFeatureCursor(request), diagnosticResultAttributes)
  }

  addDocumentVariable(request: AddDocumentVariableRequest) {
    return this.measure('addDocumentVariable', () => this.inner.addDocumentVariable(request), diagnosticResultAttributes)
  }

  updateDocumentVariable(request: UpdateDocumentVariableRequest) {
    return this.measure('updateDocumentVariable', () => this.inner.updateDocumentVariable(request), diagnosticResultAttributes)
  }

  evaluatePreview(request: EvaluatePreviewRequest) {
    return this.measure('evaluatePreview', () => this.inner.evaluatePreview(request), diagnosticResultAttributes)
  }

  resolveReference(request: ResolveReferenceRequest) {
    return this.measure('resolveReference', () => this.inner.resolveReference(request), diagnosticResultAttributes)
  }

  probeNativeTopologyKernelCapabilities(assets?: OccWorkerAssetConfig) {
    return this.measure('probeNativeTopologyKernelCapabilities', () => this.inner.probeNativeTopologyKernelCapabilities(assets))
  }

  buildNativeTopologySnapshot(request: GetDocumentSnapshotRequest, lodTierId?: OccTessellationTierId) {
    return this.measure('buildNativeTopologySnapshot', () => this.inner.buildNativeTopologySnapshot(request, lodTierId), nativeTopologyResultAttributes)
  }
  ...
  executeNativeFeatureHistoryRebuild(
    ...
    return this.measure('executeNativeFeatureHistoryRebuild', () =>
      this.inner.executeNativeFeatureHistoryRebuild(document, diagnostics, assets, lodTierId), nativeTopologyResultAttributes)
  }
  ...
  buildNativeBooleanFeatureTransactionPayload(
    ...
    return this.measure('buildNativeBooleanFeatureTransactionPayload', () =>
      this.inner.buildNativeBooleanFeatureTransactionPayload(documentId, baseRevisionId, leftBodyId, rightBodyId, operation, lodTierId), nativeTopologyResultAttributes)
  }
  ...
  buildNativeMeshExportPayload(
    ...
    return this.measure('buildNativeMeshExportPayload', () =>
      this.inner.buildNativeMeshExportPayload(documentId, baseRevisionId, target, options), nativeTopologyResultAttributes)
  }
  ...
  buildNativeExactBrepPayload(
    ...
    return this.measure('buildNativeExactBrepPayload', () =>
      this.inner.buildNativeExactBrepPayload(documentId, baseRevisionId, target), nativeTopologyResultAttributes)
  }
  ...
  async getExportCapabilities(
    ...
    return {
      ...
      mesh: {
        ...capabilities.mesh,
        tessellate: (target, options) => this.measure('tessellateExportMesh', () =>
          Promise.resolve(capabilities.mesh.tessellate(target, options))),
      },
      brep: {
        ...capabilities.brep,
        writeStep: (target, options: StepWriterOptions) => this.measure('writeStepExport', () =>
          Promise.resolve(capabilities.brep.writeStep(target, options))),
      },
      sketchVector: {
        ...capabilities.sketchVector,
        resolveSketchVectorModel: (target) => this.measure('resolveSketchVectorExportModel', () =>
          Promise.resolve(capabilities.sketchVector.resolveSketchVectorModel(target))),
      },
    }
  }
...
function diagnosticResultAttributes(result: unknown): PerformanceSpanAttributes {
  const diagnostics = typeof result === 'object' && result !== null && 'diagnostics' in result
...
function snapshotResultAttributes(result: unknown): PerformanceSpanAttributes {
  const snapshot = typeof result === 'object' && result !== null && 'snapshot' in result
    ? (result as { snapshot?: { document?: { features?: readonly unknown[]; sketches?: readonly unknown[]; bodies?: readonly unknown[]; render?: { records?: readonly unknown[] }; diagnostics?: readonly unknown[] } } }).snapshot
    : null
  ...
  return snapshot?.document
    ? {
        ...
        'cadara.render_record_count': snapshot.document.render?.records?.length ?? 0,
        'cadara.diagnostic_count': snapshot.document.diagnostics?.length ?? 0,
      }
    : {}
}

function nativeTopologyResultAttributes(result: unknown): PerformanceSpanAttributes {
