...
test('src/domain/modeling/document-history.spec.ts', async () => {  const adapter = new MockKernelAdapter()
  ...
  expectTrue(items.length >= 2, 'Seed document should expose multiple document history items.')
  const firstItem = items[0]
  const seedSketchItem = items.find((item) => item.kind === 'sketch')
  const seedFeatureItem = items.find((item) => item.kind === 'feature')

  expectTrue(firstItem, 'Seed document should expose a first document history item.')
  expectTrue(seedSketchItem?.kind === 'sketch', 'Seed document should expose a sketch history item.')
  expectTrue(seedFeatureItem?.kind === 'feature', 'Seed document should expose a feature history item.')

  const featureRollback = getDocumentHistoryCursorBeforeTarget(items, {
    kind: 'feature',
    featureId: seedFeatureItem.featureId,
  })
  expectTrue(featureRollback !== null, 'Feature targets should resolve to a rollback cursor.')
  expectTrue(
  ...
  expectTrue(sketchRollback !== null, 'Sketch targets should resolve to a rollback cursor.')
  expectTrue(
  ...
  expectTrue(firstRollback?.kind === 'empty', 'The first history item should roll back to the empty cursor.')
  expectTrue(
  ...
  expectTrue(previous !== null, 'Undo should be available at the document history tail.')
  expectTrue(
    getDocumentHistoryCursorIndex(items, previous) === items.length - 2,
    'Previous document cursor should step back one history item.',
  )
  expectTrue(getNextDocumentHistoryCursor(snapshot) === null, 'Redo should be unavailable at the document history tail.')
  ...
  expectTrue(next !== null, 'Redo should be available after a document cursor rollback.')
  expectTrue(
  ...
  expectTrue(committed.revisionState.kind === 'accepted', 'History-order sketch commit should be accepted.')
