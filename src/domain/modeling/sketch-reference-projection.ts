...
function failedReference(
  ...
  return {
    ...
    diagnostics: [
      diagnostic(
        code,
        status === "missingSource" ? "error" : "warning",
        message,
      ),
    ],
  };
...
export function createProjectedGeometryId(
  referenceId: ReferenceId,
  suffix: string,
): ProjectedGeometryId {
...
function mapSketchPointToWorld(
...
function canUseExistingGeometry(
  ...
  if (
    reference.kind === "modelReference" &&
    reference.projectionMode === "projectAlongPlaneNormal"
  ) {
    return true;
  }
  ...
  return points.every(
    (point) =>
      Math.abs(pointPlaneDistance(plane, point)) <= tolerances.coincidence,
  );
}
...
function findRenderRecords(snapshot: WorkspaceSnapshot, target: DurableRef) {
  return snapshot.document.render.records.filter((record) =>
    sameTarget(record.binding.target, target),
  );
}
...
function isCollinear(points: readonly SketchPoint2D[], tolerance: number) {
  ...
  return points
    ...
    .every(
      (point) =>
        Math.abs((point[0] - start[0]) * dy - (point[1] - start[1]) * dx) /
          length <=
        tolerance,
    );
}
...
function circleFromPoints(points: readonly SketchPoint2D[], tolerance: number) {
  ...
  const maxError = Math.max(
    ...points.map((point) => Math.abs(distance2d(center, point) - radius)),
  );
  return maxError <= Math.max(tolerance, radius * 1e-4)
    ? { center, radius }
    : null;
}
...
function geometryFromWorldPolyline(input: {
  ...
  const projected = input.points.map((point) =>
    projectWorldPoint(input.plane, point),
  );
  const unique = projected.filter(
    (point, index) =>
      index === 0 ||
      distance2d(point, projected[index - 1]!) >
        input.tolerances.minimumSegmentLength,
  );
  const closesOnStart =
    unique.length >= 2 &&
    distance2d(unique[0]!, unique.at(-1)!) <=
      input.tolerances.minimumSegmentLength;
  ...
  if (
    !isClosed &&
    isCollinear(candidate, input.tolerances.minimumSegmentLength)
  ) {
    return {
  ...
  return {
    ...
    endPosition: candidate.at(-1)!,
    sweepDirection:
      signedPolylineArea(candidate) < 0 ? "clockwise" : "counterClockwise",
  };
}
...
function projectModelVertex(
  ...
  if (!marker || marker.geometry.kind !== "marker") {
    return failedReference(
      ...
      "missingSource",
      "missing-model-vertex-source",
      `Model vertex ${reference.referenceId} does not resolve in the requested revision.`,
    );
  }

  if (
    !canUseExistingGeometry(
      reference.reference,
    ...
    return failedReference(
      reference.referenceId,
      "outOfPlane",
      "projection-source-out-of-plane",
      `Model vertex ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
...
function projectModelEdge(
  ...
  const target = reference.reference.source;
  const records = findRenderRecords(snapshot, target);
  const polyline = records.find(
    (record) => record.geometry.kind === "polyline",
  );
  if (!polyline || polyline.geometry.kind !== "polyline") {
    return failedReference(
      reference.referenceId,
      "missingSource",
      "missing-model-edge-source",
      `Model edge ${reference.referenceId} does not resolve in the requested revision.`,
    );
  }
  ...
  if (
    ...
    return failedReference(
      ...
      "outOfPlane",
      "projection-source-out-of-plane",
      `Model edge ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
  ...
  return geometry
    ...
    : failedReference(
        reference.referenceId,
        "unsupportedSource",
        "unsupported-model-edge-source",
        `Model edge ${reference.referenceId} cannot be represented as a projected line, circle, or arc.`,
      );
}

function boundaryEdgesFromTriangles(
...
function projectModelFace(
  ...
  const meshGeometry =
    meshRecord?.geometry.kind === "mesh" ? meshRecord.geometry : null;
  if (!meshGeometry) {
    return failedReference(
      reference.referenceId,
      "missingSource",
      "missing-model-face-source",
      `Model face ${reference.referenceId} does not resolve in the requested revision.`,
    );
  }
  ...
  if (
    ...
    return failedReference(
      ...
      "outOfPlane",
      "projection-source-out-of-plane",
      `Model face ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
  ...
  if (boundary.length < 3) {
    return failedReference(
      reference.referenceId,
      "ambiguous",
      "ambiguous-model-face-boundary",
      `Model face ${reference.referenceId} does not expose a representable boundary.`,
    );
  }

  const geometry = boundary.map(
    ([startIndex, endIndex], index): ProjectedSketchReferenceGeometry => ({
      geometryId: createProjectedGeometryId(
        reference.referenceId,
        `face-edge-${index + 1}`,
      ),
      kind: "lineSegment",
      startPosition: projectWorldPoint(
        plane,
        meshGeometry.vertexPositions[startIndex]!,
      ),
      endPosition: projectWorldPoint(
        plane,
        meshGeometry.vertexPositions[endIndex]!,
      ),
    }),
  );
...
function projectSketchPoint(
  ...
  if (source.kind !== "sketchPoint") {
    return failedReference(
      ...
      `Sketch reference ${reference.referenceId} does not target a sketch point.`,
    );
  }

  const sketch = snapshot.document.sketches.find(
    (entry) => entry.sketchId === source.sketchId,
  );
  ...
  if (!sketch || !point) {
    return failedReference(
      reference.referenceId,
      "missingSource",
      "missing-sketch-point-source",
      `Sketch point ${reference.referenceId} does not resolve in the requested revision.`,
    );
  }
  ...
  const world = mapSketchPointToWorld(
    sketch.plane.frame,
    solvedPoint?.solvedPosition ?? point.position,
  );
  if (
    !canUseExistingGeometry(reference.reference, [world], plane, tolerances)
  ) {
    return failedReference(
      reference.referenceId,
      "outOfPlane",
      "projection-source-out-of-plane",
      `Sketch point ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
...
function projectSketchEntityGeometry(input: {
  ...
  const sketch = input.snapshot.document.sketches.find(
    (entry) => entry.sketchId === input.sourceSketchId,
  );
  const entity = sketch?.sketch.definition.entities.find(
    (entry) => entry.entityId === input.sourceEntityId,
  );
  if (!sketch || !entity) {
    return null;
  }
...
function projectSketchEntity(
  ...
  if (source.kind !== "sketchEntity") {
    return failedReference(
      ...
      "unsupportedSource",
      "unsupported-sketch-reference-source",
      `Sketch reference ${reference.referenceId} does not target a sketch entity.`,
    );
  }
  ...
  if (!geometry) {
    return failedReference(
      ...
      "missingSource",
      "missing-sketch-entity-source",
      `Sketch entity ${reference.referenceId} references missing sketch points.`,
    );
  }
  if (geometry.length === 0) {
    return failedReference(
      reference.referenceId,
      "unsupportedSource",
      "unsupported-sketch-entity-source",
      `Sketch entity ${reference.referenceId} cannot be represented as projected point, line, circle, or arc geometry.`,
    );
  }

  const worldPoints = sketch.sketch.definition.points.map((point) =>
    mapSketchPointToWorld(sketch.plane.frame, point.position),
  );
  if (
    !canUseExistingGeometry(reference.reference, worldPoints, plane, tolerances)
  ) {
    return failedReference(
      reference.referenceId,
      "outOfPlane",
      "projection-source-out-of-plane",
      `Sketch entity ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
...
function projectWholeSketch(
  ...
  if (source.kind !== "sketch") {
    return failedReference(
      ...
      "unsupportedSource",
      "unsupported-sketch-reference-source",
      `Sketch reference ${reference.referenceId} does not target a sketch.`,
    );
  }
  ...
  const sourcePoints = sketch.sketch.definition.points.map((point) =>
    mapSketchPointToWorld(sketch.plane.frame, point.position),
  );
  if (
    !canUseExistingGeometry(
    ...
    return failedReference(
      ...
      "outOfPlane",
      "projection-source-out-of-plane",
      `Sketch ${reference.referenceId} is not coplanar with the active sketch plane.`,
    );
  }
  ...
  return geometry.length > 0
    ...
    : failedReference(
        reference.referenceId,
        "unsupportedSource",
        "unsupported-sketch-source",
        `Sketch ${reference.referenceId} does not contain supported projectable geometry.`,
      );
}
...
function projectReference(
  ...
  const source = reference.reference.source;
  if (
    (source.kind === "sketchPoint" ||
      source.kind === "sketchEntity" ||
      source.kind === "sketch") &&
...
export function projectSketchExternalReferencesFromSnapshot(
  ...
  if (
    snapshot.document.documentId !== request.documentId ||
    snapshot.document.revisionId !== request.revisionId
  ) {
    const diagnostics = [
      diagnostic(
  ...
  const projectedReferences = request.references.map((reference) =>
    projectReference(snapshot, reference, request),
  );
  return {
    ...responseBase(request),
    projectedReferences,
    diagnostics: projectedReferences.flatMap(
