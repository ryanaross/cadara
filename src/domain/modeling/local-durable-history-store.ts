...
export function createMemoryLocalDurableHistoryStore(
  ...
  return {
    ...
    async load({ documentId, scope }) {
      return {
        ok: true,
        value: structuredClone(states.get(createScopedStorageKey(documentId, scope)) ?? createEmptyDocumentLocalDurableHistoryState()),
      }
    },
...
export function createIndexedDbLocalDurableHistoryStore(
  ...
  async function withStore<T>(
    ...
    try {
      const database = await openLocalDurableHistoryDatabase(indexedDBFactory!, databaseName, storeName)
      try {
  ...
  return {
    ...
    async load({ documentId, scope }) {
      const result = await withStore('readonly', (store) =>
        store.get(createScopedStorageKey(documentId, scope)) as IDBRequest<LocalDurableHistoryRecord | null>,
      )
      if (!result.ok) {
        return result
      }

      const parsed = parseDocumentLocalDurableHistoryState(result.value?.state ?? createEmptyDocumentLocalDurableHistoryState())
      if (!parsed.ok) {
        return {
          ok: false,
          reason: 'failed',
    ...
    async clear({ documentId, scope }) {
      return withStore('readwrite', (store) => store.delete(createScopedStorageKey(documentId, scope)) as unknown as IDBRequest<null>)
    },
  }
}
...
function openLocalDurableHistoryDatabase(
  ...
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error('Local durable history database failed to open.'))
  })
}
...
function resolveIdbRequest<T>(request: IDBRequest<T>) {
  return new Promise<T>((resolve, reject) => {
    request.onsuccess = () => resolve(request.result)
    request.onerror = () => reject(request.error ?? new Error('Local durable history request failed.'))
  })
}
