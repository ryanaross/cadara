...
export class MemoryGeometryAssetStore implements GeometryAssetStore {
  ...
  async put(input: GeometryAssetBlobInput): Promise<GeometryAssetStorePutResult> {
    const validation = await validateGeometryAssetBytes(input.asset, input.bytes)
    if (!validation.ok) {
      return validation
    }
    ...
    if (existing) {
      const existingValidation = await validateGeometryAssetBytes(input.asset, existing)
      if (existingValidation.ok) {
        return { ok: true, hash: input.asset.hash, byteLength: existing.byteLength, deduped: true }
      }
    }

    this.blobs.set(input.asset.hash, input.bytes.slice())
    return { ok: true, hash: input.asset.hash, byteLength: input.bytes.byteLength, deduped: false }
  }
  ...
  async get(asset: GeometryAssetRecord): Promise<GeometryAssetStoreGetResult> {
    ...
    if (!bytes) {
      return {
        ok: false,
        diagnostic: createAssetStoreDiagnostic('geometry-asset-missing', asset, 'Referenced geometry asset bytes are missing.'),
      }
    }
...
export class IndexedDbGeometryAssetStore implements GeometryAssetStore {
  ...
  private async openDatabase() {
    ...
    this.databasePromise ??= new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.onerror = () => reject(request.error ?? new Error('Geometry asset database could not be opened.'))
  ...
  private async read(hash: GeometryAssetHash) {
    ...
    return new Promise<Uint8Array | null>((resolve, reject) => {
      const request = database.transaction(this.storeName, 'readonly').objectStore(this.storeName).get(hash)
      request.onerror = () => reject(request.error ?? new Error('Geometry asset read failed.'))
      request.onsuccess = () => {
  ...
  private async write(hash: GeometryAssetHash, bytes: Uint8Array) {
    ...
    return new Promise<void>((resolve, reject) => {
      const request = database.transaction(this.storeName, 'readwrite').objectStore(this.storeName).put(bytes.slice(), hash)
      request.onerror = () => reject(request.error ?? new Error('Geometry asset write failed.'))
...
export function createIndexedDbGeometryAssetStore(options?: IndexedDbGeometryAssetStoreOptions) {
...
export async function validateGeometryAssetBytes(
  ...
  if (bytes.byteLength !== asset.byteLength) {
    return {
      ok: false,
      diagnostic: createAssetStoreDiagnostic('geometry-asset-corrupt', asset, 'Geometry asset byte length does not match the authored manifest.'),
    }
  }
  ...
  if (hash !== asset.hash) {
    return {
      ok: false,
      diagnostic: createAssetStoreDiagnostic('geometry-asset-corrupt', asset, 'Geometry asset content hash does not match the authored manifest.'),
    }
  }

  return { ok: true, hash: asset.hash, byteLength: bytes.byteLength, deduped: false }
}

export async function hashGeometryAssetBytes(bytes: Uint8Array): Promise<GeometryAssetHash> {
...
export async function collectAssetAvailability(
  ...
  for (const asset of records) {
    ...
    if (!result.available) {
      diagnostics.push(createAssetStoreDiagnostic(
...
export async function storeGeometryAssetInputsForManifest(
  ...
  const manifestById = new Map(manifestRecords.map((record) => [record.assetId, record]))
  ...
  for (const input of assetInputs) {
    ...
    if (!manifestRecord || !sameManifestAssetRecord(manifestRecord, input.asset)) {
      return {
        ok: false as const,
        diagnostic: {
    ...
    const validation = await validateGeometryAssetBytes(manifestRecord, input.bytes)
    if (!validation.ok) {
      return validation
    }
...
export function filterGeometryAssetInputsForManifest(
  ...
  return assetInputs.filter((input) => {
    const manifestRecord = manifestById.get(input.asset.assetId)
    return Boolean(manifestRecord && sameManifestAssetRecord(manifestRecord, input.asset))
  })
}

function sameManifestAssetRecord(left: GeometryAssetRecord, right: GeometryAssetRecord) {
...
function createAvailability(asset: GeometryAssetRecord, available: boolean): GeometryAssetAvailability {
  return {
...
function createStorageFailureDiagnostic(asset: GeometryAssetRecord, error: unknown) {
