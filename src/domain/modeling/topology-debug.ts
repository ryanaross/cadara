...
function isTopologyReference(
  reference: ReferenceRecord,
): reference is ReferenceRecord & {
  target: { kind: TopologyKind; bodyId: string };
} {
  return (
    reference.target.kind === "face" ||
...
export function createTopologyDebugSummary(
  snapshot: WorkspaceSnapshot | null,
): TopologyDebugSummary {
  if (!snapshot) {
    return {
      bodyCount: 0,
  ...
  const topologyReferences =
    snapshot.document.references.filter(isTopologyReference);
  const liveTopologyReferences = topologyReferences.filter(
    (reference) => reference.invalidation === null,
  );
  const invalidatedTopologyReferences = topologyReferences.filter(
    (reference) => reference.invalidation !== null,
  );
  ...
  for (const reference of invalidatedTopologyReferences) {
    ...
    invalidationsByReason.set(reason, [
      ...(invalidationsByReason.get(reason) ?? []),
      reference,
    ]);
  }
  ...
  return {
    ...
    bodies: snapshot.document.bodies.map((body) => {
      const bodyReferences = topologyReferences.filter(
        (reference) => reference.target.bodyId === body.bodyId,
      );
      ...
      return {
        ...
        liveReferences: bodyReferences.filter(
          (reference) => reference.invalidation === null,
        ).length,
        invalidatedReferences: bodyReferences.filter(
          (reference) => reference.invalidation !== null,
        ).length,
      };
    }),
