...
import type { DocumentVariableRecord, ModelingDiagnostic } from '@/contracts/modeling/schema'
...
export function evaluateDocumentVariableExpressions(
  ...
  const dependenciesByName = collectDependencies(parsedVariables.variables, variablesByName)

  if (!dependenciesByName.ok) {
    return dependenciesByName
  }

  return evaluateInDependencyOrder(parsedVariables.variables, dependenciesByName.dependenciesByName)
}

function parseVariables(
  variables: readonly DocumentVariableRecord[],
): { ok: true; variables: readonly ParsedDocumentVariable[] } | { ok: false; diagnostics: readonly DocumentVariableExpressionDiagnostic[] } {
  const diagnostics: DocumentVariableExpressionDiagnostic[] = []
  ...
  return diagnostics.length > 0 ? { ok: false, diagnostics } : { ok: true, variables: parsedVariables }
}
...
function collectDependencies(
...
): { ok: true; dependenciesByName: ReadonlyMap<string, readonly string[]> } | { ok: false; diagnostics: readonly DocumentVariableExpressionDiagnostic[] } {
  const diagnostics: DocumentVariableExpressionDiagnostic[] = []
  ...
  return diagnostics.length > 0 ? { ok: false, diagnostics } : { ok: true, dependenciesByName }
}
...
function evaluateInDependencyOrder(
  ...
  const parsedByName = new Map(parsedVariables.map((parsed) => [parsed.variable.name, parsed]))
  ...
  const evaluateName = (name: string): boolean => {
    ...
    if (visitingNames.includes(name)) {
      const cycle = [...visitingNames.slice(visitingNames.indexOf(name)), name].join(' -> ')
      diagnostics.push({
