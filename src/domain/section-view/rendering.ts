...
import type { SectionViewSession, Vec3 } from '@/domain/section-view/session'
import {
  add,
  cross,
  dot,
...
export function createSectionCapRenderables(
  ...
  const faceRenderables = renderables.filter((renderable) =>
  ...
  faceRenderables.forEach((renderable) => {
    const key = renderable.ownerBodyId
      ?? (renderable.binding.target.kind === 'face' ? renderable.binding.target.bodyId : renderable.id)
...
export function resolveSectionDragOffset(input: {
  ...
  if (Math.abs(denominator) <= SECTION_TOLERANCE) {
    return dot(subtract(input.pointerRayOrigin, input.section.plane.frame.origin), axisDirection)
  }
...
function buildSectionCapsForGroup(
  ...
  return loops.flatMap((loop, index) => {
    const projected = loop.map((point) => projectPointToSectionPlane(section.plane.frame, point))

    if (Math.abs(getSignedLoopArea(projected)) <= SECTION_TOLERANCE) {
      return []
    }

    const contour = getSignedLoopArea(projected) >= 0 ? projected : [...projected].reverse()
    ...
    const normal = section.retainedSide === 'positive'
    ...
    return [{
      ...
      textureCoordinates: contour.map((point) => [point[0], point[1]] as const),
      triangleIndices: triangles.map((triangle) => [triangle[0], triangle[1], triangle[2]] as const),
    }]
  })
}
...
function collectSectionTrianglePoints(
  ...
  const distances = triangle.map((point) => getSectionPlaneSignedDistance(section, point))
  const [p0, p1, p2] = triangle
  const [d0, d1, d2] = distances

  if (
    (d0 > SECTION_TOLERANCE && d1 > SECTION_TOLERANCE && d2 > SECTION_TOLERANCE)
    || (d0 < -SECTION_TOLERANCE && d1 < -SECTION_TOLERANCE && d2 < -SECTION_TOLERANCE)
  ) {
    return []
  }
...
function collectSectionEdgePoints(start: Vec3, startDistance: number, end: Vec3, endDistance: number) {
  if (Math.abs(startDistance) <= SECTION_TOLERANCE && Math.abs(endDistance) <= SECTION_TOLERANCE) {
    return [start, end]
  }
  ...
  for (const [startKey, neighbors] of adjacency) {
    for (const nextKey of neighbors) {
      ...
      while (currentKey !== startKey) {
        ...
        const candidateKey = currentNeighbors.find((neighborKey) =>
          neighborKey !== previousKey && !visitedEdges.has(getEdgeKey(currentKey, neighborKey)),
...
export function createSectionHatchTexture(spacing = 8, size = 64) {
  ...
  context.strokeStyle = rootStyles.getPropertyValue('--workbench-shell-text').trim() || 'black'
  context.lineWidth = 2
...
export function getSectionRenderableBounds(renderables: readonly RenderableEntityRecord[]) {
...
export function getSectionPlaneBasis(frame: SectionViewSession['plane']['frame']) {
