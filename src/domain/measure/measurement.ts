...
export type MeasurementWitness = MeasurementWitnessPolyline | MeasurementWitnessMarker
...
export function resolveMeasureSelectionCandidate(
  ...
  if (selection.length === 1 && supportsPairwiseMeasurement(currentTarget, nextTarget)) {
    return {
      accepted: true,
...
export function deriveMeasurementViewModel(input: {
  ...
  const resolvedSelection = input.selection
    .slice(0, 2)
    .flatMap((target) => {
...
function resolveBodyTarget(
  ...
  const bodyTriangles = faceRenderables.flatMap((renderable) => trianglesFromMesh(renderable.geometry))
  const surfaceArea = faceRenderables.length === body.topology.faceIds.length
    ? faceRenderables.reduce((sum, renderable) => sum + getMeshArea(renderable.geometry), 0)
    : null
  const volume = faceRenderables.length === body.topology.faceIds.length && bodyTriangles.length > 0
    ? getClosedMeshVolume(bodyTriangles)
    : null

  const rows = [
    ...(surfaceArea === null ? [] : [{ id: 'surface-area', label: 'Surface Area', value: formatArea(surfaceArea) }]),
    ...(volume === null ? [] : [{ id: 'volume', label: 'Volume', value: formatVolume(volume) }]),
  ]

  const witnesses = edgeRenderables.flatMap((renderable, index) => polylineWitnessFromPoints(
  ...
  return {
    ...
    rows,
    note: rows.length > 0 ? null : 'Surface area and volume are unavailable for this body in the current snapshot.',
    witnesses,
  }
}
...
function resolveFaceTarget(
  ...
  const perimeter = boundaryEdges.reduce((sum, [start, end]) => sum + distanceBetween(start, end), 0)
...
function resolveEdgeTarget(
  ...
  const length = polylineLength(renderable.geometry.points, renderable.geometry.isClosed)
...
function resolveRegionTarget(
  ...
  const region = sketch?.sketch.regions.find((entry) => entry.regionId === target.regionId)
  ...
  const perimeter = loopMeasurements.reduce((sum, entry) => sum + entry.length, 0)
  const area = Math.abs(loopMeasurements.reduce((sum, entry) => sum + entry.signedArea, 0))
  ...
  return {
    ...
    label: getTargetLabel(snapshot, target),
    category: 'surface',
    distancePrimitive: area > DISTANCE_EPSILON
      ? {
          kind: 'mesh',
          triangles: triangulateLoopFan(loopMeasurements[0]?.worldPoints ?? []),
        }
      : null,
    rows: [
      { id: 'area', label: 'Area', value: formatArea(area) },
    ...
    note: loopMeasurements.length === 0 ? 'Region boundary geometry is unavailable.' : null,
    witnesses,
  }
}
...
function resolveSketchPointTarget(
  ...
  const sketch = snapshot.document.sketches.find((entry) => entry.sketchId === target.sketchId)
  const point = sketch?.sketch.definition.points.find((entry) => entry.pointId === target.pointId)
...
function resolveSketchEntityTarget(
  ...
  const sketch = snapshot.document.sketches.find((entry) => entry.sketchId === target.sketchId)
  const entity = sketch?.sketch.definition.entities.find((entry) => entry.entityId === target.entityId)
  ...
  if (entity.kind === 'point') {
    const point = sketch.sketch.definition.points.find((entry) => entry.pointId === entity.pointId)
    return point
      ? resolveSketchPointTarget(snapshot, { kind: 'sketchPoint', sketchId: target.sketchId, pointId: point.pointId })
      : null
  }
  ...
  if (entity.kind === 'lineSegment') {
    ...
    return createCurveTarget({
      ...
      key: getPrimitiveRefKey(target),
      polyline: [start, end],
      rows: [{ id: 'length', label: 'Length', value: formatLength(distanceBetween(start, end)) }],
    })
  }
  ...
  if (entity.kind === 'circle') {
    ...
    return createCurveTarget({
      ...
      rows: [
        ...
        { id: 'diameter', label: 'Diameter', value: formatLength(entity.radius * 2) },
        { id: 'circumference', label: 'Circumference', value: formatLength(Math.PI * entity.radius * 2) },
      ],
    })
  }
  ...
  if (entity.kind === 'arc') {
    ...
    const sweepRadians = getArcSweepRadians(center, start, end, entity.sweepDirection)
    const chordLength = distanceBetween2D(start, end)
    const polyline = sampleArcPoints(sketch, center, radius, start, end, entity.sweepDirection)
    ...
    return createCurveTarget({
      ...
      rows: [
        ...
        { id: 'chord-length', label: 'Chord Length', value: formatLength(chordLength) },
        { id: 'arc-length', label: 'Arc Length', value: formatLength(radius * sweepRadians) },
      ],
    })
  }
  ...
  if (entity.kind === 'spline') {
    ...
    return createCurveTarget({
      ...
      rows: [
        { id: 'length', label: 'Length', value: formatLength(polylineLength(sampledPoints, false)) },
        { id: 'closed', label: 'Closed', value: 'No' },
        { id: 'degree', label: 'Degree', value: String(entity.degree) },
        { id: 'fit-points', label: 'Fit Points', value: String(entity.fitPointIds.length) },
      ],
    })
  }
...
function resolveProjectedGeometryTarget(
  ...
  const sketch = snapshot.document.sketches.find((entry) =>
    entry.sketch.projectedReferences?.some((reference) => reference.referenceId === target.referenceId),
  )
  ...
  const geometry = reference?.geometry.find((entry) => entry.geometryId === target.geometryId)
  ...
  if (geometry.kind === 'point') {
    const position = mapSketchPointToWorkspaceWorld(sketch.plane, geometry.position)
    return {
      target,
      key: getPrimitiveRefKey(target),
      label: getTargetLabel(snapshot, target),
  ...
  if (geometry.kind === 'lineSegment') {
    ...
    return createCurveTarget({
      ...
      key: getPrimitiveRefKey(target),
      polyline,
      rows: [{ id: 'length', label: 'Length', value: formatLength(distanceBetween(polyline[0], polyline[1])) }],
    })
  }
  ...
  if (geometry.kind === 'circle') {
    ...
    return createCurveTarget({
      ...
      rows: [
        ...
        { id: 'diameter', label: 'Diameter', value: formatLength(geometry.radius * 2) },
        { id: 'circumference', label: 'Circumference', value: formatLength(Math.PI * geometry.radius * 2) },
      ],
    })
  }

  if (geometry.kind === 'arc') {
    const radius = distanceBetween2D(geometry.centerPosition, geometry.startPosition)
    ...
    return createCurveTarget({
      ...
      rows: [
        ...
        { id: 'diameter', label: 'Diameter', value: formatLength(radius * 2) },
        { id: 'sweep', label: 'Sweep', value: formatAngle(sweepRadians) },
        { id: 'chord-length', label: 'Chord Length', value: formatLength(distanceBetween2D(geometry.startPosition, geometry.endPosition)) },
        { id: 'arc-length', label: 'Arc Length', value: formatLength(radius * sweepRadians) },
      ],
    })
  }
  ...
  if (geometry.kind === 'spline') {
    const sampledPoints = sampleSplinePoints(
      geometry.fitPoints.map((point) => mapSketchPointToWorkspaceWorld(sketch.plane, point)),
      SPLINE_SEGMENTS,
    )
    return createCurveTarget({
      target,
      ...
      rows: [
        { id: 'length', label: 'Length', value: formatLength(polylineLength(sampledPoints, geometry.isClosed)) },
        { id: 'closed', label: 'Closed', value: geometry.isClosed ? 'Yes' : 'No' },
        { id: 'degree', label: 'Degree', value: String(geometry.degree) },
        { id: 'fit-points', label: 'Fit Points', value: String(geometry.fitPoints.length) },
      ],
    })
  }
...
function supportsPairwiseMeasurement(first: MeasuredTarget, second: MeasuredTarget) {
...
function measurePairwiseDistance(
  ...
  return measureDistanceBetweenPrimitives(first.distancePrimitive, second.distancePrimitive)
}
...
function resolvePairwiseAngleRow(
  ...
  if (firstMagnitude <= DISTANCE_EPSILON || secondMagnitude <= DISTANCE_EPSILON) {
    return []
  }

  const cosine = clamp(
    Math.abs(dotProduct(firstDirection, secondDirection) / (firstMagnitude * secondMagnitude)),
    -1,
    1,
  )
...
function getLinearSegmentFromPrimitive(
  ...
  if (!primitive || primitive.kind !== 'polyline' || primitive.points.length !== 2) {
    return null
  }
...
function measureDistanceBetweenPrimitives(
  ...
  if (first.kind === 'polyline' && second.kind === 'point') {
    ...
    return result ? { distance: result.distance, start: result.end, end: result.start } : null
  }
...
function measurePolylineToPolyline(
  ...
  forEachSegment(first, false, (firstStart, firstEnd) => {
    forEachSegment(second, false, (secondStart, secondEnd) => {
      const segmentResult = closestPointsBetweenSegments(firstStart, firstEnd, secondStart, secondEnd)
...
function measurePolylineToMesh(
  ...
  forEachSegment(polyline, false, (segmentStart, segmentEnd) => {
    for (const triangle of triangles) {
      const segmentTriangle = closestPointsBetweenSegmentAndTriangle(segmentStart, segmentEnd, triangle)
...
function measureMeshToMesh(
  ...
  for (const firstTriangle of first) {
    for (const secondTriangle of second) {
      const triangleResult = closestPointsBetweenTriangles(firstTriangle, secondTriangle)
...
function closestPointsBetweenTriangles(
  ...
  for (const [start, end] of triangleEdges(second)) {
    ...
    candidates.push({ distance: reversed.distance, start: reversed.end, end: reversed.start })
  }

  for (const point of [first.first, first.second, first.third]) {
    const closest = closestPointOnTriangle(point, second)
    candidates.push({ distance: distanceBetween(point, closest), start: point, end: closest })
  }

  for (const point of [second.first, second.second, second.third]) {
    const closest = closestPointOnTriangle(point, first)
    candidates.push({ distance: distanceBetween(point, closest), start: closest, end: point })
  }
...
function getSketchDefinitionPoint(
  ...
  const point = sketch.sketch.definition.points.find((entry) => entry.pointId === pointId)
  return point ? mapSketchPointToWorkspaceWorld(sketch.plane, point.position) : null
}
...
function getSketchDefinitionPoint2D(
  ...
  return sketch.sketch.definition.points.find((entry) => entry.pointId === pointId)?.position ?? null
}
...
function sampleArcPoints(
  ...
  const normalizedEnd = normalizeArcEndAngle(startAngle, endAngle, sweepDirection)

  return Array.from({ length: ARC_SEGMENTS + 1 }, (_, index) => {
    const angle = startAngle + ((normalizedEnd - startAngle) * index) / ARC_SEGMENTS
    return mapSketchPointToWorkspaceWorld(sketch.plane, [
      center[0] + Math.cos(angle) * radius,
      center[1] + Math.sin(angle) * radius,
    ])
...
function sampleSplinePoints(
  ...
  if (points.length === 3) {
    return Array.from({ length: segmentCount + 1 }, (_, index) => {
      ...
      const oneMinusT = 1 - t
      return [
        oneMinusT * oneMinusT * points[0]![0] + 2 * oneMinusT * t * points[1]![0] + t * t * points[2]![0],
        oneMinusT * oneMinusT * points[0]![1] + 2 * oneMinusT * t * points[1]![1] + t * t * points[2]![1],
        oneMinusT * oneMinusT * points[0]![2] + 2 * oneMinusT * t * points[1]![2] + t * t * points[2]![2],
      ] as const
    })
  }
  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    for (let step = 0; step < segmentCount / (points.length - 1); step += 1) {
      const t = step / Math.max(1, Math.floor(segmentCount / (points.length - 1)))
...
function measureRegionLoops(
  ...
  return region.loops.flatMap((loop) => {
    ...
    const worldPoints = points.map((point) => mapSketchPointToWorkspaceWorld(sketch.plane, point))
    return [{
...
function getRegionLoopSketchPoints(
  ...
  const pointById = new Map(sketch.sketch.definition.points.map((point) => [point.pointId, point.position] as const))
...
function getMeshArea(geometry: RenderMeshGeometry) {
  return trianglesFromMesh(geometry).reduce((sum, triangle) => sum + triangleArea(triangle), 0)
}
...
function collectMeshBoundaryEdges(geometry: RenderMeshGeometry) {
  ...
  for (const [firstIndex, secondIndex, thirdIndex] of geometry.triangleIndices) {
    addBoundaryEdge(edges, geometry.vertexPositions[firstIndex], geometry.vertexPositions[secondIndex])
    addBoundaryEdge(edges, geometry.vertexPositions[secondIndex], geometry.vertexPositions[thirdIndex])
    addBoundaryEdge(edges, geometry.vertexPositions[thirdIndex], geometry.vertexPositions[firstIndex])
  }
...
function addBoundaryEdge(
  edges: Map<string, { count: number; start: WorkspaceVec3; end: WorkspaceVec3 }>,
...
function groupBoundaryLoops(edges: readonly (readonly [WorkspaceVec3, WorkspaceVec3])[]) {
  ...
  for (const [start, end] of edges) {
    ...
    let length = 0
    const stack: Array<readonly [WorkspaceVec3, WorkspaceVec3]> = [[start, end]]
...
function signedTetrahedronVolume(triangle: Triangle) {
  return dotProduct(triangle.first, crossProduct(triangle.second, triangle.third)) / 6
}

function getClosedMeshVolume(triangles: readonly Triangle[]) {
  const vertices = triangles.flatMap((triangle) => [triangle.first, triangle.second, triangle.third])
...
function orientTriangleOutward(
  triangle: Triangle,
  centroid: WorkspaceVec3,
) {
...
function closestPointOnTriangle(
  ...
  if (va <= 0 && (d4 - d3) >= 0 && (d5 - d6) >= 0) {
    ...
    return add(triangle.second, scale(subtract(triangle.third, triangle.second), w))
  }
...
function closestPointsBetweenSegments(
  ...
  if (
    ...
    return closestPointsBetweenParallelSegments(firstStart, firstEnd, secondStart, secondEnd)
  }
  ...
  if (a <= DISTANCE_EPSILON) {
    ...
    if (e <= DISTANCE_EPSILON) {
      ...
      firstT = denominator !== 0 ? clamp((b * f - c * e) / denominator, 0, 1) : 0
...
function closestPointsBetweenParallelSegments(
  ...
  const secondStartProjection = dotProduct(subtract(secondStart, firstStart), axis)
  ...
  if (overlapStart <= overlapEnd + DISTANCE_EPSILON) {
    ...
    const anchor = add(firstStart, scale(axis, clamp(overlapMidpoint, 0, firstLength)))
  ...
  return candidates.reduce((best, entry) =>
    distanceBetween(entry.start, entry.end) < distanceBetween(best.start, best.end) ? entry : best,
  )
}
...
function closestPointOnSegment(
  ...
  const t = clamp(dotProduct(subtract(point, start), direction) / lengthSquared, 0, 1)
  return add(start, scale(direction, t))
}
...
function getArcSweepRadians(
  ...
  const startAngle = Math.atan2(start[1] - center[1], start[0] - center[0])
  const endAngle = Math.atan2(end[1] - center[1], end[0] - center[0])
  return Math.abs(normalizeArcEndAngle(startAngle, endAngle, sweepDirection) - startAngle)
}
