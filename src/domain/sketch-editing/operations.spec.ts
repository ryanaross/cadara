...
test('src/domain/sketch-editing/operations.spec.ts', () => {
  ...
  function makePoint(pointId: string, label: string, position: SketchPoint) {
    return {
      ...
      label,
      target: { kind: 'sketchPoint', sketchId: 'sketch_primary' as SketchId, pointId: pointId as SketchPointId },
      position,
  ...
  function makeLine(entityId: string, label: string, startPointId: string, endPointId: string) {
    return {
      ...
      label,
      target: { kind: 'sketchEntity', sketchId: 'sketch_primary' as SketchId, entityId: entityId as SketchEntityId },
      isConstruction: false,
  ...
  function makeDefinition(points: SketchDefinition['points'], entities: SketchDefinition['entities']): SketchDefinition {
    return {
  ...
  function createFactories(): SketchEditOperationFactories {
    return {
      ...
      createEntityId: (suffix) => `sketch_entity_10_${suffix}` as SketchEntityId,
      createPoint: (label, pointId, position) => ({
        pointId,
        label,
        target: { kind: 'sketchPoint', sketchId: 'sketch_primary' as SketchId, pointId },
        position,
        isConstruction: false,
      }),
      createLineEntity: (label, entityId, startPointId, endPointId) => ({
        ...
        entityId,
        label,
        target: { kind: 'sketchEntity', sketchId: 'sketch_primary' as SketchId, entityId },
        isConstruction: false,
        startPointId,
        endPointId,
      }),
