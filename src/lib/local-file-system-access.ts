...
export interface LocalFileSystemFileHandle {
  ...
  queryPermission?(descriptor: LocalFileSystemPermissionDescriptor): Promise<LocalFileSystemPermissionState>
  requestPermission?(descriptor: LocalFileSystemPermissionDescriptor): Promise<LocalFileSystemPermissionState>
}
...
export interface LocalFileSystemPickerEnvironment {
  ...
  showOpenFilePicker?: (options: LocalFileSystemOpenPickerOptions) => Promise<LocalFileSystemFileHandle[]>
  showSaveFilePicker?: (options: LocalFileSystemSavePickerOptions) => Promise<LocalFileSystemFileHandle>
}

export type LocalFileSystemSupport =
  | { supported: true }
  | { supported: false; reason: 'insecure-context' | 'missing-open-picker' | 'missing-save-picker' }
...
export async function showSaveLocalDocumentPicker(
  ...
  try {
    return { ok: true, handle: await showSaveFilePicker(CADARA_SAVE_FILE_PICKER_OPTIONS) }
  } catch (error: unknown) {
    return isPickerAbort(error)
      ? { ok: false, reason: 'cancelled' }
      : { ok: false, reason: 'failed', error }
...
export async function ensureLocalFileWritePermission(handle: LocalFileSystemFileHandle) {
...
export async function createLocalFileWritableStream(handle: LocalFileSystemFileHandle) {
  ...
  try {
    ...
    if (writable?.abort) {
      try {
        ...
        console.warn('Local file writable abort failed after write failure.', abortError)
      }
    }
