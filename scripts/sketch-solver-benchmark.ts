...
let interactiveSink: ReturnType<typeof updateCompiledSketchSolveSession> | null = null

function assertFixtureResult(fixture: SketchSolverBenchmarkFixture, result: SketchSolverBenchmarkEvaluation) {
...
function assertFullSolveResult(
  ...
  if (result.solveState !== 'solved') {
    throw new Error(`${fixture.name} did not solve; received ${result.solveState}.`)
  }
  if (result.regionCount !== fixture.expectedRegionCount) {
    throw new Error(`${fixture.name} expected ${fixture.expectedRegionCount} regions, received ${result.regionCount}.`)
  }
...
function createInteractiveDragBenchmarkState(fixture: SketchSolverBenchmarkFixture): {
...
if (!benchmarkSink) {
  throw new Error('Sketch solver full-solve benchmark did not execute any fixture.')
}

if (!interactiveSink) {
  throw new Error('Sketch solver interactive benchmark did not execute any fixture.')
}
