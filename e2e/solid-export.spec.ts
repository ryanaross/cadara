...
test('solid STL export downloads through the browser worker path', async ({ page }) => {
  const workbench = new FeatureWorkbenchHarness(page)

  await workbench.openWithBaseExtrudeFixture()

  const bodyRow = page.locator('[data-parts-tree-row]').filter({ hasText: 'feature_extrude-1' }).first()
  await expect(bodyRow).toBeVisible({ timeout: 30_000 })
  await bodyRow.click({ button: 'right', force: true })

  await page.getByRole('menuitem', { name: 'Export' }).click()
  await expect(page.getByRole('dialog', { name: 'Export feature_extrude-1' })).toBeVisible({ timeout: 10_000 })
  ...
  expect(bytes.subarray(0, 24).toString('utf8')).toContain('cadara binary stl export')
})
