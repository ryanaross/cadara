...
test("toolbar file menu downloads and opens a cadara document copy", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("menuitem", { name: "New document", exact: true }),
  ).toHaveCount(0);
  ...
  await expect(
    page.getByRole("status").filter({ hasText: "Created a new document." }),
  ).toBeVisible({
    timeout: 10_000,
  });
  await page
    .locator(
      '[data-document-tab][aria-selected="true"] [data-document-tab-close]',
    )
    .click();
  await expect(
    page.getByText("This document is stored only in this browser."),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Download a copy" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Save and keep linked" }),
  ).toBeVisible();
  ...
  if (!exported.settings) {
    throw new Error(
      "Exported cadara document did not contain document settings.",
    );
  }
  ...
  await expect(
    page.getByRole("status").filter({ hasText: "Opened imported.cadara." }),
  ).toBeVisible({
    timeout: 10_000,
  });
