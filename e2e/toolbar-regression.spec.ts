...
test('pattern dropdown stays responsive without restarting shortcut profile loads', async ({
  ...
  await page.addInitScript((storageKey) => {
    ...
    Storage.prototype.getItem = function getItemWithShortcutLoadCount(key: string) {
      if (key === storageKey) {
        const trackedWindow = window as ShortcutLoadWindow
        trackedWindow.__shortcutProfileLoadCount = (trackedWindow.__shortcutProfileLoadCount ?? 0) + 1
      }
