...
test("section handle drags forward and back along the active section normal", async ({
  page,
}) => {
  ...
  await expect
    .poll(
      () => page.evaluate(() => window.__cadProjectSectionHandleToScreen?.()),
      { timeout: 10_000 },
    )
    .not.toBeNull();
  ...
  await page.mouse.move(
    viewportBox.x + projection.handle.x,
    viewportBox.y + projection.handle.y,
  );
  ...
  await expect
    .poll(
      () =>
        page.evaluate(
          () => window.__cadaraDebug?.getState()?.sectionOffset ?? null,
        ),
      { timeout: 10_000 },
    )
  ...
  expect(
    Math.abs((movedOffset as number) - (initialSection.offset as number)),
  ...
  await expect
    .poll(
      async () =>
        Math.abs(
          (await page.evaluate(
            () => window.__cadaraDebug?.getState()?.sectionOffset ?? 999,
          )) as number,
        ),
      { timeout: 10_000 },
    )
