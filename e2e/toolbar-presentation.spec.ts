...
async function expectToolbarToUseAtMostTwoToolRows(page: Page) {
  const rowTops = await page.evaluate(() => {
    const toolbar = document.querySelector('[role="toolbar"][aria-label="CAD tools"]')
    if (!toolbar) {
    ...
    return [...new Set(buttons.map((button) => Math.round(button.getBoundingClientRect().top)))]
      .sort((a, b) => a - b)
  })
...
async function expectToolbarToolsToAvoidSearch(page: Page) {
  const overlappingToolIds = await page.evaluate(() => {
    ...
    const search = document.querySelector<HTMLInputElement>('[placeholder="Search tools"]')
    if (!search) {
    ...
    return buttons
      .filter((button) => {
        ...
        return buttonRect.left < searchRect.right
          && buttonRect.right > searchRect.left
          && buttonRect.top < searchRect.bottom
          && buttonRect.bottom > searchRect.top
      })
      .map((button) => button.getAttribute('data-tool-id') ?? button.getAttribute('data-tool-dropdown-trigger'))
  })
...
async function expectToolbarPillShadowsToHaveBreathingRoom(page: Page) {
  const shadowLayout = await page.evaluate(() => {
    const primaryRail = document.querySelector<HTMLElement>('[data-toolbar-primary-rail]')
    if (!primaryRail) {
      throw new Error('Toolbar primary rail was not found.')
    }

    const historySection = primaryRail.querySelector<HTMLElement>('[data-toolbar-section="history"]')
    if (!historySection) {
    ...
    const rightMostPrimarySection = Math.max(
      ...primarySections.map((section) => section.getBoundingClientRect().right),
    )

    return {
      historyLeftShadowGutter: Math.round(historyRect.left - primaryRailRect.left),
