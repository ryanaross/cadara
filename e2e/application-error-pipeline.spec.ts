...
test('invalid variable edit reports a UI status and actionable console error', async ({ page }) => {
  ...
  await expect(page.getByRole('status').filter({ hasText: /missing/i })).toBeVisible({ timeout: 30_000 })
