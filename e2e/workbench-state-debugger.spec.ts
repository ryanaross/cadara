...
test("state debugger owns workbench debug readouts and collapses locally", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Collapse state debugger" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Collapse state debugger" }).click();
  await expect(page.getByText("Active mode")).toHaveCount(0);
});

test("feature inspector omits debugger-only contract and revision readouts", async ({
  page,
}) => {
...
test("diagnostics and debugger stay in-frame without page scrollbars", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
  ...
  await expect(
    page.getByText("Snapshot References", { exact: true }),
  ).toHaveCount(0);
