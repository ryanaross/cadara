...
const FACE_TARGET =
  "body_feature_extrude-1.face_body_feature_extrude-1_g6c121d31a8b2062a";
const EDGE_TARGET =
  "body_feature_extrude-1.edge_body_feature_extrude-1_gf733a84eddab6da7";
const VERTEX_TARGET =
  "body_feature_extrude-1.vertex_body_feature_extrude-1_gcacb85faae727c4a";
...
test("front-most solid face hover and click change the rendered crop deterministically", async ({
  page,
}) => {
...
test("edge priority and enlarged vertex hitboxes resolve at fixed viewport points", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
...
test("reload preserves the same topology targets and blank-space behavior", async ({
  page,
}) => {
  const workbench = new FeatureWorkbenchHarness(page);
