...
test('importing a sketch reference image through the file chooser should add sketch history and change the viewport', async ({ page }) => {
  ...
  await workbench.activateTool('Import one or more reference images into the active sketch.')
  ...
  const historyItemCount = await sketchHistory.getByRole('button', { name: /reference\.png/i }).count()
  const emptyHistoryVisible = await sketchHistory.getByText('Empty sketch history').count()
